...
- **総合評価の配点を実装済み（2026-09-14・本番未デプロイ）**: 定量:定性 ＝ マーケ/BPO 60:40・営業/CA 70:30（役員確定）。各メンバーカードに「📊 総合評価」ブロックを追加し、定量得点・定性得点・総合点（100点満点）と昇格判定チェックリスト5項目を表示する。定性は `neweval_qualitative` から取得し、未入力は0点にせず保留する
