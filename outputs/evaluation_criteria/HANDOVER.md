# 引き継ぎ文書 — LET評価制度プロジェクト（let-hyoka）【旧評価シート・保管】

最終更新: 2026-08-25 ／ 現行作業ブランチ: claude/evaluation-finance-dashboard-d0u7iz（50w8t9 の全作業を取り込み済み。PR #22 まではmainへマージ済み）

> ⚠️ **【2026-08-25 決定】本プロジェクトの評価シート一式は「旧評価シート」として保管扱いに移行。**
> 新評価シートは別プロジェクトとして作成する（この配下に混ぜない）。公開URLは移行完了まで現状のまま稼働。
> 旧シートへの変更は稼働維持に必要な修正（バグ・実績データ更新・セキュリティ）のみ。
> 本文書内の「進行中タスク」（マーケv2.1確定・実績表反映等）は旧シート前提のタスクであり、新評価シートの方針決定後に要否を再判断する。
