...
export async function ensureSchema(): Promise<void> {
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_customers_is_rep ON customers(is_rep)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_customers_last_edited ON customers(last_edited)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_customers_appt ON customers(appointment_date)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_customers_name_norm ON customers(name_norm)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_customers_phone_norm ON customers(phone_norm)`;
  ...
  await sql`CREATE TABLE IF NOT EXISTS sync_meta ( key text PRIMARY KEY, value text )`;
}
...
async function upsertCustomers(rows: ListCustomer[], stamp: string): Promise<void> {
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    const text =
      `INSERT INTO customers (id,url,name,name_norm,phone,phone_norm,status,rank,industry,phase,method,pref,is_rep,s_rep,call_count,last_call_date,appointment_date,last_edited,address,confirm,synced_at) VALUES ${tuples.join(",")} ` +
...
async function upsertContracts(rows: Contract[], stamp: string): Promise<void> {
  ...
  for (let i = 0; i < rows.length; i += BATCH) {
    ...
    const text =
      `INSERT INTO contracts (id,name,status,monthly,start_date,end_date,kinds,churn_risk,next_renewal,s_rep,cs_rep,health,customer_id,synced_at) VALUES ${tuples.join(",")} ` +
...
export async function syncAll(): Promise<{ customers: number; contracts: number; ms: number }> {
  ...
  const sql = db();
  await sql.query(`DELETE FROM customers WHERE synced_at < $1`, [stamp]);
  await sql.query(`DELETE FROM contracts WHERE synced_at < $1`, [stamp]);
  await sql.query(`INSERT INTO sync_meta (key,value) VALUES ('last_sync',$1) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value`, [stamp]);
...
export async function dbGetAllSlim(): Promise<ListCustomer[]> {
  const sql = db();
  const rows = (await sql`SELECT id,url,name,phone,status,rank,industry,phase,method,pref,is_rep,s_rep,call_count,last_call_date,appointment_date,last_edited,address,confirm FROM customers`) as any[];
  return rows.map((r) => ({
...
export async function dbGetContracts(): Promise<Contract[]> {
  const sql = db();
  const rows = (await sql`SELECT id,name,status,monthly,start_date,end_date,kinds,churn_risk,next_renewal,s_rep,cs_rep,health,customer_id FROM contracts`) as any[];
  return rows.map((r) => ({
...
export async function dbLastSync(): Promise<string | null> {
  try {
    const sql = db();
    const rows = (await sql`SELECT value FROM sync_meta WHERE key='last_sync'`) as any[];
