...
2. **SQL Editor** で以下を順に実行
   ...
   - `supabase/migrations/0008_two_tier.sql`
   - `supabase/migrations/0009_reward_plan_type.sql`
   - `supabase/migrations/0010_rate_plan_type.sql`
...
3. 環境変数 `NOTION_TOKEN` を設定（`.env.local` と Vercel の両方。サーバ側のみ・ブラウザには出さない）→ 再デプロイ
4. **既存パートナーの同期**：`/admin` →「パートナー登録」タブ下部の「Notion 同期」で、
   個別同期または「全パートナーを一括同期」を実行（重複防止のため2回目以降は同じ行を更新）

> 連携先 database_id = `007f6461b55d4013879bd4a94abbc05c`（Notion-Version `2022-06-28`）。
> もし作成時に 404/400 が出る場合は、インテグレーションの共有漏れか、API版とID種別の不一致。
> `lib/integrations/notion.ts` の `NOTION_DATABASE_ID` / `NOTION_VERSION` を確認。
