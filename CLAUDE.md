...
- 各エージェントの処理が完了するたびに、成果を `README.md` にMarkdownでまとめてから次のエージェントに進むこと
- これによりタイムオーバー時にも途中成果が保全され、中断箇所から再開できる
- 最終成果物は `README.md`（統合レポート）と各エージェントの `output.json` の両方に保存する
