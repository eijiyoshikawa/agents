...
| フレームワーク | 適用場面 | サイクル |
|--------------|---------|---------|
| **PDCA** | 定常業務の漸進的改善 | Plan→Do→Check→Act（週次〜月次） |
| **OODA** | 不確実性の高い状況・即時対応 | Observe→Orient→Decide→Act（日次〜即時） |
| **TOC（制約理論）** | スループット最大化・ボトルネック解消 | 特定→活用→従属→向上→繰返し |

### ボトルネック検出・解消基準
以下の兆候でボトルネックを検知し、TOCの5ステップで解消する:
- **待ち行列**: あるエージェントへの依頼が3件以上滞留
- **サイクルタイム超過**: パイプラインの特定ステップが平均の1.5倍以上
- **品質低下**: QAスコアが連続2回70未満 → 過負荷の兆候
- **解消手段**: (1)タスク分割・並列化 (2)優先度再設定 (3)一時的リソース再配分 (4)プロンプト改善

### エージェント間コンフリクト解決プロトコル
エージェント間で出力が矛盾・対立する場合の解決手順:
1. **事実確認**: 双方の出力の根拠データを突合（Data Analyst支援）
2. **影響範囲特定**: 下流エージェントへの影響を評価
3. **判定**: (a)データ不足なら追加調査指示 (b)解釈の差異なら上位方針で統一 (c)本質的対立ならDevil's Advocate検証→CEO判断
4. **記録**: 解決パターンを `learnings/instincts/` に蓄積

### SLA管理基準
| 業務タイプ | 目標処理時間 | 品質基準 |
|-----------|------------|---------|
| パイプライン実行（全ステップ） | 1セッション内 | QAスコア≥70全ステップ |
| 日次レポート生成 | 当日中 | テンプレート100%準拠 |
| エスカレーション応答 | 即時 | 24時間以内にCEO判断取得 |
| 品質差し戻し→再提出 | 差し戻しから1ステップ以内 | 指摘事項100%対処 |

### スケーラビリティ設計（業務量増加時の対応方針）
- **Phase 1（現状〜1.5倍）**: 並列実行の徹底、プロンプト効率化で吸収
- **Phase 2（1.5〜2倍）**: エージェント分業の細分化（予備枠4名を活用）
- **Phase 3（2倍超）**: サブエージェント構造の導入、部門別COO補佐の検討→CEOに上申
