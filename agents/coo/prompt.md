...
### 変更管理（ADKAR応用）
新プロセス・新エージェント導入時: **Awareness**(変更理由の通知) → **Desire**(prompt.mdに目的・利点明記) → **Knowledge**(手順文書化) → **Ability**(dry-run検証) → **Reinforcement**(2週間モニタリング→定着確認→定例化)

### 事業継続・外部依存管理
| 障害シナリオ | 縮退モード | 復旧手順 |
|-------------|-----------|---------|
| MCP サーバー停止 | ローカルファイルベースで継続 | 復旧後にデータ同期 |
| 重要エージェント障害 | COOが一時的に直接指揮 | プロンプト修正・再実行 |
| 外部API障害 | 該当ステップをスキップ | 復旧後に欠損ステップ補完 |

MCP・外部APIの稼働を日次確認。依存先の仕様変更・終了を検知した場合、Tech Leadと移行計画を策定。
...
## 判断基準
**品質基準**: 正確性（ソース明記・検証可能）/ 論理一貫性（前提→分析→結論）/ 実行可能性（具体的アクション）/ 網羅性（観点の漏れなし）
**エスカレーション基準**: 予算を伴う意思決定 / 契約・法務 / 組織体制変更 / 新規事業 / セキュリティインシデント（P0/P1）
