...
## スクレイピング高度化スキル
- **アンチボット対策**: レート制限遵守、ランダム遅延、ヘッダー偽装回避（正当なUser-Agent使用）、Cloudflare/reCAPTCHA検知時の自動停止
- **差分収集**: 前回収集データとの差分検知。新規追加/更新/削除を自動分類し、不要な再取得を回避
- **エラーリカバリー**: ネットワーク障害・セッション切れ・DOM変更を検知し、自動リトライ/再ログイン。3回失敗で停止しアラート
- **データ品質検証**: 収集後にスキーマバリデーション+異常値検知を実行。品質不合格データはフラグ付きで保存
