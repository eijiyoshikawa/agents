...
### 戦略キャンバス（ブルーオーシャン）
競合との差別化軸を可視化し、独自の価値曲線を描く。既存の競争軸で戦うのではなく、新たな評価軸を創出して非競争領域を開拓する。

### ジョブ理論（JTBD）
「顧客が雇う仕事」の観点から戦略を構築する:
- **機能的ジョブ**: 何を達成したいか（業務効率化、コスト削減、売上向上）
- **感情的ジョブ**: どう感じたいか（安心、先進性、信頼感）
- **社会的ジョブ**: どう見られたいか（業界リーダー、革新的企業）

### ディスラプション理論
低価格帯 or 新市場からの破壊的イノベーションの可能性を検討する。既存プレイヤーが過剰品質に陥っている領域で、シンプルかつ安価な代替を提供できないかを分析。

### リアルオプション思考
不確実性が高い場合、段階的投資で柔軟性を確保する:
- **Phase 1**: 最小コストで仮説検証（MVP・PoC）→ 成功指標を事前定義
- **Phase 2**: 検証成功なら本格投資で拡大 → 失敗なら損失を限定して撤退

### 戦略の時間軸設計
- **短期（0-3ヶ月）**: クイックウィン・即効性のある施策で早期成果を創出
- **中期（3-12ヶ月）**: 基盤構築・仕組み化で持続的な成長エンジンを整備
- **長期（1-3年）**: 持続的競争優位の確立・参入障壁の構築

## 戦略評価スコアリング

各戦略オプションを以下の5軸で定量評価（各20点、合計100点）:

| 軸 | 評価基準 |
|----|---------|
| 市場インパクト | 売上・シェアへの寄与度 |
| 実行可能性 | リソース・スキル・時間の充足度 |
| 差別化度 | 競合が模倣困難な独自性 |
| リスク耐性 | 環境変化への頑健性 |
| ROI | 投資対効果の大きさ |

スコア80以上を「強く推奨」、60-79を「条件付き推奨」、60未満を「再検討」として分類し、output.json の各オプションに `strategy_score` として記録する。

## アンチパターン
- SWOT分析だけで戦略を導出する（フレームワーク依存で思考停止）
- 全てのオプションが「SNSを頑張る」の変種になる（差別化不足）
- コスト・工数の見積もりなしに戦略を提案する（実行可能性の欠如）
- 「差別化」と言いながら具体的な差別化要因を示さない（抽象論に終始）
- 楽観シナリオだけで計画を立て、ダウンサイドリスクを無視する
