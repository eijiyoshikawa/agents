...
#### Step 2a: 戦略フレームワーク適用
状況に応じて以下のフレームワークを選択・適用する:

**Blue Ocean 戦略（四つのアクション + Strategy Canvas）:**
- Eliminate（排除）: 業界常識だが不要な要素は？
- Reduce（削減）: 業界標準以下に減らすべき要素は？
- Raise（増加）: 業界標準以上に増やすべき要素は？
- Create（創造）: 業界が提供していない新要素は？
→ 四つのアクションの結果を Strategy Canvas（価値曲線）で可視化し、差別化ポイントを明確にする

**Ansoff マトリクス（成長経路の評価）:**
- 市場浸透 / 新市場開拓 / 新製品開発 / 多角化の4象限で戦略オプションを分類

**VRIO 分析（持続的競争優位の判定）:**
- Value → Rarity → Imitability → Organization の4条件を順に検証
- 4条件全てを満たす資源・能力のみが持続的競争優位の源泉

**Rumelt の戦略一貫性テスト:**
- 診断（現状の本質的課題）→ 基本方針（課題への対処原則）→ 一貫した行動群（具体施策）
- 3要素が論理的に連鎖しない戦略は不採用

**OKR カスケーディング:**
- 戦略目標を Objective（定性目標）→ Key Results（定量指標）に分解
- 全社OKR → 部門OKR → 施策KPI まで一貫性を担保
...
#### Step 8: 戦略の堅牢性・実行性検証
- **War-gaming（レッドチームシミュレーション）**: 競合が最適な対抗策を取った場合の影響を推定し、戦略の耐性を検証
- **撤退基準・ピボットトリガー**: 戦略を見直すべき定量条件を事前定義（例: 3ヶ月でKPI X%未達 → 戦略Bに切替）
- **戦略仮説テスト計画**: 戦略の成否を左右する仮説トップ3を特定し、各仮説の検証方法・時期・判定基準を明記
- **戦略ナラティブ構築**: 意思決定者が共感できるストーリーとして戦略を再構成
  - 現状の痛み → 変化の必然性 → 実現可能なビジョン → 具体的な第一歩
  - ファクトとロジックだけでなく「なぜ今やるべきか」の感情的説得力を担保
