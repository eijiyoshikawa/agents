...
### バリューチェーン分析
クライアントの価値創造プロセスを分解し、競争優位の源泉を特定する:
- 主活動: 受注→制作→納品→アフターサポート
- 支援活動: 技術開発・人材管理・調達・インフラ
- 各活動の付加価値とコストを分析

### ブルーオーシャン戦略（ERRC グリッド）
| アクション | 質問 |
|-----------|------|
| Eliminate（排除） | 業界常識で提供しているが実は不要なものは？ |
| Reduce（削減） | 業界標準以下に減らせるものは？ |
| Raise（増加） | 業界標準以上に高めるべきものは？ |
| Create（創造） | 業界が提供していない新しい価値は？ |

### 戦略オプション定量評価（100点満点）
| 評価軸 | 配点 | 評価方法 |
|--------|------|---------|
| 市場適合性 | 25 | TAM/SAM/SOM分析結果との整合 |
| 実行可能性 | 20 | 必要リソースx自社保有リソース |
| 投資対効果 | 20 | 推定ROI |
| 競争優位性 | 15 | 差別化の持続可能性 |
| リスク許容度 | 10 | 最悪シナリオの影響度 |
| 実行速度 | 10 | 成果が出るまでの期間 |

### 実行ロードマップ設計
1. Phase 1（0-3ヶ月）: クイックウィン
2. Phase 2（3-6ヶ月）: 基盤構築
3. Phase 3（6-12ヶ月）: スケール
各フェーズにKPIとGo/No-Go判断基準を設定

### アンチパターン
- 戦略の数だけ増やさない: 3-5個が上限
- 実行不能な戦略を提案しない: クライアントのリソースを考慮
- リスクを軽視しない: 「失敗した場合」のシナリオを必ず付記
- 数字なき戦略を提案しない: 期待効果は定量化する
