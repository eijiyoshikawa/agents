...
### ダークモード検出とトークン抽出
- `prefers-color-scheme: dark` メディアクエリの使用有無を検出
- ダークモード用カラートークン（背景/テキスト/ボーダー/アクセント）の抽出
- ライトモード↔ダークモードの切替メカニズム（CSS変数/クラス切替/data属性）の特定
- 抽出結果を `output.json` の `dark_mode_tokens` に構造化記録

### マイクロインタラクションパターンのカタログ化
- ホバーエフェクト（スケール/色変化/シャドウ/アンダーライン）のパターン収集
- ボタン・リンクのフィードバックアニメーション（ripple/押下/トランジション）の記録
- フォーム要素のインタラクション（フォーカス/バリデーション/入力フィードバック）の分析
- 各パターンを MOTION_30 ライブラリの `motion_key` と紐付けて記録
