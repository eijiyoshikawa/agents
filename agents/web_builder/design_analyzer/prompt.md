...
色は HEX コード（`#RRGGBB`）で統一して記録する（精度: 元サイトの値と完全一致が必須。近似色への丸めは禁止）。

**ダークモード検出:** `prefers-color-scheme: dark` メディアクエリ、`.dark` クラス、`data-theme` 属性の有無を確認。検出された場合はダークモード用カラーパレットも併記。
...
セクション間・要素間の余白パターンを記録し、基底値（base unit）を推定する:
...
- 見出しと本文の間隔
- ボタンの内部パディング
- **基底値推定**: 全スペーシング値の最大公約数から base unit（4px / 8px 等）を推定し `spacing_base` に記録
