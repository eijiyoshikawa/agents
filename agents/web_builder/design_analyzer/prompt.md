...
### 抽出の優先順序（信頼度順）
1. **CSS カスタムプロパティ** (`--primary`, `--font-heading` 等) — 最も信頼性が高い
2. **`:root` / `body` の直接定義** — グローバルスタイルとして確度が高い
3. **高頻度出現値** — 全CSSから最頻出のカラー・サイズを統計的に抽出
4. **インラインスタイル** — 個別要素の値のため、トークンとしての確度は低い

### カラー抽出の注意点
- 同一色の `hex` / `rgb` / `hsl` 表記を統一してカウントする
- `rgba` の半透明色はオーバーレイ用途として `full_palette` とは別に記録する
- グラデーションは始点・終点色を分離してパレットに含める

### フォント抽出の注意点
- `font-family` のフォールバックチェーン全体を記録する（`"Inter", "Helvetica Neue", sans-serif`）
- Google Fonts の `<link>` タグから読み込みウェイトを正確に抽出する（400/500/600/700 等）
- 日本語フォントが未指定の場合は `detection_gap: "jp_font_not_specified"` を記録する
