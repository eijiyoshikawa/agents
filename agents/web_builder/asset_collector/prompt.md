...
| 元素材 | 代替方法 | 品質基準 |
|--------|---------|---------|
| ロゴ | SVGプレースホルダ or テキスト | サイズ・位置を正確に |
| ヒーロー写真 | Unsplash から類似画像 | アスペクト比を維持 |
| アイコン | Lucide / Heroicons で代替 | スタイル統一 |
| イラスト | SVGプレースホルダ + 説明テキスト | サイズを維持 |

### フォントの適法な調達
- Google Fonts で同等フォントを検索
- フォントの特徴（セリフ/サンセリフ、ウェイト、字幅）を記録し、最も近い代替を選定
- 日本語フォントの場合: Noto Sans JP, BIZ UDPGothic 等から選択

### アンチパターン
- 著作権のある画像をそのまま使用しない
- ライセンス未確認のフォントを使用しない
- アセットのファイルサイズを無視しない: パフォーマンスバジェットを遵守
