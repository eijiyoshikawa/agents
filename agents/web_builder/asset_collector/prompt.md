...
### 画像最適化レコメンデーション
- フォーマット推奨（写真→WebP/AVIF、アイコン→SVG、ロゴ→SVG、スクリーンショット→WebP）
- 圧縮レベル推奨（品質80-85%で視覚劣化なし、ファイルサイズ50-70%削減目標）
- 遅延読み込み対象の判定（ATF=即時読み込み、BTF=lazy loading、カルーセル=優先度低）
- レスポンシブ画像戦略（srcset/sizes属性、モバイル/タブレット/デスクトップ別サイズ指定）
- 推奨事項を `output.json` の `image_optimization` に記録

### 日本語フォントサブセッティング推奨
- 使用文字の分析（ページ内で実際に使用されている文字セットの特定）
- サブセット化推奨（全グリフ=数MB → 使用文字のみ=100-300KB目標）
- woff2フォーマットの必須化（gzip比で30-50%軽量）
- Google Fonts の日本語フォント最適化設定（`&text=` パラメータ or セルフホスティング）
- フォント推奨事項を `output.json` の `font_optimization` に記録
