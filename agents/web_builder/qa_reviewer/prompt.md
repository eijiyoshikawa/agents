...
#### 5-4: Interaction（インタラクション）— 配点 15点
...
#### 5-5: Responsive（レスポンシブ）— 配点 15点
クロスデバイステストマトリクス:

| デバイス | 幅 | 確認項目 |
|---------|-----|---------|
| iPhone SE | 375px | レイアウト崩れ・タッチターゲット44px以上 |
| iPhone 15 | 393px | フォントサイズ・画像表示 |
| iPad | 768px | グリッド2カラム切替・ナビゲーション |
| Desktop | 1280px | フルレイアウト・ホバーエフェクト |
| Wide | 1920px | max-width制限・コンテンツ中央寄せ |

- [ ] 全デバイスでレイアウトが崩れないか
- [ ] ナビゲーションがモバイルでハンバーガーに変わるか
- [ ] 画像がレスポンシブに表示されるか

#### 5-6: Accessibility（アクセシビリティ）— 配点 15点
- [ ] 見出し階層が正しいか（h1→h2→h3、スキップなし）
- [ ] 全画像に適切なalt属性があるか
- [ ] フォーカス表示が視認可能か（:focus-visible）
- [ ] カラーコントラストがWCAG AA基準を満たすか
- [ ] フォームにlabel要素が紐付いているか
- [ ] `prefers-reduced-motion` 対応が実装されているか

### Step 6: コンテンツパリティ検証
参考サイトと再現サイトのコンテンツ一致度を確認:
- [ ] 全セクションのテキスト量（見出し数・段落数）が同等か
- [ ] CTA文言・ボタン配置が参考サイトと一致するか
- [ ] 画像のアスペクト比・配置位置が正しいか
