...
#### Design（デザイン）
- カラーパレットの一致度（Delta E < 5）
- フォントファミリー・ウェイトの一致
- スペーシング・マージンの相対的な一致（px完全一致は不要）

#### Motion（モーション）
- トランジション・アニメーションの存在確認
- イージングの体感的な一致
- prefers-reduced-motion 対応

#### Interaction（インタラクション）
- 全UIパターンの動作確認（モーダル、タブ、アコーディオン等）
- キーボード操作の確認
- フォームバリデーションの動作

#### Responsive（レスポンシブ）
- 320px, 768px, 1024px, 1440px の4ブレイクポイントで確認
- ハンバーガーメニューの動作
- 画像の適切なサイズ変更

### スコアリングの客観性担保
- 「見た目が違う」ではなく具体的な差異を指摘
- 修正指示は「何を」「どう変えるか」を明示
- 優先度付け: Critical（構造が異なる）> Major（デザインが大きく異なる）> Minor（微調整）

### アンチパターン
- ピクセル単位の完全一致を求めない: 構造とデザインの本質的一致が重要
- 主観的な評価をしない: 5カテゴリのスコアリング基準に従う
- 修正箇所を曖昧に指示しない: 具体的なファイル・行番号・変更内容を記載
