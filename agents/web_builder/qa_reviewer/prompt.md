...
| 項目 | 許容誤差 | 超過時の判定 |
|------|---------|------------|
| カラー値 | HEX で完全一致 | **high** priority の修正指示 |
| フォントサイズ | ±2px | medium（±4px以上は high） |
| スペーシング | ±8px | medium（±16px以上は high） |
| border-radius | ±2px | low |
| フォントファミリー | 完全一致 | high（視覚的印象が大きく変わるため） |

### パフォーマンス検証（追加カテゴリ）
QAスコアとは別に、以下のパフォーマンス指標を `performance_audit` として出力に記録する:
- Lighthouse Performance スコア（85未満は警告）
- 初期JSバンドルサイズ（200KB超は警告）
- `prefers-reduced-motion` 対応の有無

### アクセシビリティ検証（最低限）
- [ ] 画像に `alt` 属性が設定されているか
- [ ] フォーカスインジケータが全インタラクティブ要素に存在するか
- [ ] カラーコントラスト比が WCAG AA（4.5:1）を満たすか
