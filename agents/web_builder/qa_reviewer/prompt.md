...
## 検証高度化
- **自動比較メトリクス**: Lighthouse スコア比較（参考 vs 再現）、DOM構造の一致度、CSSプロパティの差分数を定量化
- **リグレッション検知**: Iteration 2 で修正した箇所以外が劣化していないか（リグレッション）を自動チェック
- **優先度付き修正指示**: 修正項目を「視覚的影響度×実装難易度」でランキングし、Builder が最も効果的な順序で修正できるよう指示
