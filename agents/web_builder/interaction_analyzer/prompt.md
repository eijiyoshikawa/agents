...
### Step 7: キーボードナビゲーション・フォーカス管理監査
WAI-ARIA Authoring Practices に基づき、各インタラクティブ要素のキーボード操作を記録する:

**要素タイプ別の必須キーボード操作:**
| 要素 | キー | 期待される動作 |
|------|------|--------------|
| ボタン | Enter / Space | クリックと同等のアクション |
| リンク | Enter | ナビゲーション |
| モーダル | Escape | 閉じる |
| モーダル | Tab | モーダル内でフォーカストラップ |
| タブ | Arrow Left/Right | タブ切り替え |
| アコーディオン | Enter / Space | 開閉トグル |
| ドロップダウン | Arrow Up/Down | 項目移動 |
| スライダー | Arrow Left/Right | スライド移動 |

**フォーカス管理の検証:**
- フォーカスインジケーター（`:focus-visible`）の可視性 — 色・太さ・コントラスト比
- モーダルオープン時のフォーカストラップ有無
- モーダルクローズ時のフォーカス復帰位置
- スキップリンク（`#main-content` へのジャンプ）の有無
- `tabindex` の使用パターン（`0`: フォーカス可能化、`-1`: プログラム的フォーカス、正の値: 原則禁止）

### Step 8: タッチターゲットサイズ検証
モバイルでのタッチ操作性を検証する:

**最小サイズ基準:**
- **WCAG 2.2 Level AA**: 最小 24x24 CSS px（Target Size Minimum）
- **推奨サイズ**: 44x44 CSS px（Apple HIG / Material Design 準拠）
- **理想サイズ**: 48x48 CSS px（Google Material Design 3）

**検証対象:**
- ナビゲーションリンク（テキストリンクの実際のタッチ領域）
- ハンバーガーメニューアイコン
- フォーム入力フィールド
- チェックボックス / ラジオボタン
- 閉じるボタン（モーダル・バナー）
...
**ターゲット間の間隔**: 隣接するタッチターゲット間の最小間隔 8px を確認
...
```json
{
  "touch_targets": {
    ...
    "passed_24px": 14,
    "failures": [
      {"element": "フッターSNSアイコン", "actual_size": "20x20", "fix": "padding を追加して 44x44 に拡大"}
    ]
  }
}
```

### Step 9: インタラクションパターン分類
検出した全インタラクションを以下のカテゴリに分類する:

| カテゴリ | パターン例 | 実装複雑度 |
|---------|----------|----------|
| **直接操作** | ドラッグ&ドロップ、ピンチズーム、スワイプ | 高 |
| **フォーム入力** | テキスト入力、選択、日付ピッカー、ファイルアップロード | 中 |
| **ナビゲーション** | メニュー展開、タブ切替、ブレッドクラム、ページネーション | 中 |
| **情報開示** | アコーディオン、ツールチップ、モーダル、ライトボックス | 低〜中 |
| **フィードバック** | トースト通知、プログレスバー、スケルトンスクリーン | 低 |
| **メディア制御** | 動画再生/停止、カルーセル操作、音量調整 | 中 |
...
| 状況 | 対処 |
|------|------|
| **JavaScript が無効化された場合のフォールバック** | `<noscript>` タグの有無を確認。プログレッシブエンハンスメント対応の度合いを記録 |
| **フォーム送信が SPA 内ルーティング** | `e.preventDefault()` + fetch/axios パターンの検出。`submission_method: "ajax"` として記録 |
| **reCAPTCHA / hCaptcha の検出** | CAPTCHA タイプとバージョンを記録。再現時の代替（テスト環境用のダミー）を提案 |
| **サードパーティウィジェット（チャットボット等）** | 外部サービス依存の要素は `third_party_widgets` として分離記録。再現対象外とマーク |
| **Web Components（Shadow DOM）** | Shadow DOM 内の要素は通常の DOM 解析では検出不能。`shadow_dom_detected: true` を記録 |
| **無限スクロール / 仮想スクロール** | `intersection_observer` + 動的 DOM 追加パターンの検出。`pagination_type: "infinite"` で記録 |
