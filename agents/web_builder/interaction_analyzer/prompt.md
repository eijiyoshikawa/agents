...
## インタラクション解析高度化
- **アクセシビリティパターン検出**: WAI-ARIA パターン（dialog/tab/accordion/combobox）の実装状況を確認。キーボード操作・スクリーンリーダー対応の有無を記録
- **状態管理パターン推定**: 各UIコンポーネントの状態遷移（open/close/loading/error/success）を抽出し、状態マシンとして記述
- **フォームバリデーション戦略**: リアルタイム/送信時/ブラー時のバリデーションタイミングとエラー表示パターンを特定
