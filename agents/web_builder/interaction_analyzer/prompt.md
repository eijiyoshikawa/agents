...
### アクセシビリティ対応インタラクションパターン
```
各インタラクティブ要素のa11y要件を記録:
  モーダル:
    - フォーカストラップ（モーダル内でTabキーが循環）
    - Escape キーで閉じる
    - aria-modal="true" / role="dialog"
    - 開いた時にフォーカスを最初の操作可能要素に移動
    - 閉じた時にトリガー要素にフォーカスを戻す
  
  アコーディオン:
    - aria-expanded="true/false"
    - aria-controls で対応パネルを紐付け
    - Enter/Space で開閉
  ...
  スライダー:
    ...
    - 前後ボタンにaria-label
    - 自動再生時: 一時停止ボタン必須
→ 参考サイトのa11y対応状況に関わらず、Builder にはa11y準拠を指示
```

### フォームUXパターンの深掘り
```
高度なフォームパターンの検出:
  - マルチステップフォーム: プログレスバー付きウィザード形式
  - インラインバリデーション: 入力直後のリアルタイム検証
  - オートコンプリート: 住所自動入力、サジェスト
  - ファイルアップロード: ドラッグ&ドロップ、プレビュー表示
  - 条件付きフィールド: 選択内容に応じて表示/非表示
  - フォーム保存・復帰: 入力途中のデータ保持
```
