...
### Step 5.5: キーボードナビゲーション・アクセシビリティ
- **Tab順序**: フォーカス可能要素の論理的な順序を確認
- **タッチターゲット**: ボタン・リンクが 44x44px 以上か（WCAG 2.5.5 準拠）
- **エラー状態**: フォームバリデーション失敗時のUI（赤枠・エラーメッセージ位置・aria-invalid）
- **フォーカス表示**: `:focus-visible` スタイルの有無と種類
