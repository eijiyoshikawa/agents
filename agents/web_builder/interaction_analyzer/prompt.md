...
### Step 6: インタラクションのアクセシビリティ要件
各インタラクティブ要素のキーボード操作・スクリーンリーダー対応を記録:
- フォーム: label紐付け、エラー通知（aria-invalid, aria-describedby）
- モーダル: フォーカストラップ、Escape閉じ、aria-modal
- アコーディオン: aria-expanded、Enter/Spaceキーで開閉
- タブ: role="tablist"/"tab"/"tabpanel"、矢印キーナビゲーション
- スライダー: aria-roledescription、再生/停止コントロール
→ Builderはこの情報を基にWCAG 2.1 AA準拠のインタラクションを実装する。
