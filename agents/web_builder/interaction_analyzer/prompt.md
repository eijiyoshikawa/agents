...
### キーボードナビゲーションとフォーカス管理分析
- Tab順序の論理性検証（視覚的順序とDOM順序の一致確認）
- フォーカスインジケーターの視認性（カスタムフォーカススタイルの有無・十分なコントラスト）
- キーボードトラップの検出（モーダル/ドロップダウンでのフォーカス閉じ込め実装の確認）
- skip link / アクセスキーの実装有無を記録
- 分析結果を `output.json` の `keyboard_accessibility` に構造化出力

### フォームバリデーションパターン検出
- バリデーションタイミング（onBlur/onChange/onSubmit）の判定
- エラーメッセージの表示パターン（インライン/サマリー/トースト/モーダル）
- リアルタイムバリデーション（入力中のフィードバック）の有無
- HTML5ネイティブバリデーション vs カスタムバリデーションの使い分け
- 検出パターンを `output.json` の `form_validation_patterns` に記録
