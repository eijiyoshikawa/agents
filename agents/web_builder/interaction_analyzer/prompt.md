...
3. **フィールド一覧**:
   - フィールド名（name属性）/ 入力タイプ / 必須かどうか / プレースホルダー
   - バリデーションルール（パターン: メール形式・電話番号・文字数制限等）
   - エラー表示方式（インライン / ツールチップ / サマリー）
4. **送信先**: action属性 / JavaScriptでの送信処理
5. **レイアウト**: 1カラム / 2カラム / インライン
6. **送信ボタン**: テキスト、スタイル
7. **確認画面/完了メッセージ**: 有無とその内容
8. **エラー状態カタログ**: 必須未入力・形式不正・サーバーエラー時のUI変化
...
### Step 5: キーボードナビゲーション・タッチターゲット
- **キーボード操作**: Tab順序の論理性、フォーカス表示、Escape でモーダル閉じ
- **タッチターゲット**: モバイルでのボタン・リンクの最小サイズ（44x44px推奨）
- **マイクロインタラクション**: ボタン押下フィードバック、フォーム送信中スピナー等
