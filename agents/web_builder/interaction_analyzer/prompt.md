...
### Step 6: インタラクションのアクセシビリティ評価
各UI要素のアクセシビリティ対応状況を記録:
- フォーム: labelとinputの関連付け、エラーメッセージの aria-live
- モーダル: フォーカストラップ、aria-modal、Escapeキー閉じ
- タブ: role="tablist"/role="tab"/role="tabpanel"、矢印キー操作
- アコーディオン: aria-expanded、aria-controls
- スライダー: aria-roledescription、キーボード操作対応
これらは Builder が実装時にアクセシビリティを担保するための必須情報。
