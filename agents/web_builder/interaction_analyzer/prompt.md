...
| パターン | 挙動の記録項目 | 再現の注意点 |
|---------|-------------|------------|
| モーダル | トリガー、背景スクロールロック、クローズ方法 | focus trap 必須 |
| ドロワー | 方向、オーバーレイ有無、スワイプ対応 | touch イベント対応 |
| タブ | 初期タブ、URL連動、キーボード操作 | ARIA roles |
| アコーディオン | 複数開放可否、アニメーション | details/summary or ARIA |
| フォーム | バリデーション方式、エラー表示位置 | アクセシビリティ対応 |
| ナビゲーション | メガメニュー、スティッキー、スクロール挙動 | レスポンシブ切替 |

### アクセシビリティ要件
- キーボードナビゲーション（Tab/Shift+Tab/Enter/Escape）の対応状況
- スクリーンリーダー対応（aria-expanded, aria-controls等）
- フォーカス管理（モーダル開閉時のフォーカストラップ）

### アンチパターン
- 見た目だけ再現しない: キーボード操作とアクセシビリティも再現
- 独自実装しない: 可能な限り Headless UI / Radix UI 等の実績あるライブラリを推奨
