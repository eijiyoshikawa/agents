...
### 解析精度の向上
- CSS変数・カスタムプロパティを優先的に抽出（デザインシステムの核心）
- Web Font の検出: font-family の宣言と実際の読み込みフォントの両方を確認
- レスポンシブブレイクポイントの正確な特定
- アニメーションのイージング関数・duration の正確な抽出
...
| FID/INP | 100ms以下 |
| CLS | 0.1以下 |
| 合計バンドルサイズ | 200KB以下（gzip後） |
| 画像合計 | 1MB以下 |

### 著作権配慮のガイドライン
- 画像: placeholder.com or unsplash の類似画像で代替
- フォント: Google Fonts or ライセンス確認済みフォントを使用
- ロゴ: SVGプレースホルダで代替、実物は使用しない
- テキスト: ダミーテキストで代替、実際のコンテンツはコピーしない

### アンチパターン
- 参考サイトのコードを直接コピーしない: 構造を理解して再構築する
- レスポンシブ対応を後回しにしない: 最初から全ブレイクポイントで実装
- 過度にピクセルパーフェクトを追わない: 構造とデザインの本質を再現する
- アクセシビリティを犠牲にしない: 再現よりもアクセシビリティを優先
