...
- Site Scanner の深掘り:
  - フレームワーク検出: Next.js / Nuxt / Gatsby / WordPress のフットプリント特定
  - CDN検出: Vercel Edge / Cloudflare / AWS CloudFront のレスポンスヘッダー
  - フォント検出: Google Fonts / Adobe Fonts / カスタムフォントの特定
  - アナリティクス検出: GA4 / GTM / Hotjar の埋め込みスクリプト特定

- Design Analyzer の精度向上:
  - カラー抽出: 実際に使われている色を頻度順にソート、ブランドカラーを特定
  - タイポグラフィ: font-family / font-weight / line-height / letter-spacing を全見出しレベルで記録
  - スペーシング: セクション間・要素間の余白パターンを数値化
  - グリッドシステム: 最大幅 / カラム数 / ガター幅を特定

- 再現度スコアの評価基準詳細:

  | カテゴリ | 配点 | 主要チェック項目 |
  |---------|------|---------------|
  | Structure | 20 | ヘッダー/フッター/セクション構成/ナビ |
  | Design | 25 | カラー/タイポ/余白/角丸/シャドウ |
  | Motion | 20 | スクロール/ホバー/ページ遷移 |
  | Interaction | 20 | フォーム/モーダル/タブ/アコーディオン |
  | Responsive | 15 | モバイル/タブレット/デスクトップ |

## 著作権・法的配慮

- 画像: 参考サイトの画像を直接使用しない → Unsplash/Pexels の代替画像 or プレースホルダー
- フォント: ライセンスを確認（Google Fonts は OK、Adobe Fonts は契約必要）
- ロゴ: 参考サイトのロゴは使用禁止 → クライアントのロゴ or プレースホルダー
- コード: HTML/CSS の構造参考は OK、JavaScript のコピペは避ける

## アンチパターン

- 参考サイトの画像・ロゴをそのまま使用（著作権侵害）
- レスポンシブ対応を後回しにする
- 1回のイテレーションで完成を期待（2周が標準）
- アニメーションの完全再現に固執してパフォーマンスを犠牲にする
- QAスコア85未満でも「だいたい同じ」で妥協する
