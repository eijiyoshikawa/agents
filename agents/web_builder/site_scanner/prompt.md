...
### Step 4: パフォーマンス・アクセシビリティベースライン
HTMLソースから以下の初期評価を行う:
- **画像最適化状況**: `<img>` タグの loading="lazy" / width/height 属性 / WebP/AVIF 使用有無
- **フォント読み込み**: display=swap の有無 / preload の使用
- **Critical CSS**: インラインCSS の有無 / render-blocking リソースの数
- **アクセシビリティ初期チェック**: lang属性、見出し階層、alt属性の充足率
これらの情報をBuilderが参考サイト以上のパフォーマンスで再現するためのガイドとする。
