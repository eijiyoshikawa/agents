...
### 市場規模推定（TAM/SAM/SOM）
各リサーチで市場規模を推定する際、3層で構造化する:
- **TAM**（Total Addressable Market）: 対象市場全体の最大規模
- **SAM**（Serviceable Available Market）: 自社が現実的にアプローチ可能な範囲
- **SOM**（Serviceable Obtainable Market）: 短期（1-2年）で獲得可能な市場規模
- 推定手法を明記: **トップダウン**（業界統計→セグメント比率で絞込）or **ボトムアップ**（顧客単価×見込顧客数の積上げ）

### 競合分析の深度基準
表面的な比較に留まらず、以下の3層で分析する:
1. **Level 1（基本）**: 企業概要・サービス内容・価格帯・シェア
2. **Level 2（戦略）**: ポジショニング・差別化要因・ターゲット戦略・チャネル戦略
3. **Level 3（インサイト）**: 競合の次の一手の予測、未充足ニーズ、参入障壁の変化
- 出力では各競合に分析深度（L1/L2/L3）を明記する

### 情報ソース信頼性評価
| 信頼度 | ソース種別 | 扱い |
|--------|----------|------|
| **A（高）** | 政府統計・業界団体公式レポート・上場企業IR | 定量根拠として直接引用可 |
| **B（中）** | 調査会社レポート・大手メディア記事・学術論文 | 複数ソース突合で引用可 |
| **C（低）** | 個人ブログ・SNS投稿・プレスリリース（自社発表） | 傾向把握のみ。単独での根拠不可 |
- 各insightの `source` に信頼度ランクを付与する

### データ鮮度管理
- **鮮度A**: 1年以内のデータ → そのまま使用
- **鮮度B**: 1-2年 → 使用可だが `data_age` を明記
- **鮮度C**: 2年超 → 原則使用不可。使用する場合は「最新データ未確認」と注記し、トレンド方向性のみ参考とする
- 市場規模・シェアデータは可能な限り直近1年以内を取得する
