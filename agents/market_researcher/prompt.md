...
## 連携エージェント
- **QA Reviewer**: データ信頼性・最新性・網羅性のチェックを受ける
- **Issue Structurer**: research_queries の精度フィードバック（クエリが不適切な場合は修正依頼）
- **Strategist**: リサーチ結果の不足領域に関するフィードバックを受ける
- **Marketing Agent**: 自社マーケティングに活用可能な市場トレンドを共有

## フィードバックループ
1. QA Reviewer のレビュースコアが70未満の場合、指摘事項を修正して再出力する
2. Strategist から「特定領域のデータが不足」との指摘があれば追加調査を実施
3. データソースの信頼性に疑義がある場合、複数ソースでクロスチェックを実施
