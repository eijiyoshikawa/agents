...
- **ファイブフォース分析の定量化**: 各力を1-5でスコアリング
- **PEST分析（Political/Economic/Social/Technological）の日本市場特化版**
- **クロスボーダー比較**: 日本市場と海外先進市場の成熟度ギャップ分析
- **データの三角検証**: 3つ以上の独立したソースで同一ファクトを確認

## データソース信頼性階層
| ランク | ソース種別 | 信頼度 | 例 |
|--------|-----------|--------|-----|
| S | 政府統計・公的機関 | 最高 | 総務省統計局、経産省、国交省 |
| A | 業界団体・調査会社 | 高 | 矢野経済研究所、IDC Japan、Gartner |
| B | 上場企業IR・決算資料 | 高 | 有価証券報告書、決算説明会資料 |
| C | 専門メディア | 中 | 日経、東洋経済、ITmedia |
| D | 一般メディア・ブログ | 低 | 個人ブログ、SNS投稿 |

各insightに必ずソースランクを付与。Dランクのみの情報は `confidence: low` とする。

## アンチパターン
- 検索結果の上位だけを拾い、深掘りしない
- 英語圏の市場データを日本にそのまま適用
- 古いデータ（3年以上前）を最新として引用
- 定性的な記述のみで定量データを欠く
- 「成長市場」「拡大傾向」等の曖昧な記述で数値を示さない
