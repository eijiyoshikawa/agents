...
| フレームワーク | 構造 | 最適用途 |
|-------------|------|---------|
| AIDA | Attention→Interest→Desire→Action | LP・広告コピー・メルマガ |
| PAS | Problem→Agitate→Solution | ブログ記事・事例紹介・SNS投稿 |
| StoryBrand | 主人公(顧客)→課題→ガイド(自社)→計画→行動→成功 | ブランドストーリー・提案資料 |

制作開始時に必ずフレームワークを選定し、output.json の `framework_used` に記録。

### プラットフォーム別コンテンツ仕様
```
Instagram:
  フィード画像: 1080×1080px / カルーセル最大10枚
  リール: 9:16 / 15-90秒（推奨30秒）/ テロップ必須（ミュート視聴対応）
  ストーリーズ: 1080×1920px / 15秒以内 / CTA スタンプ活用

TikTok:
  動画: 9:16 / 15-60秒（推奨15-30秒）/ 最初の1秒にフック
  テロップ: 画面上部1/3に配置（下部はUIで隠れる）

YouTube:
  サムネイル: 1280×720px / 顔+テキスト3語以内 / コントラスト強調
  動画: 16:9 / 8-15分（通常）/ 60秒以内（ショート）
  概要欄: 最初の2行にKW+CTA / チャプター設定必須
```

### SEOライティング技術
```
キーワード配置ルール:
  1. タイトル（H1）に主要KWを含める（先頭寄り）
  2. 最初の100文字以内に主要KWを自然に含める
  3. H2/H3 見出しにサブKWを分散配置
  4. メタディスクリプション 120文字以内（KW含有）
  5. alt属性に画像内容+KW を記述

E-E-A-T強化:
  Experience: 実体験・事例データを根拠として挿入
  Expertise: 専門用語を適切に使用し解説を添える
  Authoritativeness: 公的データ・一次情報を引用
  Trustworthiness: 更新日明記・出典リンク・免責事項
```

### コンテンツA/Bテスト設計
| テスト対象 | 変数例 | 最低サンプル | 判定基準 |
|-----------|-------|------------|---------|
| メール件名 | 疑問形 vs 数字入り | 開封 500件 | 開封率差 2%+ |
| SNSコピー | 短文 vs 長文 | リーチ 1万 | ER差 0.5%+ |
| CTA文言 | 「無料相談」vs「まず相談」 | クリック 200件 | CTR差 1%+ |
| サムネイル | 顔あり vs テキストのみ | 表示 5000件 | CTR差 1%+ |
