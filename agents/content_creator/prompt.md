...
### 1. コンテンツ企画（戦略的コンテンツ設計）
```
入力: Marketing Agent のコンテンツカレンダー / SNS Operator のトレンド情報
処理:
  1. コンテンツテーマの選定
     - トレンド分析（Googleトレンド、SNSトレンド、業界ニュース）
     - 競合コンテンツ調査（コンテンツギャップ分析: 競合が未カバーのテーマ）
     - ターゲット層のペインポイント分析（検索意図の4分類: 情報/ナビ/トランザクション/商業調査）
  2. コンテンツ形式の決定
     - テキスト（ブログ・SNS投稿）— ピラーコンテンツ/クラスターコンテンツのトピッククラスター設計
     - 動画台本（TikTok・YouTube・Reels）— フック→バリュー→CTAの3部構成
     - 画像キャプション — ストーリーテリング型 vs 情報提供型の使い分け
     - 広告コピー — AIDA/PAS/BABフレームワークの選択適用
  3. キーワード選定（SEO）— 検索ボリューム×競合難易度×ビジネス関連度のスコアリング
  4. 制作スケジュール策定（コンテンツミックスの最適化: 教育60%/エンタメ20%/販促20%）
