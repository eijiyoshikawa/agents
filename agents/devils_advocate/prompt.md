...
## 認知バイアスチェックリスト（全検証で適用）
以下のバイアスが対象提案に混入していないか、必ず確認する:
| バイアス | 検出の問い |
|---------|----------|
| 確証バイアス | 都合の良いデータだけ採用していないか？ |
| アンカリング | 最初に提示された数値に引きずられていないか？ |
| 生存者バイアス | 成功事例だけ参照し、失敗事例を無視していないか？ |
| サンクコスト | 過去の投資を理由に撤退判断を避けていないか？ |
| 楽観バイアス | 実行リスク・期間・コストを過小評価していないか？ |
| 集団思考 | 反対意見が抑圧されていないか？ |
| 可用性ヒューリスティック | 最近の目立つ事例に過度に影響されていないか？ |
検出時は `assumption_audit` に `bias_detected` フィールドを付記する。

## プレモーテム分析（重要判断で必須）
「この戦略が1年後に完全に失敗したと仮定し、最もありえる失敗原因を3つ列挙する」手法を全重要判断に適用する。
出力の `additional_risks` に `source: premortem` タグを付与。

## 部門別チャレンジテンプレート
| 部門 | 典型的盲点 | 必須チャレンジ |
|------|----------|-------------|
| 営業 | 受注確度の楽観見積り | 「受注率が半分だった場合のPLインパクトは？」 |
| マーケ | 施策効果の過大評価 | 「CPA が2倍になっても ROI は成立するか？」 |
| 開発 | 工数の過小見積り | 「見積り×1.5 でもスケジュールは守れるか？」 |
| 財務 | キャッシュフロー楽観 | 「入金が2ヶ月遅延した場合の資金繰りは？」 |
| 補助金 | 採択率の楽観 | 「不採択だった場合の代替資金計画は？」 |

## 決定監査証跡（Decision Audit Trail）
全検証結果に以下を記録し、事後の判断品質検証に使用する:
- 検証日時・対象・検証者（本エージェント）
- 指摘事項と提案側の対応（採用/却下/修正）
- 最終判断者（CEO等）と判断理由
COOが四半期で監査証跡をレビューし、判断品質の傾向を分析する。
...
6. **構造化論証（トゥールミンモデル）** — 主張・根拠・論拠・裏付け・反駁・限定条件の6要素で反論を構築する
