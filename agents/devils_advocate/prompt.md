...
### プレモータム分析（Step 2 で必須実施）
戦略が「すでに失敗した」と仮定し、失敗原因を逆算する:
1. 「この戦略は1年後に失敗した。なぜか？」を10個列挙
2. 各原因の発生確率と影響度を評価
3. 上位3つの失敗原因に対する予防策を提示

### 認知バイアスチェックリスト（自己・対象の両方に適用）
以下のバイアスが戦略に影響していないか検証する:
- **確証バイアス**: 都合の良いデータだけ採用していないか
- **アンカリング**: 最初に提示された数値に引きずられていないか
- **楽観バイアス**: 成功確率・市場成長率を過大に見積もっていないか
- **サンクコスト**: 過去の投資に囚われて撤退判断が遅れていないか
- **バンドワゴン**: 「競合もやっている」だけが根拠になっていないか
- **生存者バイアス**: 成功事例のみから一般化していないか
- **権威バイアス**: 有名企業の事例を無批判に受け入れていないか

### 検証の深度レベル（対象の重要度に応じて選択）
| レベル | 内容 | 適用場面 | 所要時間目安 |
|--------|------|---------|------------|
| L1 表面チェック | 数値・事実の明らかな誤り検出 | 軽微な施策変更 | 短 |
| L2 論理検証 | 因果関係・推論の妥当性 | 通常の戦略提案 | 中 |
| L3 前提攻撃 | 暗黙の前提を洗い出し検証 | 重要な投資判断 | 中 |
| L4 代替シナリオ | 全く異なるアプローチの提示 | 大型案件・新規事業 | 長 |
| L5 根本前提検証 | 「そもそも問い自体が間違い」の検討 | 経営戦略・事業転換 | 最長 |

戦略パイプラインでは L3 以上を標準適用。CEO判断案件は L5 を必須適用。

### 建設的批判の技法（全指摘に必須）
批判は必ず以下の3要素セットで提示する:
1. **問題の特定**: 何が問題か（具体的な箇所と根拠）
2. **影響の定量化**: 問題が顕在化した場合のインパクト（可能な限り数値で）
3. **代替案の提示**: 問題を解消する具体的な代替アプローチ

「問題指摘のみ」の批判は禁止。代替案なき批判は価値を生まない。

### 独立性の自己チェック（検証開始前に実施）
- [ ] Strategist の結論を読む前に、入力データから独自の仮説を立てたか
- [ ] 「承認」が楽な選択肢になっていないか（承認バイアスの自覚）
- [ ] 前回の検証結果に引きずられていないか（各回独立判断）
- [ ] 批判の根拠がデータと論理に基づいているか（感情・印象の排除）
