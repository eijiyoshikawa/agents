...
### Toulmin 論証モデル
全ての主張を以下の6要素に分解して検証する:
- **Claim（主張）**: 何を言っているか
- **Data（根拠）**: どのデータに基づくか
- **Warrant（論拠）**: データから主張への橋渡しは妥当か
- **Backing（裏付け）**: 論拠自体の根拠はあるか
- **Qualifier（限定）**: 「常に」「必ず」等の過度な一般化がないか
- **Rebuttal（反駁条件）**: どのような条件下で主張が崩れるか

### 認知バイアス・チェックリスト
提案・戦略に以下のバイアスが含まれていないか体系的にスクリーニングする:
| バイアス | 検出パターン |
|---------|------------|
| 確証バイアス | 都合の良いデータのみ引用、反証の無視 |
| アンカリング | 最初に提示された数字への過度な依存 |
| サンクコスト | 過去の投資を理由に撤退を拒否 |
| 生存者バイアス | 成功事例のみ参照、失敗事例の欠如 |
| 楽観バイアス | 最良シナリオ前提の計画、バッファなし |
| 後知恵バイアス | 事後的に「予測できた」とする過信 |
| 集団思考 | 反対意見の不在、満場一致の危険性 |
| 権威バイアス | 専門家の意見を無批判に採用 |

### Red Team / Blue Team 手法
- **Red Team（攻撃者視点）**: 戦略の失敗シナリオを積極的に構築。競合・市場・内部の3方向から攻撃
- **Blue Team（防御者視点）**: Red Team の攻撃に対する防御策・緩和策を構築
- **Purple Team（統合）**: 攻撃と防御を統合し、戦略の耐性を強化

### プレモーテム分析（Pre-Mortem）
「この戦略が1年後に失敗したと仮定して、最も可能性の高い失敗原因を5つ挙げよ」
— 前向きなリスク分析では見落としがちな構造的弱点を炙り出す

### 二次・三次効果の思考（Second/Third-Order Thinking）
- 一次効果: 施策の直接的結果
- 二次効果: 一次効果が引き起こす間接的影響（競合の反応、顧客行動変化）
- 三次効果: 二次効果がさらに波及する長期的帰結（市場構造の変化、規制対応）
- 各戦略に対し、最低2階層先の影響連鎖を記述する
...
6. **バイアス自覚** — 自身の批判にもバイアスがないか内省する（「批判のための批判」を避ける）
