...
| バイアス | 説明 | 検出の問い |
|---------|------|-----------|
| 確証バイアス | 自説を裏付ける情報だけ集める | 「反証するデータは探されたか？」 |
| アンカリング | 最初の情報に過度に依存 | 「初期の数値が判断を歪めていないか？」 |
| 後知恵バイアス | 結果を知った後に予測できたと感じる | 「事前に結論に達する根拠はあったか？」 |
| フレーミング効果 | 提示方法で判断が変わる | 「逆のフレームで見たらどう判断するか？」 |
| 楽観バイアス | 成功確率を過大に見積もる | 「ベースレートはどうか？」 |
| 計画錯誤 | 所要時間を過小評価 | 「過去の類似プロジェクトの実績は？」 |
| 集団思考 | 全員が同意しているように見える | 「反対意見を表明する余地はあったか？」 |

### プレモーテム分析
1. 「1年後、この戦略は完全に失敗した」と仮定する
2. 「なぜ失敗したか」の理由を独立して列挙する
3. 最も致命的な失敗理由トップ3を選定する
4. 各理由に対する予防策を戦略に組み込む

### レッドチーム手法
1. 異なる前提で戦略を再構築
2. 競合視点シミュレーション: 自分が競合のCEOなら、どう対抗するか
3. 規制当局視点: 法規制変更時に持続可能か
4. 顧客視点の逆張り: 顧客がこの戦略に否定的なシナリオ

### 検証の深度レベル
| レベル | 適用場面 | 内容 |
|--------|---------|------|
| L1 ライト | 定型業務改善 | 主要前提3つの検証 |
| L2 スタンダード | 新規施策・中規模投資 | 全前提検証+ストレステスト |
| L3 ディープ | 大型投資・事業方針変更 | L2+プレモーテム+レッドチーム |

### アンチパターン
- 批判のための批判をしない: 建設的でない批判は価値を生まない
- 完璧を求めない: すべての前提が100%確実である必要はない
- 個人攻撃に陥らない: 戦略の欠陥を指摘するのであって提案者を攻撃しない
- 形式的な検証で済ませない: 「問題ない」という結論は本当に検証した結果か自問する
