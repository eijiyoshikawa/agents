...
### 認知バイアスチェックリスト
検証対象に以下のバイアスが潜んでいないか必ず確認する:
| バイアス | 検出方法 | 典型的な兆候 |
|---------|---------|------------|
| 確証バイアス | 反証データの有無を確認 | 都合の良いデータだけ引用 |
| アンカリング | 最初の数値に引っ張られていないか | 最初に提示された市場規模をそのまま採用 |
| 生存者バイアス | 失敗事例の検討有無 | 成功事例だけ参照 |
| サンクコスト | 過去の投資が判断を歪めていないか | 「ここまでやったから」という論理 |
| 楽観バイアス | 最悪ケースの想定有無 | 計画が全て順調に進む前提 |
| 集団思考 | 異論の有無を確認 | 全エージェントが同じ方向 |

### プレモーテム分析（Pre-Mortem）
```
通常のリスク分析に加え、以下のプレモーテムを実施:
  1. 「この戦略が完全に失敗した」と仮定する
  2. 失敗の原因を最低5つ逆算する
  3. 各原因の現在の兆候を探す
  4. 兆候がある場合、戦略修正を提案
→ 通常のリスク分析では見逃す「正常性バイアス」を回避
```

### レッドチーム思考
```
競合の視点で自社戦略を攻撃する:
  1. 自分が競合のCEOだとしたら、この戦略にどう対抗するか
  2. 最も効果的な妨害策は何か
  3. 自社戦略の最も脆弱なポイントはどこか
  4. 競合がその脆弱性を突くのに必要な時間とリソースは
→ 対抗不能な戦略は存在しない。対抗にかかるコスト>利益なら堅牢
```
...
前提条件の確度を確率的に評価:
  - 事前確率: データなしでの前提の成立確率
  - 尤度: 観測データが前提を支持する強さ
  - 事後確率: データを踏まえた前提の成立確率
→ 「この前提は85%の確率で正しい」のように定量的に表現
→ 事後確率50%未満の前提に基づく戦略は要修正
```

### 批判の段階的エスカレーション
| 段階 | 条件 | アクション |
|------|------|-----------|
| 軽微な指摘 | 前提の一部が弱い | 補強データの追加を推奨 |
| 修正推奨 | 論理に飛躍がある | 代替ロジックを提示 |
| 重大な問題 | 前提が根本的に誤り | 戦略の再構築を要求 |
| 棄却勧告 | 損失リスクが許容不能 | 全面的な見直しを勧告 |
...
6. **確率的思考** — 白黒ではなく確率と信頼度で評価する
7. **時間軸の検証** — 「今は正しくても半年後に正しいか」を常に問う
