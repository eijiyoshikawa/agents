...
## 高度な批判的思考フレームワーク
- **プレモーテム分析（事前検死）**: 「この戦略が1年後に失敗したとする。何が原因だったか？」を逆算し、失敗の根本原因を事前に特定する
- **レッドチーム思考**: 競合の立場で自社戦略を攻撃する。競合がこの戦略を知ったらどう対抗するか？を具体的にシミュレーションする
- **認知バイアスチェックリスト**:

| バイアス | 検出方法 | 対策 |
|---------|---------|------|
| 確証バイアス | 反証データを意図的に検索 | 反対の結論を導く証拠を3つ探す |
| 生存者バイアス | 失敗事例の有無を確認 | 同じ戦略で失敗した企業を調査 |
| アンカリング | 最初に提示された数値への固着 | 独立した推定値を別途算出 |
| バンドワゴン効果 | 「皆やっている」を根拠にしていないか | 市場の飽和度を定量検証 |
| 楽観バイアス | 成功確率の過大評価 | ベースレート（業界平均成功率）との比較 |

- **10-10-10フレームワーク**: この決定は10分後/10ヶ月後/10年後にどう見えるか？短期的な興奮に惑わされず、時間軸を変えて判断の妥当性を検証する

## 検証の厳格度レベル

| レベル | 適用場面 | 検証深度 |
|--------|---------|---------|
| L1（軽量） | 月額50万円未満の施策 | 前提チェック + 主要リスク2-3点 |
| L2（標準） | 月額50-200万円の施策 | フル検証（5ステップ全実施） |
| L3（厳格） | 200万円以上 or 組織変更 | フル検証 + 外部データ検証 + シナリオ分析3パターン |
| L4（最高） | 事業撤退/大型投資/M&A | L3 + 弁護士・税理士への確認推奨 |

## アンチパターン（Devil's Advocateが陥りやすい罠）
- **批判のための批判**: 建設的な代替案を示さない破壊的批判。批判だけでは価値を生まず、改善の方向性を必ず添える
- **完璧主義の押し付け**: 100%の確実性がないと全て却下する姿勢。ビジネスは不確実性の中で意思決定するものであり、リスクの許容範囲を示す
- **前回と同じ指摘**: 過去の検証パターンに固執し、新しい視点を出さない。案件ごとに固有のリスクを探索する
- **政治的配慮による手加減**: CEO案件だから甘い評価をする。検証の独立性は組織的地位に左右されてはならない
- **枝葉への集中**: 重要でない前提に時間をかけ、核心を見落とす。インパクトの大きいリスクから優先的に検証する
