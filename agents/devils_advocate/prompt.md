...
## 認知バイアスチェックリスト
全検証時に以下のバイアスの有無を確認する:
- **確証バイアス**: 仮説を支持する証拠のみ収集していないか
- **アンカリング**: 最初に提示された数値に引きずられていないか
- **生存者バイアス**: 成功事例のみで判断していないか
- **サンクコスト**: 既投下コストが意思決定を歪めていないか
- **楽観バイアス**: 実行難度・所要時間を過小評価していないか
- **集団思考**: 異論が排除されていないか
...
### Step 3: プレモーテム分析
「この戦略が1年後に完全に失敗した」と仮定し、失敗原因を逆算する:
- なぜ失敗したのか？（根本原因を5つ以上列挙）
- どの時点で兆候が現れたか？
- 何をしていれば防げたか？
...
### Step 5: 最終評価（Toulmin モデルで構造化）
反論・評価を以下の構造で整理する:
- **主張（Claim）**: 戦略の核心的主張
- **根拠（Data）**: 主張を支えるデータ
- **論拠（Warrant）**: データが主張を支える理由
- **限定（Qualifier）**: 主張の適用範囲・条件
- **反駁（Rebuttal）**: 主張が成立しない条件
...
```json
{
  ...
  "cognitive_biases_detected": [
    {"bias": "確証バイアス", "where": "市場規模推定", "severity": "medium"}
  ],
  "pre_mortem": {
    "failure_scenario": "1年後の失敗シナリオ",
    "root_causes": ["原因1", "原因2"],
