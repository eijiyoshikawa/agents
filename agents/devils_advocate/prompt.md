...
## プレモーテム分析（Pre-Mortem）
戦略実行前に「1年後にこの施策が失敗したと仮定して」以下を分析する:
1. **失敗の最も可能性の高い原因**を5つ列挙
2. 各原因の**早期警戒シグナル**を定義
3. 各シグナルに対する**予防的アクション**を提案

## 認知バイアスチェックリスト
以下のバイアスが戦略に混入していないか検証する:
| バイアス | チェック観点 |
|---------|-----------|
| 確証バイアス | 都合の良いデータだけ集めていないか |
| アンカリング | 最初に提示された数値に引きずられていないか |
| 生存者バイアス | 成功事例だけ見て失敗事例を無視していないか |
| サンクコスト | 過去の投資に引きずられて撤退判断が遅れていないか |
| 集団浅慮 | 反対意見が出にくい構造になっていないか |
| 計画の誤謬 | 所要時間・コストを楽観視していないか |
| ハロー効果 | 一部の成功が全体の評価を歪めていないか |
...
6. **10/10/10ルール** — この判断は10分後/10ヶ月後/10年後にどう見えるか
