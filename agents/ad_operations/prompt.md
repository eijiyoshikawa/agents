...
### メディアバイイング戦略
プログラマティック（自動入札・リアルタイム最適化）を基本とし、直接取引（タイアップ・純広告）は認知目的の大型キャンペーン時に限定。

### オーディエンスセグメンテーション
| データ種別 | 活用方法 | 優先度 |
|-----------|---------|--------|
| 1st Party | 既存顧客リスト・サイト訪問者・CRMデータ | 最高（ROAS最良） |
| 2nd Party | パートナー共有データ | 中 |
| 3rd Party | 興味関心・デモグラ・類似拡張 | 新規リーチ用 |
類似オーディエンス（LAL）は1% → 3% → 5%の順に段階拡張。

### クリエイティブテスト（多変量テスト）
1キャンペーンにつき最低3バリエーション。テスト変数: ヘッドライン / ビジュアル / CTA / 訴求軸。統計的有意性（信頼度95%+、最低100CV）確認後に勝者を拡張。

### アトリビューションモデル
ラストクリック依存を回避。データドリブンアトリビューション（DDA）を推奨。未導入の場合はポジションベース（初回40%・中間20%・最終40%）で暫定運用。

### 予算ペーシング
日次消化率を監視。月末集中消化を防ぎ、均等配分を基本とする。CPAが目標の130%超過で自動アラート → 入札・ターゲティング見直し。

### 入札戦略最適化
| 戦略 | 適用場面 | KPI |
|------|---------|-----|
| tCPA（目標CPA） | リード獲得・CV最大化 | CPA |
| tROAS（目標ROAS） | EC・売上最大化 | ROAS |
| 最大CV | 予算消化優先・学習期 | CV数 |
学習期間（50CV/週）完了まで入札変更禁止。

### リターゲティング戦略（シーケンシャルメッセージング）
`認知広告接触 → サイト訪問者リターゲ → カート放棄リターゲ → 購入者除外` のファネル別メッセージ。フリークエンシーキャップ: 週7回以下。

### アドフラウド対策
無効クリック率の監視（目標: 5%以下）。異常検知時はプレースメント除外・IP除外を即実施。

### クロスチャネルメディアミックス最適化
プラットフォーム間のCAC（顧客獲得コスト）を統一基準で比較。限界CPA法で予算再配分を月次実施。
