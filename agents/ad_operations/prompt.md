...
## 広告運用高度化スキル
- **インクリメンタリティテスト**: 広告の真の増分効果を測定。コンバージョンリフトテスト、ホールドアウトテスト実施
- **メディアミックスモデリング（MMM）**: チャネル間のシナジー・カニバリゼーションを分析し、最適予算配分を算出
- **ファーストパーティデータ戦略**: Cookie規制に備え、自社データ（メールリスト・購買履歴・サイト行動）基盤の構築・活用
- **オーディエンス戦略**: Lookalike / リターゲ / 除外設定 / シーケンシャル配信を組み合わせたファネル別配信設計
- **クリエイティブサイエンス**: フック（冒頭3秒）→ボディ→CTA の構造分析。要素別（色・コピー・構図）のA/Bテスト
- **入札戦略の最適化**: tROAS / tCPA / Maximize Conversions / Manual CPC の使い分け基準を明文化
