...
### 入札戦略の選定フレームワーク
| 目的 | Google推奨 | Meta推奨 | 適用条件 |
|------|-----------|---------|---------|
| CV最大化 | tCPA / tROAS | CBO | CV数が週30件以上 |
| 認知拡大 | CPM入札 | リーチ最適化 | ブランド認知KPI |
| トラフィック | CPC入札 | リンククリック | LP誘導が主目的 |
...
対処:
1. 新クリエイティブの投入（バリエーション3種以上同時テスト）
2. ターゲティングの拡張または変更
3. キャンペーン構造のリセット

### A/Bテスト設計原則
- 一度に変更する変数は1つだけ
- 統計的有意性を達成するまでテストを継続（最低7日間）
- 勝ちパターンは横展開し、負けパターンは即停止
- テスト結果はナレッジベースに蓄積

### 予算配分の動的最適化
- 日次でROAS/CPAを確認し、高パフォーマンスキャンペーンに予算を寄せる
- 低パフォーマンス（目標CPA150%超）は予算削減 or 停止
- 週末・月末など曜日・時期による変動パターンを把握

### アンチパターン
- 全予算を1つのキャンペーンに集中しない: リスク分散のため最低3キャンペーンに分散
- 学習期間中に設定を頻繁に変更しない: 自動入札の学習を阻害する
- クリエイティブ疲弊を放置しない: CTR低下は即対応
- コンバージョン未計測の広告を出さない: 効果測定不能な出稿は無駄
