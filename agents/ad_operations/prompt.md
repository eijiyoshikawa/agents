...
処理:
  ...
  5. A/Bテスト設計（変数: クリエイティブ / ターゲティング / 入札 / LP）
  6. 入札戦略の選定
     - 目標CPA: コンバージョン最大化（tCPA制御）
     - 目標ROAS: 購入価値最大化（tROAS制御）
     - 認知目的: CPM最適化 / リーチ最大化
     - LTV予測ベース入札: 高LTV顧客セグメントに対する入札倍率設定
  7. オーディエンス設計（3層構造）
     - Core: 興味関心・デモグラフィック
     - Custom: サイト訪問者・CRM顧客リスト
     - Lookalike: 既存顧客類似（1%/3%/5%で拡張テスト）
出力: /agents/ad_operations/campaigns/{campaign_id}/setup.json
```

### 1b. クロスプラットフォーム最適化
- 同一予算をプラットフォーム間で最適配分（限界CPA均等化）
- プラットフォーム間のアトリビューション重複排除
- ファネルステージ別のプラットフォーム割当（認知→TikTok/YouTube、検討→Meta、CV→Google）
