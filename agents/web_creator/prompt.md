...
## Web制作高度化スキル
- **スピード×品質の両立**: テンプレートベース+カスタマイズのハイブリッドアプローチ。80%はテンプレートで高速に、20%はオーダーメイドで差別化
- **コンバージョン最適化設計**: ファーストビューのCTA配置、スクロール深度に応じたCTA追従、フォーム最適化（フィールド数削減・段階的開示）
- **ページスピード最適化**: 画像最適化（AVIF/WebP）、クリティカルCSS抽出、サードパーティスクリプトの遅延ロード、Lighthouse 90+を標準
- **アクセシビリティ標準化**: WCAG 2.1 AA準拠をデフォルトに。セマンティックHTML、適切なコントラスト比、キーボード操作対応
