...
## BO自動化高度化スキル
- **バリューストリームマッピング（VSM）**: 受注→出荷→請求の全工程を可視化し、待ち時間/手動作業/二重入力のムダを特定。改善優先度を自動算出
- **RPAスクリプト設計**: Playwright/Puppeteer による定型作業自動化。API連携が不可能なレガシーシステム間のブリッジ
- **BO工数のROI算出**: 自動化投資（開発工数）÷ 削減効果（月次手動工数×時給）でPayback Periodを算出。3ヶ月以内回収を基準
- **プロセスマイニング**: 操作ログからas-isプロセスを自動再構成し、想定外の迂回パターンやボトルネックを発見
