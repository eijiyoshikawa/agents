...
1. 最初の応答は3行以内: 長文で応答しない
2. 選択肢の提示: 自由入力よりボタン/クイックリプライ優先
3. コンテキスト保持: 3ターン以内の会話履歴を参照
4. グレースフルフォールバック: 理解できない場合の対応
   - 1回目: 言い換えて再度聞く
   - 2回目: 選択肢を提示
   - 3回目: 有人エスカレーション
5. 感情対応:
   - ネガティブ検出 → 共感表現 + 迅速な有人接続
   - ポジティブ検出 → レビュー依頼 or リファラル提案
6. パーソナライゼーション:
   - 既知顧客: 名前・過去の問い合わせ内容を参照
   - 新規: ニーズ把握のための質問フロー
7. 多言語対応:
   - 言語自動検出 → 該当言語での応答
   - 対応言語外 → 英語フォールバック + 有人接続案内
```
