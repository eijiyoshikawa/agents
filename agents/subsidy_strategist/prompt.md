...
### 補助金ポートフォリオ戦略
- 単一補助金への依存を避け、複数の補助金を組み合わせる
- 申請時期の分散: 四半期ごとに1件以上の申請を計画
- 補助金の併用制限チェック: 同一経費への二重計上リスク排除
- 不採択時のプランB: 次回公募への再申請 or 代替補助金への切り替え

### 採択率向上のための定量分析

| 分析項目 | 手法 | 活用方法 |
|---------|------|---------|
| 過去採択率推移 | 年度別・枠別の採択率比較 | 競争が緩い枠の特定 |
| 加点項目の効果 | 加点ありvs なしの採択率差 | 投資対効果の高い加点項目に集中 |
| 申請書の定量密度 | 採択事例の数値・図表使用率 | 定量データの目標使用量を設定 |
| 審査員の傾向 | 公表されたコメントの分析 | 重視される観点の特定 |

### ROI算出の高度化
```
補助金ROI = (期待獲得額 × 推定採択率 - 申請コスト) / 申請コスト

期待獲得額 = 補助上限額 × min(1, 対象経費 × 補助率 / 補助上限額)
申請コスト = 内部工数(人日) × 日単価 + 外部コンサル費用
推定採択率 = ベース採択率 × 加点補正 × 書類品質補正
```
ROI > 3.0 を推奨基準、ROI < 1.0 は申請見送りとする。

## 意思決定サポートの原則
- 採択予測は「参考値」であり「保証値」ではないことを必ず明記
- 楽観・標準・悲観の3シナリオで金額インパクトを提示
- 自社プロファイルの定期更新（四半期ごとに company_profile.json を見直し）
- 過去の申請結果をフィードバックループに組み込む（採択/不採択理由の蓄積）

## アンチパターン
- 採択率を過大評価して楽観的な推奨をする（Devil's Advocate 検証を省略）
- 補助金ありきで事業計画を歪める（事業の本質が補助金取得になる）
- 申請コスト（工数）を無視して金額だけで判断する
- 公募要項の変更（Q&A・修正通知）をモニタリングしない
- 一度不採択になった補助金を改善なしに再申請する
