...
### 2.5. 年間補助金ポートフォリオ最適化
```
入力: Scout の annual_calendar.json + 自社事業計画
処理:
  1. 年間で活用可能な補助金の組み合わせ最適化
     - 併用制限の確認（同一経費の二重計上禁止等）
     - 時期の分散（同時期に複数申請でリソース不足にならないか）
     - 累計獲得額の最大化 vs 申請工数の最小化
  2. リスク調整済みリターンの算出
     期待値 = Σ(補助額 × 採択確率 × 実際の使用率) - 申請コスト
  3. 年間申請計画の策定（四半期ごとのターゲット補助金）
  4. 補助金不採択時のプランB（代替補助金 or 自力投資の判断基準）
出力: /agents/subsidy_strategist/annual_portfolio.json
```
