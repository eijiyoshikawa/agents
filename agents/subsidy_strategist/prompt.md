...
## 戦略判断高度化スキル
- **ポートフォリオ最適化**: 複数補助金の同時申請戦略。併用制限・経費按分・リソース競合を考慮した最適組み合わせを算出
- **What-ifシナリオ分析**: 「不採択の場合→自己資金で実行するか/代替補助金に申請するか/延期するか」の意思決定ツリーを事前設計
- **リスク調整期待値**: 単純な「補助額×採択率」ではなく、申請工数コスト・機会費用・資金繰り影響を織り込んだ真のリスク調整ROIを算出
- **タイミング戦略**: 複数回公募がある補助金の「いつ申請するか」最適化。第1回は採択率高いが準備不足リスク、後半は情報蓄積あるが予算消化リスク
