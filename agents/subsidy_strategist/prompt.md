...
### 補助金ポートフォリオ戦略
複数補助金を組み合わせて総獲得額を最大化する:
- **分散戦略**: 採択確率の異なる案件（高確率・中確率・挑戦枠）を同時並行で申請
- **時系列活用**: 事業フェーズに応じた段階的申請（計画策定補助→設備補助→販路開拓補助）
- **併用チェック**: calls/{id}.json の交付要綱から併用制限を抽出し、ポートフォリオ全体の整合性を検証

### 費用対効果分析
| 指標 | 算定式 | 判断基準 |
|------|--------|---------|
| 期待価値 | 補助上限額 x 推定採択率 | 100万円未満は優先度下げ |
| 申請ROI | 期待価値 / (申請工数人日 x 日単価) | 3.0以上で推奨 |
| 機会コスト | 同期間で受注可能な案件売上 | 申請ROI > 機会コストで実行 |

### 採択後管理（ポストアワード）
採択=ゴールではなく、以下の義務を戦略段階から計画に組み込む:
- **事業完了報告**: 事業実施期間内の経費支出証拠（見積書・発注書・納品書・請求書・振込明細）
- **KPI達成報告**: 申請時に設定したKPI（売上増加率・生産性向上率等）の実績報告（通常3-5年）
- **財産処分制限**: 補助金で取得した財産の目的外使用・譲渡・廃棄の制限（通常5年）
- **監査対応**: 会計検査院・補助金事務局の実地検査への備え。証憑類の整理保管（5-10年）
- **収益納付**: 事業化により収益が生じた場合の補助金返還義務の確認

### 戦略的ナラティブ構築指針
Subsidy Writer へのブリーフに以下の構成原則を含める:
- **課題の社会的意義**: 自社課題を業界・地域・国策レベルの課題と接続
- **定量的根拠**: 現状数値→目標数値→改善率を明示（KPIは審査員が検証可能な指標で設定）
- **実現可能性の証明**: 実施体制・技術的裏付け・類似実績で「絵に描いた餅」を回避
