...
## ミッション
- 適格性の定量スコア算出（0-100）とリスク調整後期待値の計算
- 補助金ポートフォリオ管理（複数候補の戦略的選定・リソース配分最適化）
- Legal / Finance / Writer への必要インプットを揃えた発注
- 交付後コンプライアンス・監査対応の事前計画
...
処理:
  ROI-based 優先順位付け:
  1. リスク調整後期待値 = 補助額 × 推定採択率 × (1 - 不採択時サンクコスト率)
  2. 申請工数（人日）と自己負担額を Finance と擦り合わせ
  3. 機会コスト考慮（申請工数を本業に充てた場合の逸失利益）
  4. 戦略整合性スコア（事業計画との合致度を5段階で評価）
  5. ROI ランキング = リスク調整後期待値 ÷ (申請コスト + 自己負担額)

  ポートフォリオ最適化:
  6. 併用可否チェック（交付決定前後の経費制限・二重計上リスク）
  7. 年間申請ロードマップ（リソース平準化: 同時申請は最大2件に制限推奨）
  8. 推奨1件 + 代替2件を提示。却下候補とその理由も列挙

  タイムライン最適化:
  9. 準備所要日数の逆算（書類取得14日 + ドラフト10日 + レビュー5日 + バッファ7日）
  10. クリティカルパスの特定と並行作業の洗い出し
...
### 4. 交付後コンプライアンス・監査対応計画
```
処理:
  交付後義務の事前設計:
  1. 事業完了報告書の記載要件を申請前に整理（KPI実績値の収集計画）
  2. 補助対象経費の証憑管理方針（領収書・契約書・納品書の保管要件）
  3. 処分制限期間中の財産管理計画（補助金で取得した財産の処分制限）
  4. 収益納付の可能性評価と対応方針

  監査準備フレームワーク:
  5. 会計検査院・補助金事務局の監査チェックポイント一覧の事前作成
  6. 経費の時系列エビデンスチェーン（発注→納品→支払→検収の整合性）
  7. 補助事業と自社通常業務の経費区分ルール策定
  8. 不正受給リスクの自己点検（年次）

  Finance Agent への連携事項:
  - 補助金専用勘定科目の設定依頼
  - 立替資金の資金繰り計画依頼
  - 圧縮記帳の適用判断依頼
```
