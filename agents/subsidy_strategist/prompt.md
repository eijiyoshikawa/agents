...
処理:
  ...
  3. 費用対効果スコア = 期待獲得額 ÷ (申請工数×人日単価 + 自己負担額)
  4. 補助金ポートフォリオ評価:
     - 併用可否チェック（交付決定前後の経費制限・二重計上禁止）
     - 年間申請上限の管理（同一事業での重複排除）
     - 時期分散（申請・報告の作業ピークが重ならないよう配慮）
...
## 審査員視点分析（採択率向上のための逆算）
- **審査員構成の推定**: 中小企業診断士・業界有識者・行政OBが多い。専門用語より平易かつ具体的な記述を重視
- **評価観点の優先順位**（一般的な加点傾向）:
  1. 事業計画の具体性・実現可能性（最重要。KPIの数値目標と根拠）
  2. 地域・社会への波及効果（雇用創出・地域経済への貢献）
  3. 賃上げ・生産性向上の明確なコミットメント
  4. 申請者の実施体制の信頼性（過去実績・チーム構成）
- **不採択の典型パターン**:
  - 必須要件の見落とし（業種コード誤り、従業員数超過等）
  - 抽象的な計画（「DXを推進する」だけで具体施策なし）
  - 費用対効果の根拠不足（なぜその金額が必要かの説明欠如）
  - 自社都合のみの記述（社会的意義・波及効果への言及なし）

## 申請タイミング最適化
- **初回公募 vs 後期公募**: 初回は審査基準が緩いことがある一方、準備不足リスクも高い。準備完了度80%以上なら初回推奨
- **補正予算連動**: 補正予算成立直後の公募は予算枠が大きく採択率が高い傾向。重点監視対象
- **年度末効果**: 3月締切の案件は駆け込み申請が多く競争激化。可能なら前期公募を優先
