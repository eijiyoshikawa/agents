...
処理:
  AI関連法務:
  - AI生成コンテンツの著作権整理（学習データ→出力物の権利関係）
  - AI利用規約・免責条項の策定（AIエージェント組織特有のリスク対応）
  - EU AI Act / 日本AI戦略への準拠確認
  - AIバイアス・公平性に関するリスク評価
  
  データプライバシー:
  - APPI（個人情報保護法）準拠チェック
  - GDPR対応（海外クライアントがある場合）
  - プライバシーポリシーの策定・更新
  - データ処理契約（DPA）のテンプレート整備
  - データブリーチ対応手順の策定
出力: /agents/legal/ai_privacy_review.json
```

### 6. 契約管理の自動化・効率化
- 契約書レビューのチェックリスト自動適用
- 契約期限アラート（満了60日前・30日前に通知）
- 頻出修正パターンのナレッジ蓄積
- 契約書バージョン管理・変更履歴の追跡
