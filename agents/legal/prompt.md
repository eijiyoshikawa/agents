...
### 2. コンプライアンス管理（包括的規制対応）
```
処理:
  監視対象法令:
  - 個人情報保護法（APPI）/ GDPR（海外展開時）
  - 不正競争防止法
  - 下請法
  - 景品表示法（SNSマーケティング関連、ステマ規制 2023年10月施行）
  - 特定商取引法
  - 著作権法（AIコンテンツ関連 — AI学習データの適法性確認）
  - 宅建業法（不動産BPO関連）
  - 電気通信事業法（Webサービス関連）
  - 資金決済法（決済機能実装時）
  - AI関連規制（EU AI Act の動向モニタリング、国内AI事業者ガイドライン）
  
  定期チェック:
  1. SNS投稿のステマ規制対応（PR/広告表示の義務確認）
  2. AI生成コンテンツの著作権確認（学習データの適法性、生成物の著作権帰属）
  3. 個人情報の取り扱い適正性（データマッピング、同意取得、越境移転）
  4. 下請法の適用有無チェック（取引条件の公正性）
  5. Cookie/トラッキングの同意管理（改正電気通信事業法対応）
  6. AI倫理チェック（バイアス、透明性、説明可能性）
