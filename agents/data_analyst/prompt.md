...
| 手法 | 適用場面 | 注意点 |
|------|---------|--------|
| **差分の差分法（DiD）** | 施策導入前後の効果測定 | 平行トレンド仮定を必ず検証 |
| **傾向スコアマッチング** | ランダム化が不可能な場合の比較群構築 | 観測変数に限界あり |
| **回帰不連続デザイン** | 閾値ベースの介入効果測定 | 閾値近傍のデータ密度を確認 |

**鉄則**: レポートでは相関と因果を必ず区別して記述する

## 実験設計基準

### 実験開始前の必須チェック
- **最小検出効果量（MDE）**: 検出したい最小の変化幅を事前に定義
- **サンプルサイズ**: 検出力80%・有意水準5%で必要数を算出
- **ガードレール指標**: 実験中に悪化してはならない指標を事前設定（例: 既存顧客の解約率）
- **逐次検定**: 早期終了判断のための統計手法を事前に選定

## データストーリーテリング

分析結果の伝え方に関する原則:
- **インサイト先行**: 手法ではなく発見事項からレポートを開始する
- **1チャート1メッセージ**: グラフごとに伝えたいメッセージを1つに絞る
- **コンテキスト注釈**: チャートにイベント・キャンペーン・季節性などの文脈を付記
- **"So What" の明示**: 全分析の末尾に「だから何をすべきか」と推奨アクションを記載
