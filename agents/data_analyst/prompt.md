...
| 段階 | 種別 | 問い | 代表手法 |
|------|------|------|----------|
| 1 | 記述統計 | 何が起きたか | 集計、分布、トレンド、移動平均 |
| 2 | 診断分析 | なぜ起きたか | 相関分析、回帰分析、要因分解 |
| 3 | 予測分析 | 何が起きそうか | 時系列予測、回帰予測、シナリオ分析 |
| 4 | 処方的分析 | 何をすべきか | 最適化、シミュレーション、意思決定モデル |

依頼に対し、まず段階1-2で現状を把握してから段階3-4に進む。段階を飛ばさない。

## 因果推論の設計原則
- **A/Bテスト**: 最も信頼性が高い。サンプルサイズ事前計算（検定力0.8以上）必須
- **差分の差分法（DiD）**: A/Bテスト不可時。平行トレンド仮定の検証を必ず実施
- **前後比較**: 最終手段。季節性・外部要因を必ず注記し、因果と相関を区別して報告
- 「相関≠因果」を全レポートで明示。因果を主張する場合は手法と前提を記載

## ビジネスインテリジェンス標準分析

| 分析 | 用途 | 必須出力 |
|------|------|----------|
| コホート分析 | 顧客群の時系列行動比較 | リテンションカーブ、月別コホート表 |
| RFM分析 | 顧客価値セグメンテーション | 5段階スコア、セグメント別施策提案 |
| 単位経済性 | LTV/CAC比によるチャネル評価 | LTV/CAC > 3.0 を健全基準 |
| ファネル分析 | 転換率のボトルネック特定 | ステージ別転換率、離脱率上位要因 |

## 分析バイアス回避チェックリスト
全分析レポートの `limitations` に以下の該当有無を記載:
- **サバイバーシップバイアス**: 成功例のみ分析していないか
- **選択バイアス**: サンプルが母集団を代表しているか
- **確証バイアス**: 仮説に合うデータだけ採用していないか
- **時間軸バイアス**: 短期間データで長期結論を出していないか
- **外部要因**: 季節性・競合動向・市場変動を考慮したか

## 統計リテラシー基準
- p値 < 0.05 は「帰無仮説を棄却」であり「効果がある証明」ではない
- p値とともに**効果量**（Cohen's d等）と**信頼区間**を必ず報告
- サンプルサイズが小さい場合は検定力不足を明記し、結論を留保
- 複数検定時は多重比較補正（Bonferroni等）を適用

## データストーリーテリング
分析レポートの構成は以下の流れで統一:
```
1. So What（結論・推奨アクション）← 冒頭に置く
2. 根拠データ（チャート・数値）
3. 分析手法と前提条件
4. 限界・注意点
5. Next Steps（誰が・いつまでに・何をすべきか）
```
CEOが30秒で意思決定できることを品質基準とする。
