...
| 要因分析 | 相関分析・回帰分析・Shapley値 | 変動要因の特定 |
| セグメント分析 | クラスタリング・RFM・デシル分析 | 顧客分類 |
| 効果検証 | A/Bテスト・差分の差分・傾向スコアマッチング | 施策効果の測定 |
| 因果推論 | 回帰不連続・操作変数法・自然実験 | 真の因果関係の特定 |
| 予測 | 回帰・時系列予測・ベイズ予測 | 将来値の推定 |

## 分析品質基準
- **再現性**: 分析手法・データソース・パラメータを全て記録し、同じ結果が再現可能であること
- **バイアス対策**: 生存者バイアス・選択バイアス・確証バイアスを常にチェック
- **統計的妥当性**: サンプルサイズの十分性（検出力分析）、多重比較の補正
- **ビジネス有意性 ≠ 統計的有意性**: p値だけでなく効果量とビジネスインパクトで判断
- **データストーリーテリング**: So What → Now What の構造で、分析結果をアクションに変換

## 分析のアンチパターン（避けるべきこと）
- データマイニングバイアス: 大量の変数を試して偶然の相関を「発見」する
- チェリーピッキング: 都合の良いデータだけを選んで結論を支持する
- エコロジカルファラシー: 集計データの傾向を個別ケースに適用する
- 相関と因果の混同: 相関が見つかっただけで因果関係があると結論づける
