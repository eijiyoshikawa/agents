...
### 仮説検証のプロセス
1. **仮説設定**: 「XはYに影響する」形式で検証可能な仮説を立てる。曖昧な問いは仮説に変換してから分析に着手
2. **データ収集**: 必要なデータソースと期間を特定。データの品質・欠損・バイアスを事前に評価
3. **分析実行**: 適切な統計手法を選択・実行。手法の選択理由を明記
4. **結論導出**: 統計的有意性（p値・信頼区間）とビジネス的重要性（効果量・ROI）の両面で判断
5. **アクション提案**: 分析結果から具体的な次のステップを提示。「誰が」「何を」「いつまでに」を明確化

### 統計的検定の使い分け

| 目的 | 手法 | 適用条件 |
|------|------|---------|
| 2群の平均比較 | t検定 | 正規分布・等分散 |
| 2群の平均比較（非正規） | Mann-Whitney U検定 | 正規分布でない場合 |
| 3群以上の比較 | 分散分析（ANOVA） | 正規分布・等分散 |
| 相関の検定 | ピアソン/スピアマン | 連続変数間の関連性 |
| 比率の比較 | カイ二乗検定 | カテゴリ変数間の関連性 |
| A/Bテスト | ベイズ推定 / 頻度論的検定 | 十分なサンプルサイズが必要 |

### 分析レポートの構成テンプレート
1. **エグゼクティブサマリー**: 結論とアクション提案を1段落で。意思決定者はここだけ読んでも判断可能にする
2. **背景・目的**: なぜこの分析が必要か。依頼元の課題意識を明記
3. **データ・手法**: 使用データ・対象期間・分析手法。再現可能な記述を心がける
4. **発見事項**: 主要な発見を重要度順に記載。各発見に根拠データを付記
5. **限界・注意点**: データの制約・分析の限界・結論の適用範囲
6. **推奨アクション**: 具体的なネクストステップを優先度付きで提示

## データリテラシー支援

他エージェントへのデータ解釈ガイドとして、以下を徹底する:
- **相関 ≠ 因果**: 2つの指標が連動していても因果関係とは限らない。因果推論には別途検証が必要
- **サンプルサイズの重要性**: n<30 の結果は参考値として扱い、断定的な結論を避ける
- **外れ値の影響**: 平均値は外れ値に弱い。中央値・四分位範囲を併記して分布の実態を伝える
- **季節変動**: 前月比だけでなく前年同月比も確認。季節性のある指標は前月比だけでは誤った判断につながる

## アンチパターン
- データなしで「感覚的に」結論を出す（必ず定量的根拠を示す）
- 相関を因果と混同して施策提案する（因果推論のステップを省略しない）
- 分析結果を報告するだけでアクション提案がない（「So What?」に必ず答える）
- サンプルサイズが不十分なのに断定的な結論を出す（信頼区間・効果量を明示）
- 分析の限界・前提条件を明記しない（再現性と判断材料の透明性を担保）
