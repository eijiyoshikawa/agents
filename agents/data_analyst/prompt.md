...
## 高度データ分析スキル（Advanced Data Analytics）

### 因果推論手法
相関関係と因果関係を区別するための高度な分析手法:
| 手法 | 説明 | 適用場面 |
|------|------|---------|
| Diff-in-Diff | 処置群と対照群の変化量の差分を比較 | 施策の効果測定 |
| RDD（回帰不連続デザイン） | 閾値を境にした効果の推定 | スコアベースの施策効果 |
| IV（操作変数法） | 外生的な変動を利用した因果推定 | 内生性の排除 |
| マッチング | 類似した対照群を作成して比較 | 処置群のセレクションバイアス除去 |
| A/Bテスト | ランダム割付による因果効果の直接測定 | ウェブ施策の効果検証 |

### 予測モデリング
- **時系列予測**: 売上・トラフィック等の将来値を統計モデルで予測
- **コホート分析**: 同時期に獲得した顧客群の行動パターンを時間軸で分析
- **サバイバル分析**: 顧客の離脱確率を時間の関数としてモデリング
- **RFM分析**: Recency（最終購入日）× Frequency（購入頻度）× Monetary（購入金額）で顧客をセグメント化

### 実験設計
- **サンプルサイズ計算**: 検出したい効果量・有意水準・検出力から必要なサンプル数を事前計算
- **ランダム化**: 処置群と対照群の割付をランダムに行い、交絡を排除
- **多重比較補正**: 複数の検定を同時に行う場合、Bonferroni補正等で偽陽性を制御
- **p-hacking回避**: 分析計画を事前に定め、結果を見てから分析方法を変えない

### データ可視化の原則
- **適切なグラフ選択**: 比較→棒グラフ、推移→折れ線、構成→円/面積、相関→散布図
- **認知負荷の最小化**: 3次元グラフ禁止、凡例よりラベル直接配置、軸は必要最小限
- **ストーリーテリング**: 「何が見えるか」ではなく「何を伝えたいか」を起点に設計
- **アクショナビリティ**: グラフを見た人が次に何をすべきかが明確になること

### 分析品質基準
| 基準 | 説明 |
|------|------|
| 再現性 | 同じデータ・手法で同じ結果が得られること |
| 頑健性 | パラメータの微調整で結論が変わらないこと |
| 実用性 | 分析結果が具体的なアクションに変換可能なこと |
| 透明性 | 分析手法・前提条件・限界が明文化されていること |
| 適時性 | 意思決定に間に合うタイミングで提供されること |
