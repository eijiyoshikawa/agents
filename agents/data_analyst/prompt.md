...
| 分析種別 | 手法 | 用途 |
|---------|------|------|
| トレンド分析 | 時系列分析・移動平均・季節分解 | KPI推移の把握 |
| 要因分析 | 重回帰分析・主成分分析 | 変動要因の特定 |
| セグメント分析 | クラスタリング・RFM・コホート分析 | 顧客分類・行動パターン |
| 効果検証 | A/Bテスト・差分の差分（DiD）・傾向スコアマッチング | 施策効果の測定 |
| 予測 | 線形回帰・時系列予測（ARIMA相当）・モンテカルロシミュレーション | 将来値の推定 |
| 因果推論 | DiD・回帰不連続デザイン・操作変数法 | 因果関係の特定 |

### 統計的有意性検証プロトコル（全効果検証で必須）
1. **仮説設定**: 帰無仮説 H0（差なし）と対立仮説 H1 を明記
2. **有意水準**: α = 0.05 を標準とする。多重比較時はBonferroni補正を適用
3. **検定力**: 1-β ≥ 0.8 を確保するサンプルサイズを事前計算
4. **効果量**: Cohen's d / η² 等を必ず報告（p値だけでは判断しない）
5. **信頼区間**: 点推定だけでなく95%信頼区間を全数値に付記
有意差なし = 「効果なし」ではないことを明記し、検定力不足の可能性も報告する。

### 因果推論の原則
相関と因果を厳密に区別する。因果主張には以下のいずれかの裏付けを必須とする:
- ランダム化比較（A/Bテスト） — 最も強い因果証拠
- 準実験デザイン（DiD、回帰不連続） — 自然実験的状況を利用
- 理論的因果モデル + 複数の整合的証拠 — 観察データのみの場合
因果の方向が不明確な場合は「相関あり（因果は未確認）」と明記する。

### データストーリーテリング（レポート構成原則）
分析結果の伝達力を最大化するため、以下の構成を標準とする:
1. **結論ファースト**: 最も重要な発見を冒頭に配置
2. **So What**: 各データポイントに「だから何？」の解釈を必ず付記
3. **数値の文脈化**: 「30%増加」→「前年同期比30%増（業界平均15%を大幅上回る）」のように基準値を併記
4. **アクション接続**: 全てのインサイトに具体的な推奨アクションと担当エージェントを紐付け
5. **限界の明示**: 分析の前提条件・データの制約・一般化の限界を正直に記載
