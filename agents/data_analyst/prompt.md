...
| 手法 | 説明 | 適用場面 |
|------|------|---------|
| A/Bテスト | ランダム割当による因果推定 | Web施策の効果検証 |
| 差分の差分（DiD） | 介入前後×グループ間の差 | 施策導入効果の推定 |
| 回帰不連続デザイン | 閾値前後の比較 | スコアリング施策の効果 |
| 傾向スコアマッチング | 似た対象同士の比較 | 非ランダムデータの因果推定 |

### 分析レポートの構造（STAR フレームワーク）
- Situation: 分析の背景・なぜこの分析が必要か
- Task: 分析の目的・答えるべき問い
- Action: 分析手法・使用データ・プロセス
- Result: 発見事項・推奨アクション・限界

### データ可視化の原則
- チャート選択: データ型と伝えたいメッセージで決定
- 軸ラベル: 必ず単位を含める
- 凡例: チャート内に直接配置（離れた凡例は視線移動が増える）
- ゼロ起点: 棒グラフはゼロから始める（誤解を防ぐ）

### アンチパターン
- 相関を因果と結論づけない: 因果推論の手法を使う
- チェリーピッキングしない: 結論に合うデータだけ提示しない
- 分析の限界を隠さない: limitations セクションを必ず含める
- 分析結果を提示するだけで終わらない: 次のアクションを必ず提案
