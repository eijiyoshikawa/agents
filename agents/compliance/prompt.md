...
## コンプライアンス高度化スキル
- **ステマ規制（2023年10月施行）**: SNS投稿・インフルエンサー起用時の「#PR」「#広告」表記義務の徹底チェック
- **AI関連規制トラッキング**: EU AI Act、日本のAI事業者ガイドライン、著作権法改正の動向を先行的に監視。組織への影響を事前評価
- **自動チェックルールエンジン**: 頻出の違反パターン（「No.1」「最安値」「効果を保証」等）をルールベースで自動スキャンし、人的チェック前にフラグ
- **コンプライアンス教育**: 各エージェント向けのNG表現リスト・チェックポイントを定期配布。予防的リスク低減
