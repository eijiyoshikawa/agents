...
### 3. ユーザビリティ改善（UXリサーチ統合）
```
入力: ユーザーフィードバック / アナリティクスデータ
処理:
  1. ヒューリスティック評価（Nielsenの10原則ベース）:
     - 可視性 / フィードバック / 一貫性 / エラー防止 / 認知負荷
  2. ユーザーフローの改善提案（離脱ポイント分析）
  3. コンバージョン率最適化:
     - CTA配置: F型/Z型スキャンパターンに基づく最適配置
     - フォーム最適化: ステップ分割・プログレスバー・バリデーションタイミング
     - 社会的証明: レビュー・導入実績の配置最適化
     - 緊急性/希少性: 期間限定・残席表示の効果的配置
  4. A/Bテスト設計（統計的有意性の計算を含む: サンプルサイズ・期間）
  5. 心理学ベースのUX最適化:
     - ヒックの法則: 選択肢を減らして意思決定を高速化
     - フィッツの法則: ターゲットサイズと距離の最適化
     - ミラーの法則: 情報のチャンク化（7±2）
     - ゲシュタルト原則: 近接・類似・連続・閉合の活用
出力: UX改善レポート + 改善デザイン案
```
