...
### 3. ユーザビリティ改善（ニールセンの10ヒューリスティクス準拠）
```
入力: ユーザーフィードバック / アナリティクスデータ
処理:
  1. ヒューリスティック評価（以下の10原則でスコアリング: 各1-5点）
     H1: システム状態の可視性（ローディング・進捗・フィードバック）
     H2: システムと現実世界の一致（ユーザーの言葉・自然な順序）
     H3: ユーザーコントロールと自由度（取り消し・やり直し）
     H4: 一貫性と標準（プラットフォーム慣習の遵守）
     H5: エラー防止（確認ダイアログ・制約によるミス防止）
     H6: 記憶より認識（選択肢・操作・情報の可視化）
     H7: 柔軟性と効率性（ショートカット・カスタマイズ）
     H8: ミニマルデザイン（不要情報の排除）
     H9: エラーからの回復支援（平易な言葉・原因・解決策の提示）
     H10: ヘルプとドキュメント（検索可能・タスク指向・簡潔）
  2. ユーザーフローの改善提案
...
### 情報アーキテクチャ（IA）原則
```
- ラベリング: ユーザーの語彙に合わせる（社内用語・技術用語を避ける）
- ナビゲーション: グローバル（主要導線）+ ローカル（セクション内）+ パンくず
- 階層: 3クリック以内で目的ページに到達可能な構造設計
- 検索: コンテンツ50ページ以上で検索機能を必須化
- カードソーティング: 新規IA設計時はユーザーのメンタルモデルを検証
```
...
**Figma Handoff 記述**: 各コンポーネントに `motion_key` / `duration` / `reduced-motion fallback` を明記
