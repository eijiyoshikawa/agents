...
## 高度UXデザインスキル（Advanced UX Design）

### デザインシンキング（Stanford d.school）
1. **共感（Empathize）**: ユーザーの行動・感情・文脈を深く理解する
2. **問題定義（Define）**: 真のユーザー課題を明確にする（「〇〇として、△△したい。なぜなら□□だから」）
3. **創造（Ideate）**: 制約なく多数のソリューションを発散的に生成する
4. **プロトタイプ（Prototype）**: 最小限のモックで素早く形にする
5. **テスト（Test）**: 実ユーザーでテストし、学びをフィードバックする

### ユーザビリティヒューリスティクス（Nielsen's 10原則）
- [ ] システム状態の可視化（ローディング、進捗、完了状態）
- [ ] システムと実世界の一致（ユーザーの言葉で、業界用語で）
- [ ] ユーザーの制御と自由（取り消し・やり直しの提供）
- [ ] 一貫性と標準（同じ操作は同じ結果）
- [ ] エラー防止（確認ダイアログ・制約による誤操作防止）
- [ ] 記憶より認識（オプションを表示し、記憶に頼らせない）
- [ ] 柔軟性と効率性（ショートカット、パワーユーザー対応）
- [ ] 美的で最小限のデザイン（情報の優先度に基づく表示）
- [ ] エラーからの回復支援（エラーメッセージは解決策を含む）
- [ ] ヘルプとドキュメント（コンテキストに応じたヘルプ）

### ユーザーリサーチ手法
| 手法 | 目的 | タイミング |
|------|------|----------|
| コンテキスチュアルインクワイリ | 実環境での行動観察 | 要件定義前 |
| カードソーティング | 情報アーキテクチャ設計 | IA設計時 |
| ユーザビリティテスト | タスク完了率・エラー率の計測 | プロトタイプ後 |
| A/Bテスト | デザイン案の定量比較 | リリース後 |
| ヒートマップ分析 | クリック・スクロール行動の可視化 | リリース後 |

### 情報アーキテクチャ
- **ナビゲーション設計**: 3クリック以内で目的の情報に到達できること
- **ラベリング**: ユーザーが理解できる言葉でメニュー・ボタンを命名
- **検索設計**: サジェスト・フィルタ・ソートの適切な提供
- **コンテンツ階層**: 最重要情報をファーストビューに配置（Fパターン/Zパターン）
