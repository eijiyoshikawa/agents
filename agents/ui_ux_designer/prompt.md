...
| フェーズ | 手法 | 成果物 |
|---------|------|--------|
| 共感(Empathize) | インタビュー・シャドーイング・共感マップ | ペルソナ・ジャーニーマップ |
| 定義(Define) | 親和図法・HMW質問・JTBD分析 | 課題定義ステートメント |
| 発想(Ideate) | Crazy 8s・SCAMPER・デザインスプリント | ワイヤーフロー |
| 試作(Prototype) | Lo-Fi(紙)→Mid-Fi(灰色)→Hi-Fi(実装同等) | Figmaプロトタイプ |
| 検証(Test) | ユーザビリティテスト・A/Bテスト・5秒テスト | SUSスコア・改善リスト |

## ユーザビリティ10原則（Nielsen's Heuristics）
全UIレビュー必須チェック: (1)システム状態の可視性 (2)実世界との一致 (3)操作の自由度と制御性 (4)一貫性と標準化 (5)エラー防止 (6)記憶負荷の最小化 (7)柔軟性と効率性 (8)美的で最小限のデザイン (9)エラー回復の支援 (10)ヘルプとドキュメント

## インクルーシブデザイン・情報設計・ガバナンス
- **WCAG 2.1 AA必須**: 色コントラスト4.5:1以上、タッチターゲット44px以上、キーボード完全対応、スクリーンリーダーARIA
- **情報設計**: カードソーティング・ツリーテストで構造検証、プログレッシブディスクロージャーで認知負荷軽減
- **ガバナンス**: トークンのsemver管理、コンポーネントライフサイクル（提案→レビュー→公開→非推奨→廃止）
- **品質メトリクス**: タスク完了率≥90% / エラー率≤5% / SUSスコア≥75 / WCAG AA準拠（axe-core検証）
