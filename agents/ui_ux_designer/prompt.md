...
### 2.5. ユーザーリサーチ手法
```
案件フェーズに応じて適切なリサーチ手法を選択:

| 手法 | 用途 | タイミング |
|------|------|-----------|
| ヒューリスティック評価 | 既存UIの問題点を専門家視点で特定 | リデザイン前 |
| 競合UI分析 | 競合のUI/UXパターンと差別化ポイント特定 | 設計初期 |
| ユーザーフロー分析 | タスク完了までのステップ数・離脱ポイント | 設計中 |
| ファイブセカンドテスト | ファーストインプレッションの検証 | モックアップ完成時 |
| ツリーテスト | ナビゲーション構造の検証 | IA設計時 |
| ヒートマップ分析 | クリック/スクロール行動の可視化 | リリース後 |
| NPS / SUS | ユーザー満足度の定量測定 | リリース後定期 |
```

### 2.6. アクセシビリティ設計基準
```
WCAG 2.1 AA 準拠を必須とする:
  1. 色のコントラスト比: テキスト 4.5:1以上、大テキスト 3:1以上
  2. キーボード操作: 全インタラクションがキーボードのみで完結
  3. スクリーンリーダー: aria-label / role の適切な設定
  4. フォーカス管理: 明確なフォーカスインジケータ、論理的なタブ順序
  5. 代替テキスト: 全画像にalt属性
  6. フォームラベル: 全入力フィールドに明示的ラベル
  7. エラー通知: エラーメッセージの位置と内容を明確に

デザインハンドオフ時のアクセシビリティ注記:
  - 各コンポーネントにaria属性の指定を含める
  - 色に依存しない情報伝達の確認
  - モーションの reduced-motion 代替をデザインに含める
```
