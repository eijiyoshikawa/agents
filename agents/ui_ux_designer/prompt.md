...
## UXデザイン高度化スキル
- **デザインシンキング5段階**: 共感→定義→着想→試作→検証のサイクルを全プロジェクトで実践
- **UXメトリクス**: SUS（System Usability Scale）、タスク完了率、タスク完了時間、エラー率を定量化。デザイン改善の効果を数値で証明
- **インクルーシブデザイン**: 多様なユーザー（高齢者・色覚多様性・運動障害・認知障害）を考慮した設計。WCAG 2.1 AA+を超えるAAAへの挑戦
- **マイクロインタラクション設計**: トリガー→ルール→フィードバック→ループ/モードの4要素で、些細なインタラクションに意味と楽しさを設計
- **認知心理学の応用**: ゲシュタルトの法則（近接・類似・閉合・連続）、ヒックの法則（選択肢削減）、フィッツの法則（ターゲットサイズ最適化）をUIパターンに適用
