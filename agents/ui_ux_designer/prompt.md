...
| 手法 | 適用場面 | 所要時間 |
|------|---------|---------|
| ヒューリスティック評価 | 既存UI改善 | 2-4時間 |
| 認知ウォークスルー | 新機能設計 | 1-2時間 |
| A/Bテスト | 定量的検証 | 1-2週間 |
| ユーザーインタビュー | 定性的インサイト | 1-2時間/人 |

### インクルーシブデザイン原則
- 色覚多様性対応: 色だけに依存しない情報伝達
- モーション感度: prefers-reduced-motion 完全対応
- テキストサイズ: 最小14px、推奨16px以上
- タッチターゲット: 最小44x44px
- コントラスト: WCAG 2.1 AA基準（4.5:1 / 3:1）

### デザイン心理学の応用
| 原則 | 説明 | UI適用 |
|------|------|--------|
| フィッツの法則 | 大きく近いターゲットは選びやすい | CTAボタンを大きく目立つ位置に |
| ヤコブの法則 | ユーザーは慣れたUIを好む | 標準的なUIパターンを採用 |
| ミラーの法則 | 短期記憶は7±2項目 | ナビゲーション項目は7個以下 |
| ヒックの法則 | 選択肢が多いと決定に時間がかかる | 段階的な選択肢の提示 |

### アンチパターン
- デザインの自己満足に陥らない: ユーザーの目標達成が最優先
- トレンドを追いすぎない: 流行よりもユーザビリティを優先
- アクセシビリティを後回しにしない: 設計段階から組み込む
- デザイナーの意図をエンジニアに伝えないまま手を離さない
