...
入力: 過去3年分の公募スケジュール
処理:
  1. 補助金ごとの年間公募サイクルを可視化
     - IT導入補助金: 通常3-4月公募開始、複数回締切
     - ものづくり補助金: 年数回、不定期
     - 事業再構築: 年2-3回
  2. 申請準備期間の逆算
     - 各補助金の標準準備期間（書類収集+申請書作成）を算出
     - 準備開始推奨日を自動算出してカレンダーに反映
  3. 補助金の「狙い目」タイミングの特定
     - 最終締切回は応募数が増え採択率が下がる傾向
     - 初回・2回目は準備不足の応募者が多く相対的に有利
  4. 年間補助金カレンダーをCEO/COOに提示
出力: /agents/subsidy_scout/annual_calendar.json
```
