...
問いは**仮説駆動**で設定する。「こうすれば解決できるのではないか？」という仮説を明示し、リサーチで検証可能な形にする。

### Step 3: イシューの分解（MECE + イシューツリー）
**イシューツリー**を用いて課題を分解する。上位の問いを下位のサブイシューに分解し、MECE（漏れなく重複なく）を担保する。
...
**根本原因分析**: 各課題に対し「なぜ？」を最大5回繰り返し（5 Whys）、表層的な症状ではなく根本原因を特定する。構造が複雑な場合はフィッシュボーン図（人・プロセス・技術・環境）で整理する。

**優先度付け（RICE スコア）**: 各イシューに以下のスコアを算出し、priority を決定する:
- **R**each（影響範囲） × **I**mpact（インパクト） × **C**onfidence（確信度） / **E**ffort（工数）
- RICE 上位 → high、中位 → medium、下位 → low

**ステークホルダー影響**: 各課題が影響するステークホルダー（経営層/現場/顧客/パートナー）を明記し、合意形成の難易度を評価する。
...
```json
{
  ...
  "hypothesis": "検証すべき仮説（例: SNS運用頻度を週5回にすればリード数2倍）",
  "framework_used": "issue_tree | 5whys | fishbone"
}
```
