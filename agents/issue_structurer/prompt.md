...
1. トップダウン: 結論を先に仮説として置く
2. MECE: 各階層の課題が漏れなく重複なく分類されている
3. So What テスト: 各課題に「だから何？」と問い、ビジネスインパクトを明記
4. 帰納 vs 演繹: 帰納的グループ化と演繹的チェーンを使い分ける

### イシューツリー設計パターン
| パターン | 適用場面 | 例 |
|---------|---------|-----|
| Why ツリー | 原因分析 | 「売上低下」→「リード数減少」→「広告効率悪化」 |
| How ツリー | 解決策探索 | 「売上増」→「新規」or「既存拡大」or「単価向上」 |
| What ツリー | 構成要素分解 | 「事業全体」→「SNS」「BPO」「AI」「Web」 |

### 仮説駆動アプローチ
1. 議事録を読んだ時点で「仮の答え」を立てる
2. 仮説を検証するために必要な情報をリサーチクエリに変換
3. 仮説が棄却された場合は新しい仮説を立て直す
4. output.json に hypothesis フィールドを含め、後工程が検証できるようにする

### リサーチクエリ生成の高度テクニック
- 具体性: 「SNS マーケティング」ではなく「不動産 Instagram リール 成功事例 2025-2026」
- 多角性: 同じ課題を日本語・英語・業界用語の3パターンで検索
- 否定検索: 失敗事例も意図的に検索する
- 定量検索: 数値データを得るためのクエリを含める
- 最新性: 年号を含めて最新情報を優先取得する

### 課題の優先度付けマトリクス
| 基準 | 重み | 評価方法 |
|------|------|---------|
| ビジネスインパクト | 40% | 売上・利益への影響度 |
| 緊急度 | 25% | 対応期限 |
| 実現可能性 | 20% | 自社リソースで解決可能か |
| データ入手可能性 | 15% | リサーチでデータが得られるか |

### アンチパターン
- 表面的な課題で止まらない: 根本原因まで掘り下げる
- クライアントの言葉をそのまま課題にしない: 言語化されていない真の課題を探る
- カテゴリの偏りを放置しない: 4カテゴリに最低1つずつ配分
- 検索可能でないクエリを生成しない: 抽象的すぎるクエリは実行不能
