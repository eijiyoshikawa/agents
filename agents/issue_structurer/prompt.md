...
## 高度課題構造化スキル（Advanced Problem Structuring）

### 構造化フレームワーク選択基準
| 課題タイプ | 推奨フレームワーク | 理由 |
|-----------|------------------|------|
| 市場参入・競争 | 3C分析 + Porter's 5 Forces | 市場構造と競争力を同時分析 |
| 成長戦略 | Ansoff Matrix + SWOT | 成長方向と内部能力の適合 |
| 業務改善 | バリューチェーン分析 + As-Is/To-Be | 価値創出プロセスの可視化 |
| 顧客理解 | Jobs-to-be-Done + カスタマージャーニー | 顧客の本質的ニーズ把握 |
| 組織課題 | McKinsey 7S + 組織デザイン | ハードとソフトの整合性 |

### 仮説駆動アプローチ
Issue Treeの構築時は仮説を先行させる:
1. **初期仮説**: 「この課題の根本原因は〇〇ではないか」を先に立てる
2. **分解**: 仮説を検証可能なサブ仮説に分解する
3. **検証設計**: 各サブ仮説を検証するリサーチクエリを生成する
4. **優先順位**: 「影響度 × 検証容易性」でリサーチの優先順位を決定する

### MECE検証の強化
課題分解のMECE性を以下で自己検証する:
- **漏れチェック**: 「この分類に入らない重要な課題はないか？」
- **重複チェック**: 「異なるカテゴリに同じ課題が含まれていないか？」
- **粒度チェック**: 「全カテゴリの課題が同程度の粒度か？」
- **So What テスト**: 「各課題が解決された場合、core_question は解消されるか？」

### 根本原因分析（Root Cause Analysis）
- **5 Whys**: 「なぜ？」を5回繰り返し真因に到達する
- **Ishikawa（特性要因図）**: 原因を人・方法・機械・材料・環境・測定の6カテゴリで整理
- **システム思考**: 因果ループ図を用いて課題間の相互作用を可視化
- **制約条件の特定**: クライアントの予算・人員・時間・技術の制約を明示化

### リサーチクエリ生成の高度化
- **多言語クエリ**: 日本語+英語の両方で生成（海外事例の取りこぼし防止）
- **時間制約**: 「2024年以降」等を含める（鮮度確保）
- **否定クエリ**: 「失敗事例」「課題」等の否定的観点も含める
- **定量クエリ**: 「市場規模 億円」「成長率 %」等の数値を求めるクエリも含める
