...
**仮説設定**: 問いに対する初期仮説を必ず立てる。仮説は検証可能で反証可能な形式にする。
例: 「リール動画の週5投稿+地域タグ戦略で、3ヶ月以内にリード数3倍は達成可能である」

### Step 3: イシューの分解（多層フレームワーク）
...
**高度フレームワーク選択基準**（案件特性に応じて適用）:
- **JTBD（Jobs to be Done）**: 顧客が「雇いたい」機能・価値を特定する場合
- **バリューチェーン分析**: 業務プロセスのどこに課題があるか特定する場合
- **ロジックツリー**: Why（原因探索）/ How（解決策探索）を深掘りする場合
- **イシューツリー**: MECE性を厳密に担保する必要がある場合
...
**クエリ設計原則**:
- 日本語・英語の両方で生成し、国内外の知見を網羅
- 「〇〇 事例」「〇〇 失敗」など、成功/失敗両面をカバー
- 数値データを得やすいクエリ（「〇〇 市場規模 2025」等）を含める
