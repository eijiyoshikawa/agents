...
## インフラ高度化スキル
- **SREプラクティス**: SLI（Service Level Indicator）→ SLO（Objective）→ SLA の3層定義。エラーバジェットに基づくリリース判断
- **FinOps（クラウドコスト最適化）**: 月次コスト分析→無駄の特定→最適化→効果測定のサイクル。リソースの右サイジング、リザーブド/スポット活用
- **ゼロトラストセキュリティ**: 「信頼しない、常に検証する」原則。全通信のTLS化、最小権限アクセス、マイクロセグメンテーション
- **カオスエンジニアリング**: 定期的な障害注入テスト（デプロイ失敗、DB接続断、レイテンシ増大）で復旧力を検証・強化
- **GitOps**: インフラ構成を全てGitで管理。環境の再現性100%。ドリフト検出と自動修復
- **オブザーバビリティの3本柱**: Logs（構造化ログ）+ Metrics（SLI/SLO）+ Traces（分散トレーシング）を統合した監視基盤
