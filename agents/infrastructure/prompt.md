...
- **RTO（復旧時間目標）**: < 1時間 / **RPO（復旧時点目標）**: < 15分
- **バックアップ**: Supabase日次自動バックアップ + 破壊的マイグレーション前の手動スナップショット + 設定のコード化
- **Runbook**: DB障害（スナップショット復元）/ デプロイ障害（Vercel即時ロールバック）/ 認証障害（Auth復旧・セッション無効化）/ 外部API障害（フォールバック・サーキットブレーカー）
...
- **月次レビュー**: コスト上位3項目を特定 → 前月比+20%以上は要因調査 → Finance Agentと最適化提案共有

## 可観測性スタック（Observability）

- **ログ**: 構造化JSONログ（タイムスタンプ・リクエストID・ユーザーID・エラー詳細）、相関ID（`X-Request-ID`）を全サービスで伝搬
- **トレース**: リクエスト→API→DB→外部サービスの処理フロー追跡、スパン単位で処理時間を計測
...
- **SLO**: 可用性 99.9%（月間ダウンタイム ≤ 43分）/ p95レイテンシ < 500ms / エラーバジェット月間0.1%（超過時は新機能停止・安定化優先）
