...
### 5. 災害復旧計画（Disaster Recovery）
```
RTO（目標復旧時間）・RPO（目標復旧ポイント）の定義:
  | サービス層 | RTO | RPO |
  |-----------|------|------|
  | Webアプリ（Vercel） | 5分 | 0（Gitリポジトリが正） |
  | DB（Supabase） | 30分 | 1時間以内 |
  | 外部連携（Stripe等） | 依存先に準拠 | - |

復旧手順:
  1. Vercel障害: 別リージョンへの自動フェイルオーバー（Vercel側）
  2. Supabase障害: バックアップからのリストア手順を文書化
  3. 全面障害: メンテナンスページ表示 + 顧客通知（PR Agent連携）

バックアップ:
  - DB: Supabase自動バックアップ（日次）+ 手動エクスポート（週次）
  - コード: GitHubリポジトリ（複数ブランチ戦略で保護）
  - 環境変数: Vercel Environment Variables（ドキュメント化必須）
  - バックアップ復元テスト: 四半期に1回実施
```

### 6. キャパシティプランニング
```
月次で以下を評価:
  1. Vercel: ビルド時間・帯域幅・Edge Function実行回数のトレンド
  2. Supabase: DB容量・接続数・クエリパフォーマンスのトレンド
  3. 外部API: レートリミット消費率のトレンド
  4. 予測: 3ヶ月先のリソース需要を案件パイプラインから推算

スケーリングトリガー:
  - 使用量 > 70%: モニタリング強化、最適化検討開始
  - 使用量 > 85%: プラン変更 or スケーリング実行
  - 使用量 > 95%: 緊急対応、CEO/Finance に報告
```
