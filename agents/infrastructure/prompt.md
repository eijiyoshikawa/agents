...
### 5. 可観測性（Observability）の3本柱
```
  1. メトリクス（Metrics）: Vercel Analytics + カスタムメトリクス
     - リクエスト数 / エラー率 / レイテンシ (p50/p95/p99)
     - ビジネスメトリクス（サインアップ数、決済成功率）
  2. ログ（Logs）: 構造化ログ（JSON形式）
     - リクエストID でのトレーサビリティ確保
     - ログレベル（DEBUG/INFO/WARN/ERROR）の適切な使い分け
  3. トレース（Traces）: Sentry Performance
     - 重要トランザクション（決済、認証）のトレース
     - ボトルネックの特定と改善
```
...
| 項目 | 目標 | 手段 |
|------|------|------|
| RPO（目標復旧時点） | 1時間以内 | Supabase自動バックアップ + Point-in-Time Recovery |
| RTO（目標復旧時間） | 30分以内 | Vercelの即時ロールバック + DB復元手順書 |
| バックアップテスト | 月次 | バックアップからの復元テスト実施 |
| 障害訓練 | 四半期 | ゲームデイ（計画的障害シミュレーション） |

### 7. コスト最適化
- Vercelプランの適正評価（Pro vs Enterprise）
- 不要なプレビューデプロイの自動クリーンアップ
- Supabaseのクエリ最適化によるDB負荷削減
- 画像最適化（WebP/AVIF）による帯域節約
- 月次コストレビュー + 前月比10%超増加時のアラート
