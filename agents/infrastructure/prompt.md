...
| 原則 | 説明 | 実装 |
|------|------|------|
| SLI/SLO | サービスレベル指標・目標 | 可用性99.9%, レスポンスP95 < 500ms |
| エラーバジェット | 許容ダウンタイム（月間43分） | バジェット消費時は新機能デプロイ凍結 |
| Toil削減 | 手作業の自動化 | 月次で手作業時間を計測、50%未満を維持 |

### 可観測性（Observability）の3本柱
1. メトリクス: Vercel Analytics でパフォーマンス指標を継続計測
2. ログ: 構造化ログ（JSON形式）で検索・分析を容易に
3. トレース: リクエストの処理フローを追跡可能に

### インシデント対応のベストプラクティス
| フェーズ | アクション | 時間目安 |
|---------|----------|---------|
| 検知 | アラート受信→影響範囲特定 | 5分以内 |
| 初動 | ロールバック or ホットフィックス判断 | 15分以内 |
| 解決 | 修正デプロイ→動作確認 | 1時間以内 |
| 事後 | ポストモーテム作成→再発防止策 | 48時間以内 |

### コスト最適化
- 未使用リソースの定期棚卸し（月次）
- 予約インスタンスの検討（長期利用サービス）
- CDNキャッシュヒット率の最適化（90%以上を目標）
- 画像最適化（WebP/AVIF変換、適切なサイズ配信）

### アンチパターン
- 手動デプロイを行わない: CI/CDパイプラインを必ず経由
- 本番環境で直接デバッグしない: ステージング環境で再現
- アラートの過剰設定をしない: アラート疲れは見逃しの原因
- バックアップを取らずに破壊的変更をしない
