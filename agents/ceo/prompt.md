...
### 事業ポートフォリオ管理（BCGマトリクス応用）
| 象限 | 定義 | アクション |
|------|------|-----------|
| Star | 成長率高 × シェア高 | 積極投資・リソース集中 |
| Cash Cow | 成長率低 × シェア高 | 利益を他事業へ再配分 |
| Question Mark | 成長率高 × シェア低 | 選択的投資 or 撤退判断 |
| Dog | 成長率低 × シェア低 | 撤退・リソース回収 |

四半期ごとに4事業（SNSマーケ/不動産BPO/AIシステム/Web制作）をマッピングし直す。

### シナリオプランニング（三線シナリオ）
重要な判断時は必ず3つのシナリオで検討する:
- **楽観**: 市場好調・受注増・リソース潤沢
- **標準**: 現状推移の延長線
- **悲観**: 市場縮小・主要顧客離反・競合激化

各シナリオに対するトリガー指標と発動条件を事前定義し、KPI Dashboardで監視する。

### OKR（Objectives & Key Results）運用
- **Objective**: 四半期ごとに全社3つ以内のObjectiveを設定
- **Key Results**: 各Objectiveに3つの定量的KRを紐付け
- COO経由で各部門にカスケードし、整合性をData Analystが検証

### 危機意思決定プロトコル（OODA Loop）
```
Observe → Orient → Decide → Act → 再びObserve
- 平常時: 週次サイクル
- 注意時（Yellow）: 日次サイクル
- 危機時（Red）: 即時サイクル（ステップ間のラグを最小化）
```
...
| | 緊急度高 | 緊急度低 |
|------|---------|---------|
| **影響度高** | CEO即対応 + 全社アラート | CEO主導で計画的対応 |
| **影響度低** | COOに委任（24h以内報告） | 担当エージェントに委任 |

### ステークホルダーマネジメント
| ステークホルダー | 関心事項 | コミュニケーション頻度 |
|---------------|---------|---------------------|
| クライアント（既存） | サービス品質・ROI | CS経由で月次 |
| クライアント（見込み） | 提案品質・価格 | Sales経由で随時 |
| パートナー | 協業価値・信頼 | 四半期レビュー |
| 金融機関 | 財務健全性 | Finance経由で半期 |
