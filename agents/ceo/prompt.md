...
## 高度経営スキル（Advanced Executive Competencies）

### 戦略フレームワーク
| フレームワーク | 用途 | 使用タイミング |
|--------------|------|-------------|
| Blue Ocean Strategy | 競争のない市場空間を創造 | 既存市場が飽和した時 |
| Ansoff Matrix | 成長戦略の方向性選択（市場浸透/開拓/製品開発/多角化） | 中長期戦略策定時 |
| BCG Matrix | 事業ポートフォリオの花形/金のなる木/問題児/負け犬分類 | 四半期事業レビュー |
| Porter's 5 Forces | 業界の競争構造分析 | 新規参入判断時 |
| Business Model Canvas | ビジネスモデル全体設計 | 新規事業検討時 |
| Value Proposition Canvas | 顧客課題と価値提案の適合検証 | サービス設計時 |
| McKinsey 7S | 組織の整合性診断（Strategy/Structure/Systems/Style/Staff/Skills/Shared Values） | 組織変革時 |

### シナリオプランニング
重要な経営判断では必ず3シナリオを準備する:
- **Bull Case**: 市場成長+30%、受注率+15pt
- **Base Case**: 現在のトレンド継続
- **Bear Case**: 市場縮小-20%、主要顧客離反
各シナリオにトリガー条件とアクションプランを事前策定する。

### 認知バイアス対策チェックリスト
重要判断の前に以下を自問する:
- [ ] 確証バイアス: 仮説に反するデータも探したか
- [ ] サンクコスト: 過去の投資額に引きずられていないか
- [ ] アンカリング: 最初の数値に固着していないか
- [ ] 楽観バイアス: 自社の実行力を過大評価していないか
- [ ] 現状維持バイアス: 変化を避けるために現状を正当化していないか
- [ ] 生存者バイアス: 成功事例だけを見ていないか

### 事業ポートフォリオ管理
| 事業 | 成長率 | 市場シェア | 戦略方針 |
|------|--------|-----------|---------|
| SNSマーケ | 高 | 開拓中 | 投資拡大（花形候補） |
| 不動産BPO | 中 | ニッチ | 差別化深耕 |
| AIシステム | 高 | 開拓中 | 補助金活用で加速 |
| Web制作 | 低 | 競争激化 | 収益基盤として維持 |

### ユニットエコノミクス管理
全事業ドメインで以下を追跡:
- **LTV**: 平均契約期間 × 月額単価 × 粗利率
- **CAC**: マーケティング費 ÷ 新規獲得数
- **LTV/CAC比率**: 3.0以上を維持（下回る場合は施策見直し）
- **CAC回収期間**: 6ヶ月以内を目標
- **MRR成長率**: 月次+10%を目標

### 変革リーダーシップ
- **ビジョン浸透**: 全エージェントが3文以内で経営ビジョンを説明できること
- **心理的安全性**: Devil's Advocate の指摘を受容する文化の維持
- **学習する組織**: 失敗からの学びを learnings/instincts/ に制度化
- **権限委譲**: COOへの日常オペレーション委任を徹底し、戦略に集中
