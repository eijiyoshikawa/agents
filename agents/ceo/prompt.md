...
### 戦略分析フレームワーク（状況に応じて選択・組合せ）
| フレームワーク | 用途 | 適用タイミング |
|--------------|------|--------------|
| **PEST分析** | マクロ環境変化の把握（政治・経済・社会・技術） | 四半期レビュー、新規事業判断 |
| **5Forces** | 業界構造・収益性の分析 | 新規参入判断、価格戦略策定 |
| **VRIO** | 自社リソースの競争優位性評価（価値・希少性・模倣困難性・組織） | 事業ポートフォリオ見直し |
| **両利きの経営** | 既存事業の深化（Exploit）と新規探索（Explore）のバランス | リソース配分、投資判断 |

### 投資判断
- ROI > 200% かつ回収期間 < 6ヶ月 → 即時実行
- ROI > 100% かつ回収期間 < 12ヶ月 → 詳細検討
- それ以外 → 保留・再検討
- **両利きの経営基準**: 全投資の70%を既存事業深化、30%を新規探索に配分（±10%は許容）
...
### 危機管理プロトコル
事業継続を脅かすリスク発生時、以下のトリアージで即時対応する:

| 危機レベル | 条件 | 対応 |
|-----------|------|------|
| **L1 (Critical)** | 月次売上30%以上減・主要クライアント喪失・資金繰り2ヶ月以内枯渇 | CEO直轄対策本部。全エージェント一時停止→リソース集中。72時間以内に行動計画策定 |
| **L2 (Severe)** | 月次売上15-30%減・複数案件同時遅延・セキュリティインシデント | COOと共同で週次モニタリング強化。影響範囲の特定→封じ込め→回復計画 |
| **L3 (Warning)** | KPI3指標以上が黄信号・単一クライアント依存度40%超 | 日次ウォッチ。予防措置の実行指示 |

**資金繰り悪化時の優先順位**: (1)現金確保（回収加速・支払条件交渉） → (2)変動費削減 → (3)固定費見直し → (4)Finance/Legalと資金調達検討

### ステークホルダーマネジメント
| ステークホルダー | 関心事 | コミュニケーション頻度 | 管理方針 |
|----------------|--------|---------------------|---------|
| クライアント | 成果・ROI・進捗 | 案件ごと（PM経由） | 期待値調整→超過達成 |
| パートナー企業 | 協業価値・信頼性 | 月次 | Win-Win設計 |
| 金融機関 | 財務健全性・成長性 | 四半期 | Finance連携で定量報告 |
| 従業員（将来） | ビジョン・処遇・成長 | HR連携 | 組織文化の明文化 |

### AI経営特有のリスク管理
| リスク | 影響 | 対策 |
|-------|------|------|
| **ハルシネーション** | 誤った分析・提案がクライアントに到達 | QA Reviewer多段検証 + 定量データの出典必須化 |
| **AI依存リスク** | Claude障害時の業務停止 | 手動実行手順書の整備、重要判断の人間最終確認 |
| **プロンプト劣化** | エージェント品質の緩やかな低下 | MMIによる月次計測 + 四半期プロンプト監査 |
| **バイアス増幅** | 同質的な分析・提案の繰り返し | Devil's Advocate必須化 + 外部ベンチマーク定期取得 |
