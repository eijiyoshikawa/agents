...
### 3. リード獲得・育成（ファネル×ABM統合）
```
処理:
  1. リードソースの管理・最適化（チャネル別CPA/LTVで評価）
  2. LP/フォームの改善提案（A/Bテスト設計含む）
  3. ABM（Account Based Marketing）:
     - Tier1ターゲット企業リストの定義（Sales Agent と連携）
     - 企業別カスタムコンテンツ・広告の設計
     - 意思決定者へのダイレクトアプローチ計画
  4. リードナーチャリング施策（スコアベース自動化）
     - メールシーケンス設計（行動トリガー型）
     - リターゲティング広告（ファネルステージ別クリエイティブ）
     - セミナー招待（テーマをペインポイント別に設計）
     - LINE公式アカウントのステップ配信
  5. リードスコアリングモデルの運用・改善
     - 行動スコア（ページ閲覧、資料DL、セミナー参加）
     - 属性スコア（業界、規模、役職）
     - 閾値超えで自動的にSales Agentへ引き渡し
  6. MQL→SQL転換率の改善（月次で転換ボトルネック分析）
出力: /agents/marketing/lead_report_{month}.json
```

### 3b. マルチタッチアトリビューション分析
- リード→受注に至るまでの全タッチポイントを記録
- ファーストタッチ / ラストタッチ / 線形 / 時間減衰の各モデルで貢献度を評価
- チャネル別の真のROIを算出し、予算配分の最適化に活用
