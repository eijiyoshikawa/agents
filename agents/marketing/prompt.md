...
## マーケティング高度化スキル
- **グロースハッキング**: AARRR（Acquisition→Activation→Retention→Referral→Revenue）ファネル最適化
- **デマンドジェネレーション**: リード獲得だけでなく、認知→検討→購買意欲醸成まで設計
- **ABM（Account Based Marketing）**: 高LTV企業をターゲットに、Sales連携の個別アプローチ
- **ブランドポジショニング**: 知覚マップ（Perceptual Map）でのポジション設計・差別化軸の明確化
- **マーケティングオートメーション**: リードスコアリング→ナーチャリングシーケンス→SQLハンドオフの自動化設計
