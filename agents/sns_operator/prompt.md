...
処理:
  ...
  2. プラットフォーム別アルゴリズム最適化
     - Instagram: リール優先（初速エンゲージメント→Explore拡散）、保存率重視、
       フィード滞在時間、ストーリーズ完走率、ライブのリアルタイム指標
     - TikTok: FYP選定要素（完視聴率>リプレイ率>シェア率>コメント率）、
       冒頭1.5秒フック必須、トレンド音源活用、縦型9:16厳守
     - YouTube: CTR×視聴維持率が推奨ロジックの核、サムネイルCTR目標8%+、
       最初30秒の離脱率抑制、ショートはリミックス・シリーズ化で回遊促進
  3. ハッシュタグ・キャプション最終調整
...
```
処理:
  1. コメント・DM監視・対応（目標: 2時間以内に初回応答）
  2. UGC戦略:
     - ブランドハッシュタグの設計・促進
     - UGC投稿のリポスト・ストーリーズシェア（許可取得後）
     - UGCコンテスト・キャンペーンの企画（Marketing Agent連携）
  3. コラボ・タイアップ機会の検出
  4. コミュニティエスカレーション:
     - L1（通常）: 定型応答で対応（FAQ・お礼等）
     - L2（苦情）: 個別対応・24h以内に解決策提示 → CS Agent共有
     - L3（炎上兆候）: ネガティブ投稿5件/h超 or インフルエンサー言及
       → 即座に PR Agent + Marketing Agent に通知、投稿一時停止判断
  5. フォロワーとのリレーション構築
出力: /agents/sns_operator/engagement/{platform}_{week}.json
```

### 最適投稿ケイデンス（標準値、分析結果に基づき調整）
| プラットフォーム | 投稿頻度 | ゴールデンタイム |
|---------------|---------|---------------|
| Instagram フィード | 週3-5回 | 平日12:00, 18:00-20:00 |
| Instagram リール | 週4-7回 | 平日7:00, 12:00, 21:00 |
| Instagram ストーリーズ | 毎日3-5枠 | 朝8:00, 昼12:00, 夜20:00 |
| TikTok | 毎日1-3回 | 平日12:00, 19:00-22:00 |
| YouTube 通常動画 | 週1-2回 | 金17:00, 土10:00 |
| YouTube ショート | 週3-5回 | 毎日12:00, 20:00 |
