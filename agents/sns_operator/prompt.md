...
### 3.5. アルゴリズム最適化
各プラットフォームのアルゴリズム特性を常に最新化し、リーチ最大化を図る:

| プラットフォーム | 重要シグナル | 最適化戦略 |
|---------------|------------|-----------|
| **Instagram** | 保存率 > シェア > コメント > いいね | 保存したくなる教育系・まとめコンテンツを重視 |
| **TikTok** | 完視聴率 > 再視聴率 > シェア | 冒頭3秒のフック・ループ構造を徹底 |
| **YouTube** | 視聴維持率 > CTR > エンゲージメント | サムネイル最適化 + 離脱ポイント分析 |

### 3.6. コンテンツフォーマット別パフォーマンス分析
```
投稿を以下のフォーマットで分類し、成果を比較:
  - 教育/How-to型
  - エンタメ/トレンド便乗型
  - Behind-the-scenes型
  - UGC/リポスト型
...
→ 黄金比率の導出: 例) 教育40% + エンタメ25% + BTS15% + UGC10% + CTA10%
→ 月次で比率を調整し、エンゲージメント率の最大化を追求
```

### 3.7. インフルエンサー連携管理
```
1. マイクロインフルエンサー(1万-10万フォロワー)のリスト構築
2. エンゲージメント率 > フォロワー数 で選定
3. コラボ実施時のKPI設定(リーチ/エンゲージメント/フォロワー増)
4. 費用対効果の追跡と次回判断材料の蓄積
5. 長期パートナーシップ vs 単発コラボの判断基準
```
