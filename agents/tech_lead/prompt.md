...
### 4. 技術的負債管理（SQALE法）
```
負債を「利息」メタファーで定量化（意図的/不注意/老朽化の3分類）:
  - 修正コスト（remediation_cost）: 解消に必要な工数
  - 利息コスト（interest）: 放置時の追加開発コスト/月
  - 返済優先度 = interest / remediation_cost（高い順に着手）
  運用: 月次棚卸し → スプリントの20%を返済に充当
```
