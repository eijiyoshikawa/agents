...
### テクノロジーレーダー（四半期更新）
技術選定の透明性を高めるため、以下の4象限で技術を分類・管理する:

| リング | 定義 | アクション |
|--------|------|-----------|
| **Adopt** | 本番採用を推奨。チーム全体で使用 | 新規プロジェクトのデフォルト |
| **Trial** | 小規模プロジェクトで試行中。良い結果なら昇格 | PoCで検証後にAdopt判定 |
| **Assess** | 注目しているが未検証。調査段階 | 月次で動向チェック |
| **Hold** | 非推奨。新規採用禁止、既存は段階的に移行 | 代替技術への移行計画策定 |

```
現在のレーダー例:
  Adopt: Next.js 14+, TypeScript, Tailwind CSS, Supabase, Vercel, Claude API
  Trial: Drizzle ORM, Turborepo, tRPC
  Assess: Bun, Deno, Edge Functions
  Hold: Express.js (単独), styled-components, REST API (GraphQL移行中の場合)
```

### 技術的負債の定量管理
```
技術的負債の分類と管理:
  1. 計画的負債（Deliberate）: 意図的に受け入れた簡易実装
     → 返済期限を設定し、バックログに追加
  2. 不注意な負債（Inadvertent）: 知識不足による非最適実装
     → コードレビューで検出、即時修正
  3. 環境負債（Environmental）: 依存関係の陳腐化
     → Dependabot + 月次アップデート計画

負債インデックス = Σ(修正工数 × 影響範囲 × 放置リスク)
  → 閾値超過時にCEOにリファクタリングスプリントを提案
```

### Developer Experience (DX) 指標
開発チームの生産性を測定・改善する:
| 指標 | 計測方法 | 目標 |
|------|---------|------|
| 環境構築時間 | clone → 初回起動までの時間 | 10分以内 |
| ビルド時間 | dev/prodのビルド完了時間 | dev 5秒 / prod 2分以内 |
| テスト実行時間 | CI上の全テスト実行時間 | 5分以内 |
| PR→マージ時間 | PR作成→レビュー承認→マージ | 24時間以内 |
| ホットリロード速度 | ファイル変更→ブラウザ反映 | 1秒以内 |
