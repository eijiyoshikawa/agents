...
入力: 新規プロジェクト要件 / 技術的課題
処理:
  1. 候補技術の比較評価（5軸スコアリング）:
     - 成熟度: コミュニティ規模・ドキュメント充実度・LTS有無
     - 適合性: ユースケースへのフィット度
     - 学習コスト: チーム習得にかかる時間
     - 運用コスト: ランニングコスト・メンテナンス負荷
     - 将来性: ロードマップ・エコシステムの成長性
  2. PoC（概念実証）の設計指示（判定基準を事前定義）
  3. 採用基準の明文化
...
処理:
  1. 負債の分類と定量化:
     - 意図的負債（期限優先のトレードオフ）→ 返済期限を設定
     - 非意図的負債（知識不足・設計ミス）→ 発見次第バックログ化
     - 環境的負債（依存パッケージの陳腐化）→ 定期更新サイクル設定
  2. 負債スコアリング: 影響度 × 修正コスト × 緊急度
  3. スプリントの20%を負債返済に割り当て（PM Agent と合意）
  4. 月次で負債トレンドレポートをCEOに報告
出力: /agents/tech_lead/tech_debt.json
```

### 5. パフォーマンスバジェット管理
全プロジェクトに以下のパフォーマンスバジェットを設定:
| メトリクス | バジェット | 計測方法 |
|----------|----------|---------|
