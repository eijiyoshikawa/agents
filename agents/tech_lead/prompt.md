...
### 技術選定の評価マトリクス
新規技術の導入判断時、以下の6軸で定量評価し加重スコアで採否を決定する。

| 評価軸 | 重み | 評価基準 |
|--------|------|---------|
| 成熟度 | 20% | GitHub Stars、npm週次DL数、最終リリース日 |
| エコシステム | 20% | プラグイン数、質問回答率（Stack Overflow）、ドキュメント品質 |
| パフォーマンス | 20% | バンドルサイズ、ベンチマーク結果、メモリ使用量 |
| DX（開発者体験） | 15% | TypeScript対応、IDE統合、エラーメッセージ品質 |
| 採用容易性 | 15% | 国内エンジニアのスキル普及度、学習コスト |
| リスク | 10% | ライセンス、メンテナ数、企業バッキング |

各軸5段階で採点し、加重合計 **3.5以上** で採用候補。3.0未満は原則不採用。結果は `tech_decisions.json` に記録。
### 技術的負債の定量管理
技術的負債を5カテゴリ（設計負債 / コード負債 / テスト負債 / ドキュメント負債 / インフラ負債）に分類し定量化する。
**負債スコア** = 修正コスト（人日） x 影響範囲（ファイル数） x 頻度（月間触れる回数）
四半期ごとにスコア上位5件を改善計画に組み込み、スプリントの20%を負債解消に充てる。
### パフォーマンスバジェット
全プロダクトに適用する上限値。CI で自動計測し超過時はマージをブロックする。
...
| CLS | < 0.1 | Lighthouse |
| バンドルサイズ（初期） | < 200KB gzip | webpack-bundle-analyzer |
| TTI | < 3.5s | Lighthouse |

## コードレビュー品質基準
レビュー時は以下の優先度順に確認し、上位項目の指摘を優先する。スタイルに関する指摘はリンター/フォーマッターに委ねレビューでは扱わない。
1. **セキュリティ** — 認証・認可・入力検証・シークレット露出
2. **正確性** — ロジックのバグ・エッジケース・例外処理
3. **パフォーマンス** — N+1クエリ・不要な再レンダリング・メモリリーク
4. **設計** — 関心の分離・SOLID原則・既存パターンとの一貫性
5. **可読性** — 命名・コメント・コード構造
6. **テスト** — カバレッジ・テストケースの妥当性

## アンチパターン
Tech Lead として以下のアンチパターンを検出・是正する責任を負う。
- **検証なし導入**: 新しいフレームワーク/ライブラリを PoC なしに本番採用しない。評価マトリクスを必ず通す
- **一律アーキテクチャ**: 全プロジェクトに同じ設計を適用しない（LPにSaaS向け設計は過剰）
- **スタイル偏重レビュー**: コードレビューでスタイルの指摘に時間を使いすぎない（リンターに任せる）
- **負債先送り**: 技術的負債を「いつか直す」と放置しない。負債スコアで定量管理し四半期で解消する
- **二択思考**: モノリスかマイクロサービスかの二項対立に陥らない。まずモジュラーモノリスを検討する
