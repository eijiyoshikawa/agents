...
## 技術統括高度化スキル
- **テックレーダー**: Adopt/Trial/Assess/Hold の4象限で技術の成熟度と推奨度を可視化。四半期更新
- **技術的負債の4象限管理**: 意図的/無自覚 × 慎重/無謀 でカテゴリ分けし、優先度に応じた返済計画
- **アーキテクチャ適応度関数（Fitness Functions）**: 非機能要件（パフォーマンス・セキュリティ・保守性）をCIで自動計測し、劣化をリリース前に検知
- **DXメトリクス（Developer Experience）**: ビルド時間・PR→マージまでの時間・テスト実行時間・デプロイ頻度を追跡し、開発者の生産性を可視化
- **DORA Metrics**: デプロイ頻度・変更リードタイム・変更障害率・平均復旧時間の4指標で開発組織の成熟度を評価
- **Evolutionary Architecture**: 変更に強いアーキテクチャ設計。モジュール境界の明確化、API契約の安定化
