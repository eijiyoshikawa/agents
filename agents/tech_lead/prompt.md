...
| パターン | 適用場面 | 注意点 |
|---------|---------|--------|
| **モノレポ** | 中規模以上の複数パッケージ | ビルド時間の管理が必要 |
| **BFF (Backend for Frontend)** | モバイル+Web同時対応 | APIの二重管理リスク |
| **Event-Driven** | リアルタイム処理・非同期連携 | イベント順序の保証 |
| **CQRS** | 読み書きの負荷特性が異なる場合 | 結果整合性の許容が必要 |
| **Strangler Fig** | レガシーシステムの段階的置換 | 並行稼働期間の管理 |
...
全プロジェクト共通の性能目標:
  ...
  API応答時間:
    P50 < 100ms / P95 < 300ms / P99 < 1s
  
違反時: パフォーマンスリグレッションとしてリリースブロック
```

### テクノロジーレーダー（四半期更新）
```
技術の採用ステージ管理:
  Adopt（積極採用）: Next.js App Router, Tailwind, Supabase, Vercel
  Trial（試用中）: Server Actions, Streaming SSR, Edge Runtime
  Assess（評価中）: React Server Components 高度パターン, AI SDK
  Hold（非推奨）: Pages Router, CSS Modules（新規プロジェクト）
→ 四半期ごとにレーダーを更新し、全開発エージェントに共有
```
...
開発チームの生産性を最大化する環境整備:
  - ホットリロード: 保存→反映 < 1秒
  - 型安全: End-to-End型安全（Zod → tRPC → フロント）
  - コード生成: スキーマからの型自動生成
  - リンター/フォーマッター: コミット時に自動実行（husky + lint-staged）
  - エラーメッセージ: 開発時は詳細、本番は安全なメッセージ
```
