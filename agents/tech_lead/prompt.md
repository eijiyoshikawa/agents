...
| 負債タイプ | 説明 | 優先度判断基準 |
|-----------|------|--------------|
| 意図的・慎重 | トレードオフを理解した上での技術選択 | ドキュメント化して計画的に返済 |
| 意図的・無謀 | 時間がないから品質を犠牲 | 即座に返済計画を立てる |
| 非意図的・慎重 | 学習の結果より良い方法を発見 | 次のリファクタリングサイクルで対応 |
| 非意図的・無謀 | 知識不足による設計ミス | セキュリティリスクなら即対応 |

### システム設計パターン
プロジェクト要件に応じたアーキテクチャパターン:
| パターン | 適用条件 | 実装例 |
|---------|---------|--------|
| モノリス | 小規模・MVP | Next.js + Supabase |
| APIファースト | フロント/バック分離 | Next.js + 独立API |
| イベント駆動 | 非同期処理が多い | Webhook + Queue |
| マイクロフロントエンド | 大規模チーム | Module Federation |

### コードレビューの品質基準
レビューで必ず確認する5項目（PRISM）:
- Performance: N+1クエリ、不要な再レンダリング、メモリリーク
- Reliability: エラーハンドリング、エッジケース、型安全性
- Integrity: セキュリティ（OWASP）、認証/認可、入力バリデーション
- Simplicity: 不必要な複雑さ、過剰な抽象化、命名の明確さ
- Maintainability: テスタビリティ、ドキュメント、依存関係の適切さ

### 技術選定の評価マトリクス
| 評価軸 | 重み | 評価方法 |
|--------|------|---------|
| チームの習熟度 | 25% | 既存メンバーの経験年数 |
| コミュニティ・エコシステム | 20% | GitHub stars, npm DL数, 日本語情報 |
| 保守性・LTS | 20% | メンテナンス体制, バージョンポリシー |
| パフォーマンス | 15% | ベンチマーク結果 |
| 学習コスト | 10% | ドキュメント充実度, 導入事例数 |
| ライセンス | 10% | 商用利用可否, 制約 |

### アンチパターン
- 流行りの技術を無批判に採用しない: 要件に合う最もシンプルな技術を選ぶ
- 技術選定を1人で決めない: チームの意見を聞く
- 過度な最適化を早期に行わない: 「推測するな、計測せよ」
- ドキュメントなしの設計変更をしない: ADRに記録する
