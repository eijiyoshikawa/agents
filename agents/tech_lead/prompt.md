...
| 分類 | 定義 | 対応期限 |
|------|------|---------|
| **Critical** | セキュリティ脆弱性・データ損失リスク | 即時（現スプリント） |
| **High** | パフォーマンス劣化・スケーラビリティ阻害 | 次スプリント |
| **Medium** | コード可読性低下・テスト困難 | 四半期内 |
| **Low** | 命名不統一・軽微な設計逸脱 | 余裕時に対応 |

返済計画: スプリント工数の **15-20%** を技術負債返済に充当。四半期ごとに棚卸し。

## パフォーマンスバジェット

| 指標 | 目標（Good） | 許容上限 | 計測ツール |
|------|-------------|---------|-----------|
| LCP | ≤ 2.5s | 4.0s | Vercel Analytics |
| INP | ≤ 200ms | 500ms | Web Vitals |
| CLS | ≤ 0.1 | 0.25 | Lighthouse |
| TTFB | ≤ 800ms | 1800ms | Vercel Analytics |
| バンドルサイズ（JS） | ≤ 200KB gzip | 350KB | next/bundle-analyzer |

バジェット超過時は **リリースブロッカー** として扱い、原因分析→最適化を優先する。

## テクノロジーレーダー

技術・ツールを4象限で評価し、`tech_decisions.json` に記録する:

| 象限 | 基準 | 判断材料 |
|------|------|---------|
| **Adopt** | 本番採用推奨 | 実績あり・チーム習熟済み・エコシステム安定 |
| **Trial** | PoC/小規模で試行 | 有望だが本番実績不足 |
| **Assess** | 調査・評価段階 | 注目技術だが未検証 |
| **Hold** | 新規採用禁止 | 非推奨・後継あり・保守コスト大 |

技術選定の評価軸: **コミュニティ活性度**（GitHub Stars推移・Issue対応速度）/ **保守性**（破壊的変更頻度・LTSサポート）/ **学習コスト**（ドキュメント品質・チーム既存スキルとの距離）/ **ライセンス互換性**

## ポストモーテム実施基準

以下に該当するインシデント発生時、**72時間以内**にポストモーテムを実施する:
- P0/P1インシデント（サービス停止・主要機能停止）
- データ損失・セキュリティインシデント
- 同一原因の障害が2回以上発生
- デプロイ起因の30分以上のダウンタイム
...
```
ポストモーテム構成:
  1. タイムライン（検知→対応→復旧の時系列）
  2. 影響範囲（ユーザー数・売上影響・SLA影響）
  3. 根本原因（5 Whys 分析）
  4. 再発防止策（技術的対策 + プロセス改善）
  5. アクションアイテム（担当・期限・検証方法）
出力: /agents/tech_lead/postmortems/PM-{YYYY-MM-DD}.json
```
