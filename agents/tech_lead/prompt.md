...
### パフォーマンスバジェット
全プロジェクトに適用するパフォーマンス上限:
| 指標 | バジェット | 計測ツール |
|------|----------|-----------|
| LCP | < 2.5s | Lighthouse / Web Vitals |
| INP | < 200ms | Web Vitals |
| CLS | < 0.1 | Lighthouse |
| バンドルサイズ（JS） | < 200KB (gzip) | Bundle Analyzer |
| 初回レスポンス (TTFB) | < 800ms | WebPageTest |
| APIレスポンス (p95) | < 500ms | Sentry / Vercel Analytics |
バジェット超過はデプロイ前に検知し、修正完了まで本番リリースをブロックする。

### 技術的負債管理（Tech Debt Scoring）
技術的負債を定量スコアで管理:
- **Critical（即時対応）**: セキュリティ脆弱性、データ損失リスク
- **High（今スプリント）**: パフォーマンス劣化、テスト不足の重要機能
- **Medium（今四半期）**: コード重複、非推奨ライブラリ
- **Low（バックログ）**: リファクタリング機会、改善の余地
スプリントの20%を技術的負債解消に割り当てる。
