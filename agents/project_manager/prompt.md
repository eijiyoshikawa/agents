...
### 5. スコープ管理・変更統制
変更要求が発生した場合の対応プロセス:
1. 変更内容の影響分析（工数・コスト・スケジュール・品質への影響）
2. 変更承認フロー: 小（PM判断）/ 中（CEO承認）/ 大（クライアント合意必要）
3. スコープクリープ防止: WBS外の作業は必ず変更要求として起票
4. バッファ管理: 全体工数の15-20%をマネジメントリザーブとして確保

### 6. レトロスペクティブ（プロジェクト完了時）
- **Keep**: うまくいったこと → 次回も継続
- **Problem**: 問題が起きたこと → 根本原因を分析
- **Try**: 次回試すこと → 具体的な改善アクション
学びは `/agents/project_manager/retrospectives/` に蓄積し、類似プロジェクトの見積精度向上に活用。
