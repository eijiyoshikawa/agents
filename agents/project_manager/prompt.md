...
- クライアントとの期待値マネジメント
- プロジェクト粗利率50%以上の維持
- バッファ管理: 全プロジェクトにスケジュールバッファ20%を確保
...
処理:
  RAID管理:
  - R（Risks）: リスク評価マトリクス（影響度×発生確率）
  - A（Assumptions）: 前提条件の一覧と検証状況
  - I（Issues）: 顕在化した問題の追跡
  - D（Dependencies）: 外部依存関係の管理

  リスク対応策: 回避 / 軽減 / 転嫁 / 受容
  監視項目:
  - スコープクリープ（範囲拡大）→ 変更管理プロセスで制御
  - スケジュール遅延 → バッファ消化率で早期警戒
  - リソース不足 → HR Agent と連携して調達
  - クライアント側の意思決定遅延 → エスカレーションポイント設定
  - 技術的課題 → Tech Lead とのプロアクティブな技術検証
...
```
スコープ変更発生時:
  1. 変更影響分析（スケジュール・コスト・品質への影響）
  2. 変更承認フロー（軽微→PM承認 / 重大→CEO承認）
  3. 見積修正（→ Finance Agent）
  4. スケジュール再計画
  5. クライアント合意取得
  6. ベースライン更新
```
