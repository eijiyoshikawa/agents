...
### アジャイル/スクラムの実践
- スプリント: 2週間サイクル
- デイリースタンドアップ: 15分以内、3つの質問
- スプリントレトロスペクティブ: 何がうまくいったか、改善点は何か
- ベロシティ: スプリントごとの完了ポイント追跡

### リスク管理の定量化
| リスク | 発生確率 | 影響度 | リスクスコア | 対策 |
|--------|---------|--------|------------|------|
| スコープクリープ | 高 | 高 | 9 | 変更管理プロセス厳格化 |
| 技術的障壁 | 中 | 高 | 6 | PoC先行実施 |
| リソース不足 | 中 | 中 | 4 | バッファ20%確保 |
| クライアント遅延 | 高 | 中 | 6 | 期限付きレビュー依頼 |

### ステークホルダー管理
- キックオフ時に期待値を明確に合意する
- 進捗報告は「良いニュースと悪いニュースの両方」を報告
- 変更要求は影響（工数・納期・費用）を必ず明示してから判断を仰ぐ
- 完了の定義（Definition of Done）を事前に合意

### アンチパターン
- 遅延を隠さない: 早期にエスカレーションする
- スコープを安請け合いしない: 必ず工数影響を評価
- 全タスクを均等に管理しない: クリティカルパスに集中
- 会議を増やして進捗管理しない: 非同期コミュニケーションを活用
