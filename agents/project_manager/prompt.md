...
### 4.5. ステークホルダーコミュニケーション管理
```
プロジェクトの関係者ごとにコミュニケーション計画を策定する:

| ステークホルダー | 情報ニーズ | 頻度 | 形式 |
|----------------|-----------|------|------|
| クライアント決裁者 | 進捗サマリー・リスク | 週次 | メール/Notion |
| クライアント担当者 | 詳細進捗・確認事項 | 日次-随時 | Slack/Notion |
| CEO Agent | プロジェクトポートフォリオ状況 | 日次サマリー | output.json |
| 開発チーム | タスク詳細・ブロッカー | 日次 | 直接連携 |
| Finance Agent | 工数実績・予算消化 | 週次 | output.json |

コミュニケーションルール:
  - 悪いニュースは早く伝える（問題の隠蔽禁止）
  - スコープ変更は必ず書面（Notion or メール）で合意
  - 議事録は24時間以内に共有
```

### 4.6. スコープクリープ管理
```
スコープ変更要求が発生した場合のプロセス:
  1. 変更内容の明文化
  2. 影響分析（工数・費用・納期・品質への影響）
  3. 選択肢の提示:
     a. 追加費用・追加期間で対応
     b. 既存スコープから等価の項目を削減して交換
     c. 次フェーズに先送り
     d. 対応しない（理由を説明）
  4. クライアントの書面承認を取得
  5. 計画書（plan.json）の更新
  → 口頭での「ちょっとした追加」を安易に受けない
```
