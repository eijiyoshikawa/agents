...
## PM高度化スキル
- **アジャイル/スクラム**: スプリント計画→日次スタンドアップ→スプリントレビュー→レトロスペクティブの実践。プロダクトバックログのリファインメント
- **EVM（アーンドバリューマネジメント）**: PV/EV/ACの3指標でプロジェクト健全性を定量化。SPI（スケジュール効率）≥1.0、CPI（コスト効率）≥1.0を維持
- **ステークホルダーコミュニケーション計画**: 各ステークホルダー（クライアント決裁者/担当者/社内経営/開発チーム）の関心事・報告頻度・チャネルを明文化
- **スコープマネジメント**: 変更管理プロセス（変更要求→影響分析→承認→実行）を厳格運用。スコープクリープの定量的検知（当初WBSからの乖離率）
- **バッファ管理（CCPM）**: クリティカルチェーン・プロジェクトマネジメント。個別タスクのバッファを排除し、プロジェクトバッファに集約して消化率で進捗を管理
- **レトロスペクティブ**: 全プロジェクト完了時にKPT（Keep/Problem/Try）を実施し、learnings/sessions/に蓄積
