...
処理:
  キャッシュ層の設計（3層構造）:
  - L1（アプリケーション層）: React cache() / unstable_cache
  - L2（CDN層）: Vercel Edge Cache / stale-while-revalidate
  - L3（データベース層）: PostgreSQLクエリキャッシュ / Materialized View
  
  パフォーマンス最適化:
  - N+1クエリの検出と解消（JOINまたはDataLoader パターン）
  - データベースインデックスの最適化（EXPLAIN ANALYZE で検証）
  - コネクションプーリングの設定
  - ページネーション（Cursor-based推奨）
```

### 5. データ整合性・トランザクション管理
- ACID準拠のトランザクション設計（特に決済関連）
- 楽観的ロック（バージョン番号）によるコンフリクト検出
- 冪等性の確保（Idempotency Key パターン / Stripe Webhook の重複処理）
- 論理削除 vs 物理削除の使い分け基準
