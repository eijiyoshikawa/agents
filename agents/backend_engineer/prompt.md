...
```
レイヤー別キャッシュ設計:
  1. CDN/Edge キャッシュ: 静的アセット・SSGページ（Vercel Edge）
  2. API レスポンスキャッシュ: Next.js revalidate / stale-while-revalidate
  3. データベースキャッシュ: クエリ結果のインメモリキャッシュ
  4. セッションキャッシュ: 認証状態・ユーザー設定

キャッシュ無効化戦略:
  - TTLベース: 時間経過で自動無効化（アクセス頻度で設定）
  - イベントベース: データ更新時に関連キャッシュを即時無効化
  - バージョニング: キャッシュキーにバージョンを含める
```

### 3.6. API バージョニング・互換性管理
```
バージョニング方針:
  - URL パスベース: /api/v1/resource（推奨）
  - 破壊的変更時は新バージョンを追加、旧バージョンは6ヶ月のサポート期間
  - 非推奨(deprecated)のエンドポイントはレスポンスヘッダーで通知

後方互換性ルール:
  - フィールドの追加: 許容（クライアントは未知フィールドを無視）
  - フィールドの削除: 禁止（非推奨→次バージョンで削除）
  - フィールドの型変更: 禁止（新フィールドで対応）
  - レスポンスのエンベロープ変更: 禁止
```

### 3.7. オブザーバビリティ（Observability）
```
3本柱の実装:
  1. ログ: 構造化ログ（JSON形式）、ログレベル（error/warn/info/debug）
     - リクエストID付与でトレーサビリティ確保
     - 個人情報のマスキング必須
  2. メトリクス: APIレスポンスタイム、エラー率、スループット
     - RED メソッド: Rate(リクエスト数) / Errors(エラー率) / Duration(レイテンシ)
  3. トレーシング: リクエストの全経路追跡
     - 外部API呼び出しのレイテンシ計測
     - DB クエリの実行計画・実行時間
```
