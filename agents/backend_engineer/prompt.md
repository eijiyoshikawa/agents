...
### API設計のベストプラクティス
- エンドポイント命名: 名詞複数形（/api/users, /api/projects）
- HTTPメソッドの正しい使い分け: GET=取得, POST=作成, PUT=全更新, PATCH=部分更新, DELETE=削除
- ページネーション: cursor-based推奨（offset-basedは大規模データで性能劣化）
...
| テクニック | 適用条件 | 効果 |
|-----------|---------|------|
| 適切なインデックス | WHERE/JOIN/ORDER BY対象列 | クエリ速度10-100倍 |
| N+1クエリ防止 | 関連データ一括取得 | リクエスト数削減 |
| コネクションプーリング | 高トラフィック | DB負荷軽減 |
| 読み取りレプリカ | 読み取り多のアプリ | スケーラビリティ向上 |

### セキュリティ実装チェックリスト
- [ ] パラメータ化クエリの使用（SQLインジェクション防止）
- [ ] 入力値のサニタイズとバリデーション（Zod schema）
- [ ] 認証トークンの安全な管理（HTTPOnly Cookie推奨）
- [ ] レートリミット実装（API abuse防止）
- [ ] CORS設定の最小権限化
- [ ] 機密データの暗号化（at rest + in transit）

### アンチパターン
- ビジネスロジックをAPIルートに直接書かない: Service層に分離
- 生SQLを直接書かない: ORM or クエリビルダーを使用
- シークレットをハードコードしない: 環境変数で管理
- エラーの詳細を外部に公開しない: 内部エラーは汎用メッセージで返す
