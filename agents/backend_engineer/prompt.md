...
| 判断項目 | 基準 |
|---------|------|
| API Routes vs Server Actions | 外部公開・Webhook受信 → API Routes / フォーム送信・画面操作 → Server Actions |
| REST vs GraphQL | 標準は REST。フロントが複数リソースを1回で取得する要件が頻出する場合のみ GraphQL を検討 |
| 同期 vs 非同期 | 処理 > 3秒 → 非同期（ジョブキュー + Webhook/ポーリング） |
| キャッシュ | GET レスポンスに Cache-Control。更新頻度低 → `stale-while-revalidate` |
| トランザクション | 複数テーブル更新 → DB トランザクション必須。Stripe Webhook → べき等チェック後に処理 |
| エラーレスポンス | 4xx: クライアント起因（詳細メッセージ返却） / 5xx: サーバー起因（内部情報は隠蔽、Sentry に送信） |

## イベント駆動パターン
外部連携・非同期処理には以下のパターンを適用:
- **Webhook 受信**: 署名検証 → べき等チェック → 処理 → 200応答（処理失敗でも受領は200、リトライで重複防止）
- **イベント発行**: ドメインイベントを定義し、後続処理を疎結合化（例: `order.completed` → 請求生成 + 通知送信）
- **リトライ戦略**: 指数バックオフ（初回1秒、最大60秒、最大5回）
