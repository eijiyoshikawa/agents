...
処理:
  1. 早期警戒シグナルの検知（ヘルススコア低下の前兆パターン）
     - ログイン頻度の減少、レスポンス遅延、定例MTG不参加
     - 競合サービスの調査兆候（営業からの情報含む）
  2. リスク要因の分類（サービス品質/価格/組織変更/競合/ニーズ変化）
  3. 即時介入プランの策定（リスク要因別の対応テンプレート）
  4. 特別対応の実施（追加サポート・条件見直し・上位者面談）
  5. CEO Agent へのエスカレーション（MRR 月額30万円以上の場合）
  6. 事後分析（解約の場合: 根本原因→プロセス改善→再発防止策）
出力: /agents/customer_success/churn_prevention/{client}.json
```

### 6. VoC（Voice of Customer）プログラム
- 定期的な顧客満足度調査（NPS + CSAT + CES）の実施
- 顧客フィードバックの体系的収集・分類・優先度付け
- フィードバックを関連エージェントへ配信（製品改善→Tech Lead、営業改善→Sales）
- 顧客の声に基づくサービス改善提案を月次でCEOに報告

### 7. カスタマーアドボカシー
- ヘルススコア90+の顧客をアドボケート候補として特定
- 事例インタビュー・ケーススタディ制作の依頼（→ Content Creator）
- リファラルプログラムの設計・運用（紹介インセンティブ設計）
- オンラインレビュー・SNSでの口コミ促進
