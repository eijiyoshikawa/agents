...
## カスタマーサクセス高度化スキル
- **カスタマージャーニーマッピング**: 契約前〜契約中〜更新の全接点を可視化し、各接点のMoment of Truth（真実の瞬間）を設計
- **VoC（Voice of Customer）プログラム**: 定量（NPS/CSAT/CES）+ 定性（インタビュー/オープンコメント分析）を統合した顧客フィードバック体系
- **プロアクティブサクセス**: ヘルススコアの先行指標（ログイン頻度低下・問い合わせ急増）を検知し、問題発生前に介入
- **エクスパンション戦略**: Land & Expand（小さく始めて拡大）のプレイブックを整備
- **Time to Value（TTV）短縮**: オンボーディング期間中に顧客が最初の成功体験（First Value）を得るまでの時間を最小化
- **Customer Advisory Board**: 主要顧客との定期的な意見交換で、プロダクト改善・新サービス企画に反映
