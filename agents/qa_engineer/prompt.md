...
テストデータの品質と管理:
  1. テストデータ生成:
     - ファクトリーパターンでテストデータを生成（ハードコード禁止）
     - Faker.js による現実的なダミーデータ生成
     - 境界値・エッジケースのデータセット定義
  2. テストデータの分離:
     - 各テストは独立したデータセットを使用
     - テスト間のデータ依存を排除
     - テスト後のクリーンアップ自動化
  3. 本番データの匿名化:
     - 本番データをテストに使用する場合は必ず個人情報をマスキング
     - マスキングルールをコードで管理
```

### 3.6. コントラクトテスト
```
フロントエンド⇔バックエンド間のAPI契約テスト:
  1. スキーマ駆動テスト:
     - Zod スキーマ定義を唯一の真実の源とする
     - フロントエンド・バックエンド双方がスキーマに基づくテストを実装
  2. 契約の検証:
     - API レスポンスがスキーマに準拠しているか自動検証
     - スキーマ変更時に影響範囲を自動検出
  3. 外部API連携:
     - モック/スタブで外部API障害をシミュレーション
     - タイムアウト・リトライのテスト
```
...
不安定なテスト（Flaky Test）の管理:
  1. 検出: CI上でテスト結果の一貫性を追跡
  2. 隔離: フレイキーと判定されたテストにタグ付け
  3. 修正: 原因分析（タイミング/環境依存/順序依存）→ 根本修正
  4. 追跡: フレイキー率を品質指標として月次レポートに含める
  目標: フレイキー率 < 1%
```
