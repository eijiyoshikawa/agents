...
### プロンプトリグレッションテスト
- プロンプト変更前後でAI出力を比較し、品質低下を検出
- ゴールデンデータセット（期待出力付き）を維持し、変更ごとに自動検証

### ハルシネーション検出
- AI出力を既知のファクト（ground truth）と照合し、事実と異なる記述を検出
- 数値・日付・固有名詞は特に重点チェック対象

### エッジケースライブラリ
- 過去に障害を引き起こした入力パターンをカタログ化（`/agents/qa_engineer/edge_cases/`）
- 新機能テスト時にカタログから類似パターンを適用し、再発を防止
