...
処理:
  1. セキュリティテスト（多層防御の検証）
     - OWASP Top 10 チェック（全10項目を体系的にテスト）
     - 認証バイパス・権限昇格テスト（水平/垂直の両方）
     - XSS / CSRF / SQLインジェクション / SSRF検証
     - レート制限テスト（ブルートフォース耐性）
     - セッション管理テスト（固定化・ハイジャック・タイムアウト）
  2. パフォーマンステスト
     - Core Web Vitals 計測（LCP/INP/CLSの3指標）
     - API レスポンスタイム計測（p50/p95/p99）
     - 負荷テスト（同時接続数、スパイクテスト、ソークテスト）
     - メモリリーク検出（長時間稼働テスト）
  3. アクセシビリティテスト（axe-core + 手動テスト）
     - キーボードナビゲーション
     - スクリーンリーダー互換性
...
処理:
  ...
  2. 再現手順の文書化（Given-When-Then形式）
  3. 修正検証（リグレッションテスト）
  4. 品質メトリクスの集計
     - バグ検出率 / 修正率 / MTTRh（平均修正時間）
     - テストカバレッジ推移（ライン/ブランチ/パス）
     - リリースブロッカー数
     - テスト安定性（フレイキーテスト率 — 目標: 1%以下）
     - 欠陥漏洩率（本番でのバグ発生/テスト検出バグ — 目標: 5%以下）
出力: 品質レポート
```
...
リリース判定基準（全項目クリアで "go"）:
  ...
  - [ ] リリースブロッカー = 0
  - [ ] セキュリティテスト全通過
  - [ ] Core Web Vitals バジェット内
  - [ ] 全E2Eテスト通過（フレイキー除外なし）
  - [ ] アクセシビリティ違反 = 0（Critical/Serious）
```
