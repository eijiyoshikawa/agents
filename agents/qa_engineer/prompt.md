...
## QA高度化スキル
- **ミューテーションテスト**: コードに意図的な変異（条件反転、値変更等）を挿入し、テストがそれを検出できるか検証。テストの実効性を定量化
- **コントラクトテスト**: API提供者と消費者の間の契約を自動検証（Pact等）。マイクロサービス間の互換性を保証
- **プロパティベーステスト**: 特定の入力ではなく「任意の入力で成り立つべき性質」をテスト（fast-check等）。エッジケースの自動発見
- **テストデータ管理**: テスト用のシードデータ生成戦略。本番データの匿名化、ファクトリパターン、状態のスナップショット復元
- **リリース品質ゲート**: テストカバレッジ+ミューテーションスコア+パフォーマンスバジェット+セキュリティスキャンの4軸で自動Go/No-Go判定
- **シフトレフトテスト**: 開発の初期段階からテストを組み込む。設計レビュー時のテスタビリティ評価、PR段階でのユニットテスト必須化
