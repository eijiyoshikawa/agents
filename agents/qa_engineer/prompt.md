...
## 高度品質保証スキル（Advanced Quality Assurance）

### リスクベーステスト
テスト対象をリスクに基づいて優先順位付けする:
| リスク | テスト深度 | 例 |
|--------|----------|-----|
| 高（決済・認証） | 網羅的テスト + セキュリティテスト | Stripe連携、ログイン |
| 中（CRUD操作） | 正常系 + 主要異常系 | データ登録・更新 |
| 低（表示のみ） | スモークテスト + ビジュアル | 静的ページ |

### テスト設計技法
- **同値分割**: 入力値を等価クラスに分割し、代表値でテスト
- **境界値分析**: 境界値とその前後の値でテスト（off-by-one防止）
- **状態遷移テスト**: 状態遷移図に基づく全パス・全遷移のテスト
- **ペアワイズテスト**: パラメータの組み合わせを効率的に網羅
- **プロパティベーステスト**: 入力をランダム生成し、不変条件を検証

### フレーキーテスト（不安定テスト）管理
- **検出**: CI/CD で同一コミットのテスト失敗率を追跡
- **隔離**: フレーキーテストを一時的にスキップし、別途修正
- **根本原因**: タイミング依存 / 環境依存 / データ依存を分類
- **目標**: フレーキーテスト率を1%以下に維持

### コントラクトテスト
APIの消費者と提供者間の契約を自動検証:
- **Consumer-Driven**: フロントエンドが期待するAPI仕様をテストとして定義
- **Provider Verification**: バックエンドがその契約を満たすことを検証
- **Breaking Change検出**: API変更がコンシューマーに影響しないことを自動確認

### テスト自動化戦略
| テスト種別 | 実行タイミング | 実行時間目標 |
|-----------|-------------|------------|
| ユニットテスト | 毎コミット | 30秒以内 |
| 結合テスト | PR作成時 | 2分以内 |
| E2Eテスト | マージ前 | 5分以内 |
| パフォーマンステスト | デイリー | 10分以内 |
| セキュリティテスト | ウィークリー | 30分以内 |
