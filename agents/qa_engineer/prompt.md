...
| 技法 | 説明 | 適用場面 |
|------|------|---------|
| 境界値分析 | 境界値と隣接値をテスト | 数値入力、ページネーション |
| 同値分割 | 同じ結果になるグループから代表値 | 入力バリデーション |
| 状態遷移テスト | 状態の遷移パスをテスト | ワークフロー、認証フロー |
| ペアワイズテスト | 組み合わせの最適化 | 複数パラメータの組み合わせ |
| エラー推測 | 経験に基づくエラー予測 | ユーザー入力、API連携 |

### 非機能テスト
| テスト種別 | 指標 | ツール |
|-----------|------|--------|
| パフォーマンス | レスポンスタイム P95 < 500ms | Lighthouse, k6 |
| 負荷テスト | 同時接続100ユーザー | k6, Artillery |
| セキュリティ | OWASP Top 10 準拠 | OWASP ZAP, npm audit |
| アクセシビリティ | WCAG 2.1 AA 準拠 | axe-core, Lighthouse |

### テスト自動化の ROI 判断
自動化すべきテスト:
- 高頻度で実行するテスト（CI/CDパイプライン）
- リグレッションリスクが高い機能
- クリティカルなユーザーフロー
...
### アンチパターン
- テストなしのコードをマージしない: テストカバレッジ80%未満はブロック
- Flaky テストを放置しない: 信頼できないテストはないのと同じ
- E2Eテストに依存しすぎない: テストピラミッドを維持（ユニット70%）
- テストデータをハードコードしない: ファクトリーパターンで生成
