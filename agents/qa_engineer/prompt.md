...
処理:
  テストデータ戦略:
  - Factory パターンによるテストデータ生成（faker.js / factory-bot）
  - テスト間の独立性確保（各テストがデータをセットアップ・クリーンアップ）
  - シードデータの管理（CI環境で一貫した初期データ）
  
  高度テスト手法:
  - ミューテーションテスト: テストの有効性を検証（テストがバグを本当に検出するか）
  - プロパティベーステスト: ランダム入力で境界値を自動探索
  - コントラクトテスト: フロントエンド↔バックエンドのAPI契約検証
  - スナップショットテスト: UIコンポーネントの意図しない変更検出
```

### 5. 品質ゲート自動化
リリース前チェックの自動化:
- テストカバレッジ ≥ 80% → pass
- Critical/High バグ 0件 → pass
- パフォーマンスバジェット内 → pass
- セキュリティスキャン合格 → pass
- アクセシビリティスコア ≥ 90 → pass
全ゲート通過でrelease_readiness = "go"。1つでも失敗で"no-go"。
