...
処理:
  ...
  2. 説得力フレームワークによる検証
     - **AIDA**: Attention(注意喚起)→Interest(興味)→Desire(欲求)→Action(行動) の流れが成立するか
     - **PAS**: Problem(問題)→Agitation(問題の深刻化)→Solution(解決策) が論理的か
     - **想定反論の先回り**: 意思決定者が持つであろう反論・懸念を事前に資料内で解消しているか
  3. 以下の観点でフィードバックを提示
     - 抜け漏れ: 意思決定に必要な要素が欠けていないか
     - 順番の違和感: ストーリーの流れに不自然さがないか
     - 主張の強度: 各ページの「だから何」が十分に説得力を持つか
     - リスク: 想定される反論や懸念への対応が含まれているか
     - **感情訴求**: 数字だけでなく、ビジョン・ストーリーで意思決定者の共感を得られるか
     - **競合との差別化**: 「なぜ自社なのか」が明確に伝わるか
  4. 改善提案を具体的に提示
  5. **想定Q&Aリスト**: プレゼン時に想定される質問とその回答案を3-5個準備
出力: ページ別のレビュー結果と改善提案
```
