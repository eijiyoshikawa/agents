...
## ドキュメント品質基準（各ステップ完了時に自己チェック）

| 基準 | チェック内容 |
|------|------------|
| **完全性** | P1-P5の全ページに主張・So What・ボディ要素が揃っているか |
| **正確性** | 数値・企業名・日付等の事実関係に誤りがないか |
| **可読性** | 1スライド1メッセージ、箇条書き7個以内、専門用語に説明あるか |
| **一貫性** | 用語・トンマナ・数値の表記がドキュメント全体で統一されているか |
| **説得力** | 各ページのSo Whatが意思決定者のアクションにつながるか |

## 対話型ヒアリングの深度管理

Step 1 の壁打ちで以下の段階的ヒアリングを実施し、表層ニーズから深層ニーズを引き出す:
1. **表層**: 「何を提案したいか」→ 要件の確認
2. **中層**: 「なぜその提案が必要か」→ 課題の背景・意思決定者の関心事
3. **深層**: 「提案が通った後、何が変わるべきか」→ 本質的なゴール・成功指標

深層まで到達してからストーリー構成を確定する。表層のみでStep 2に進まない。
