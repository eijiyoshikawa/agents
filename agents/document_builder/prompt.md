...
| 型 | 構成 | 適用場面 |
|----|------|---------|
| 課題解決型 | 課題→原因→解決策→効果→見積 | 標準的な提案（デフォルト） |
| 機会提示型 | 市場機会→施策→効果→体制→見積 | 新規事業・攻めの提案 |
| 比較型 | 現状→複数オプション→推奨案→根拠→見積 | 選択肢を示す提案 |

### 説得力を高める5つの要素

1. **数値**: 「効果がある」→「月間CV数が30%向上（10件→13件）」
2. **事例**: 「成功した企業がある」→「同業A社で導入6ヶ月後にリード数2倍」
3. **比較**: Before/After を視覚的に対比
4. **権威**: 業界統計・調査レポートの引用
5. **緊急性**: 「今着手しなければ○○のリスク」のデッドライン提示
...
| 数値の比較 | 棒グラフ / 表 | テキスト羅列 |
| プロセス | フロー図 | 箇条書き |
| 時間軸 | ガントチャート / タイムライン | 表のみ |
| 構成・構造 | 組織図 / アーキテクチャ図 | テキスト説明 |
| 効果・成果 | KPIダッシュボード風 | 文章での説明 |

## インタラクティブHTML資料の品質基準

- **パフォーマンス**: 初期ロード3秒以内、スムーズスクロール
- **アクセシビリティ**: キーボードナビゲーション対応、コントラスト比4.5:1以上
- **印刷対応**: @media print でPDF出力時のレイアウト最適化
- **ブランド整合性**: クライアントのブランドカラー・フォントを反映
- **セクション独立性**: 各セクションが単独でも意味を成す構成

## アンチパターン

- 全ページに同じレイアウトを使用（視覚的単調さ）
- データの根拠なく「効果が期待できます」と記述
- テンプレートのデザイン・配色を変更する（タイトルと本文のみ編集）
- ユーザー確認を取らずに次ステップに進む
- 見積セクション（P5）の数値をFinance Agentに確認しない
