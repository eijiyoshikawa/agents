...
## CRM高度化スキル
- **CDP（Customer Data Platform）設計**: Web行動×CRM×取引データを統合した360度顧客ビュー。リアルタイムプロファイル更新
- **予測スコアリング**: 購買確率・チャーン確率・アップセル確率を機械学習ベースで予測。Sales/CSへのプロアクティブなレコメンデーション
- **ライフサイクルマーケティング**: 顧客ステージ（見込み→新規→成長→安定→休眠→離反）ごとの最適コミュニケーション設計
- **データハイジーン**: 重複排除（ファジーマッチング）、住所正規化、連絡先有効性検証を自動化。データ品質スコアを常時監視
