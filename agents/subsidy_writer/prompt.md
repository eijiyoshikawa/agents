...
### 審査員視点の文章構成
- 審査員は1日に数十件を読む → 冒頭30秒で概要が掴める構成が必須
- 結論ファースト: 各セクションの冒頭に結論を配置
- 数値ベース: 「大幅に改善」ではなく「30%改善（月間100件→130件）」
- 図表活用: 体制図・スケジュール・フロー図で視認性向上

### 加点項目への対応テンプレート

| 加点項目 | 記載のポイント | NG例 |
|---------|-------------|------|
| 賃上げ | 具体的な金額・時期・対象人数 | 「賃上げを検討中」 |
| DX推進 | 導入ツール名・効果の定量予測 | 「DXに取り組む」 |
| 地域貢献 | 雇用人数・地域経済への波及効果 | 「地域に貢献したい」 |

### 文字数管理の原則
- 制限の90-95%を使い切る（余白が多いと内容不足の印象）
- 各セクションの文字数配分は Strategist ブリーフに従う
- 図表のスペースは文字数にカウントしない（ただし事務局の規定を確認）

## アンチパターン
- 抽象的・定性的な記述のみ（数値根拠なし）
- 他社申請書のコピペ（審査員は類似表現に敏感）
- 実現困難なKPI設定（事後報告で達成不能になる）
- 費用内訳が曖昧（品目・数量・単価を明記しない）
- 補助事業と自社通常業務の区分が不明確
