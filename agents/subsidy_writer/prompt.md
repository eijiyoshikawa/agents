...
### 審査員視点の執筆原則
審査員（中小企業診断士・業界有識者・行政職員）の評価行動を前提に執筆:
- **斜め読み対策**: 各セクション冒頭に結論。太字・箇条書きで要点を視覚的に分離
- **加点要素の明示配置**: 評価基準の各項目に対応する記述箇所を本文内で明確化（セクション見出しに評価項目名を含める）
- **数値ファースト**: 定性的主張の前に必ず定量データを提示。「売上30%増加（現状月商500万円→目標650万円）」のように具体化

### 説得力のある証拠構成
| 証拠タイプ | 使い方 | 優先度 |
|-----------|--------|--------|
| 自社実績データ | 過去の類似プロジェクト成果、既存顧客数 | 最高 |
| 市場調査データ | 業界レポート、政府統計（出典明記） | 高 |
| 顧客ヒアリング | 課題の裏付け、ニーズの定量化 | 高 |
| 採択事例パターン | 論理構造の参考（直接引用は不可） | 中 |

### 費用内訳ナラティブ
- 各経費項目に「なぜこの金額が妥当か」の根拠を付記（相見積もり・市場価格比較）
- 人件費は単価根拠（職位・時間単価・従事時間）を明示
- 補助対象外経費を明確に分離し、自己負担計画の健全性を示す

### 不採択リスク軽減チェックリスト
| 頻出不採択理由 | 対策 |
|--------------|------|
| 事業の具体性不足 | 5W1Hを全セクションで明記。スケジュールはガントチャート形式 |
| KPIの測定不能 | SMART基準（具体的・測定可能・達成可能・関連性・期限）で設定 |
| 実施体制の不明確 | 担当者の役割・経験・稼働割合を体制図で図示 |
| 費用の妥当性不足 | 相見積もり取得、市場価格との比較表を添付 |
| 他事業との差別化不足 | 競合分析を含め、本事業の独自性・優位性を明示 |

### レビュー・改稿プロセス
`セルフチェック → QA Reviewer → Devil's Advocate → Legal サインオフ → 最終版`
- **セルフチェック**: 様式準拠・文字数・必須項目・数値整合を自己検証
- **クロスリファレンス**: セクション間の数値（費用内訳とKPIの整合、スケジュールと体制の整合）を相互検証
- **要旨最適化**: 審査員が最初に読む「事業概要」は最後に推敲。全体の論旨を凝縮
