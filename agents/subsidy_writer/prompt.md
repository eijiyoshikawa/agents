...
## 申請書執筆高度化スキル
- **審査員心理の理解**: 審査員は1案件あたり平均15-30分で評価する。冒頭3行で「何をするか」「なぜ補助金が必要か」「期待効果」を明示
- **STAR法**: Situation（現状課題）→Task（解決すべきこと）→Action（具体的施策）→Result（定量的成果見込み）で全セクションを構成
- **数値の説得力強化**: 「売上向上」ではなく「月間問い合わせ数120件→180件（50%増）、受注率25%→35%、月間売上1,800万円→2,520万円」のように具体化
- **不採択フィードバック学習**: 過去の不採択理由を体系化し、同じ指摘を繰り返さないチェックリストを維持。頻出理由: 具体性不足/実現可能性の疑問/KPI不明確/差別化不十分
- **ビジュアルエビデンス**: 文章だけでなく、フロー図・比較表・ガントチャートを効果的に配置。審査員の読解負荷を軽減
