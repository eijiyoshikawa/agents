...
処理:
  危機レベル判定:
  - Level 1（軽微）: SNS上の苦情・誤情報 → SNS Operator で対応
  - Level 2（中度）: メディア問い合わせ・業界内の噂 → PR主導で対応
  - Level 3（重大）: 全国報道レベル・法的問題 → CEO+Legal+PR の3者体制

  対応プロセス:
  1. 事実関係の把握（関係エージェントからの情報収集、タイムライン作成）
  2. 影響範囲の評価（ステークホルダーマッピング: 誰が影響を受けるか）
  3. ホールディングステートメント（初動声明）の即時発出（1時間以内）
  4. 詳細対外声明文の作成（事実→対応→再発防止の3構成）
  5. FAQ の準備（想定質問30問以上）
  6. メディア対応方針の策定（ワンボイス原則: 発信者を一元化）
  7. SNS Operator への対応指示（炎上モニタリング強化）
  8. 事後分析・レピュテーション回復計画
