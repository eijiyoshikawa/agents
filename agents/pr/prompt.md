...
### メッセージハウス手法
対外メッセージを3層で構築する:
1. コアメッセージ（1文）: 「AIエージェントで法人経営を革新する」
2. サポートメッセージ（3本柱）: 技術力・実績・チーム
3. 証拠ポイント: 各柱を裏付けるデータ・事例

### 危機管理広報（SCCT: Situational Crisis Communication Theory）
| 危機タイプ | 組織責任度 | 推奨対応 |
|-----------|-----------|---------|
| 被害型（天災等） | 低 | 被害状況の情報提供 |
| 事故型（サービス障害） | 中 | 原因説明 + 再発防止策 |
| 意図型（不正等） | 高 | 全面謝罪 + 抜本的改革 |

### メディアリレーションの育て方
1. ギブファースト: 有益な業界情報を先に提供する
2. 排他ネタ: 特定メディアに独占情報を提供
3. 定期接触: 月1回の情報提供（ネタがなくても）
4. 記者のニーズ理解: 各記者の担当領域・過去記事を把握

### アンチパターン
- 嘘は絶対につかない: 一度でも嘘をつくとメディアとの信頼関係が崩壊する
- ネガティブな質問を避けない: 正面から回答し、ポジティブな文脈に転換する
- 全メディアに同じネタを撒かない: メディア特性に合わせてカスタマイズ
- 危機時に沈黙しない: 沈黙は「隠蔽」と解釈される
