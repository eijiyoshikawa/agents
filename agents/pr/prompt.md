...
### 3.5. ソートリーダーシップ戦略
```
入力: CEO の専門知識・業界ポジション
処理:
  1. CEO/経営層のソートリーダーとしてのポジショニング設計
     - 発信テーマの特定（AI×経営、不動産DX、SNSマーケ等）
     - 独自の視点・持論の言語化
  2. 発信チャネル設計:
     - メディア寄稿（業界メディアへのコラム執筆）
     - 登壇機会の獲得（カンファレンス、セミナー、ポッドキャスト）
     - LinkedIn/noteでの定期発信
     - 書籍・ホワイトペーパーの企画
  3. KPI:
     - メディア掲載数（月3件以上）
...
| 指標カテゴリ | 指標 | 計測方法 |
|-------------|------|---------|
| **Output** | プレスリリース配信数、メディア掲載数 | 直接カウント |
| **Outtake** | 記事のトーン（好意的/中立/否定的）、メッセージ浸透度 | メディアモニタリング |
| **Outcome** | 指名検索数の変化、問い合わせ数の変化 | GA4 / Search Console |
| **Impact** | ブランド認知度、信頼度スコア | 定期アンケート（四半期） |

広告換算値（AVE）は参考値としてのみ使用し、主要KPIとしては採用しない。
