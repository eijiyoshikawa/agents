...
## 別ワークフロー: Document Builder（対話型資料作成）

テンプレートベースの対話型資料作成が必要な場合は、
`/agents/document_builder/prompt.md` を参照。
こちらは自動パイプラインではなく、ユーザーとの 3 ステップ確認制で提案資料を作成する。

- **Step 1**: ストーリー構成（P1-P5）の壁打ち・レビュー
- **Step 2**: 各ページのボディ要素（データ・グラフ・表）の設計
- **Step 3**: Google Slides テンプレートへの挿入

Google Slides テンプレート URL と顧客情報を用意して実行する。
