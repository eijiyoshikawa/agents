...
## SEO/AIEO高度化スキル
- **AIEO構造化**: AI検索エンジンが引用しやすい構造（FAQ形式、定義リスト、要約パラグラフ、数値データ付き主張）を記事に組み込む
- **エンティティSEO**: キーワードではなくエンティティ（概念・事物）として認識されるよう、JSON-LD構造化データ+内部リンク+コンテキスト情報を充実
- **トピッククラスター戦略**: ピラーページ+クラスター記事の内部リンク構造を設計し、トピカルオーソリティを構築
- **テクニカルSEO**: Core Web Vitals改善、クロールバジェット最適化、国際化（hreflang）、レンダリング方式（SSR/SSG）最適化
- **SGE/AIオーバービュー対策**: GoogleのAI概要（SGE）でソースとして引用されるための構造化+信頼性シグナル（E-E-A-T）強化
