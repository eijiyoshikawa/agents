...
## 高度プレゼンテーション設計スキル（Advanced Presentation Design）

### ピラミッド原則（Barbara Minto）
- **頂点**: キーメッセージ（1文で提案の核心を表現）
- **支柱**: キーメッセージを支える3つの主張
- **基盤**: 各主張を裏付けるデータ・事例・論理
- **So What テスト**: 各スライドに「だから何？」と問い、行動につながるか検証

### SCQA構造
エグゼクティブサマリーで使用する説得構造:
- **Situation**: 「御社は〇〇の状況にあります」
- **Complication**: 「しかし△△の課題が生じています」
- **Question**: 「どうすれば□□を実現できるか？」
- **Answer**: 「我々は■■を提案します」

### データストーリーテリング
- **コンテキスト**: データの背景を先に説明し、読み手の理解を助ける
- **アテンション**: 最も重要なデータポイントに視覚的注目を集める
- **ナラティブ**: 「何が起きていて、なぜ重要で、何をすべきか」で語る
- **認知負荷の最小化**: 1スライド1メッセージ、チャートジャンク排除

### スライド設計の黄金律
| ルール | 基準 |
|--------|------|
| 1スライド1メッセージ | タイトルが主張（アサーション）になっている |
| 箇条書き | 5-7項目以内、同一粒度、並列構造 |
| グラフタイトル | インサイトを記載（「売上推移」→「売上は3Q連続成長」） |
| フォントサイズ | 見出し28pt+、本文18pt+、注釈14pt+ |
| 色使い | 強調色は1色のみ、グレースケール+アクセント |

### エグゼクティブ向け最適化
- **3秒ルール**: 各スライドのメッセージが3秒で理解できること
- **ボトムライン先出し**: 結論→根拠の順で構成
- **意思決定ポイント**: 判断が必要な箇所を明示し、選択肢を提示
- **想定質問対応**: Appendixに想定質問への回答スライドを準備
