...
### Step 2: ストーリーライン設計（Pyramid Principle）
スライド全体を通じた説得ストーリーを設計する:
- **SCQA構造**: Situation（状況）→ Complication（課題）→ Question（問い）→ Answer（提案）
- **So What テスト**: 各スライドが「だから何？」に答えられるか検証
- **キーメッセージ**: 各スライドのタイトルだけ読んでもストーリーが伝わるか確認（タイトルライン法）

### Step 3: 各スライドの内容作成
各スライドについて以下を記述する:
- **タイトル**（アクションタイトル: 結論を述べる形式。例:「市場は年8%成長し、3年で2倍に」）
- 箇条書きポイント（5-7個以内、数値を含める）
- **データビジュアライゼーション指示**（chart_type: bar/line/pie/matrix/funnel/timeline）
- スピーカーノート（補足説明 + 想定質問への回答ポイント）

### Step 4: クライアント適応チューニング
- **プレゼン対象者の知識レベル**に応じた用語・詳細度の調整
- **意思決定者の関心軸**（コスト重視 / 成長重視 / リスク回避型）に応じたスライド順序調整
- **業界慣習**に合わせた表現・事例の選定
  ...
  "slides": [
    {
      ...
      "bullets": ["ポイント1", "ポイント2"],
      "chart_type": "none | bar | line | pie | matrix | funnel | timeline | comparison_table",
      "chart_data": "チャートに使用するデータ（該当時）",
      "design_notes": "レイアウト・カラー指示",
      "speaker_notes": "スピーカーノート",
      "anticipated_questions": ["想定質問1"]
    }
  ],
  "summary": "提案資料の全体サマリー"
}
