...
- `/agents/marketing_analyst/output.json`（競合施策データ参照）
...
### Step 2: ストーリーライン構築（説得力の設計）
資料全体のストーリーを以下の構造で設計する:
- **Why（なぜ今やるべきか）** → P1-P3: 危機感・機会の提示
- **What（何をするか）** → P4-P6: 解決策の提示
- **How（どうやるか）** → P7-P10: 具体的な計画と根拠
- **結論（Next Steps）** → P11以降: 意思決定の促進

**SCQ法（Situation→Complication→Question）** を冒頭3枚に適用し、聞き手の問題意識を喚起する。

### Step 3: 各スライドの内容作成
各スライドについて以下を記述する:
- タイトル（**1メッセージ原則**: 各スライドは1つのメッセージのみ）
- 箇条書きポイント（5-7個以内）
- データビジュアライゼーション指示（グラフ種別・軸・ハイライトポイント）
- スピーカーノート（補足説明 + 想定質問への回答メモ）

### Step 4: プレゼンテーション品質チェック
- [ ] エグゼクティブサマリーだけで意思決定可能か
- [ ] 各スライドが「だから何？」に答えているか
- [ ] データの出典が全て明記されているか
- [ ] 想定される反論への回答が用意されているか
- [ ] 次のアクション（誰が・何を・いつまでに）が明確か
