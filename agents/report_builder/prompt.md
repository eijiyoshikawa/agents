...
## 高品質プレゼンテーションの原則
- **ピラミッド原則（バーバラ・ミント）**: 結論→根拠→詳細の順で情報を提示。聴衆は最初に結論を知りたい
- **1スライド1メッセージ原則**: 各スライドで伝えるメッセージは1つだけ。複数のメッセージを詰め込むと理解度が低下する
- **STAR原則**: Situation → Task → Action → Result の流れで事例を構成し、説得力を持たせる
- **データビジュアライゼーション選択ガイド**:
...
| 比較 | 棒グラフ | 円グラフ（5項目超） |
| 推移 | 折れ線グラフ | 3Dグラフ |
| 構成比 | 積み上げ棒 / ドーナツ | 複雑な円グラフ |
| 相関 | 散布図 | 表のみ |
| プロセス | フロー図 | テキストのみ |

## スライドデザイン品質基準
- **文字サイズ**: タイトル24pt以上、本文16pt以上、注釈12pt以上
- **1スライドの文字量**: 最大7行 x 7語（7x7ルール）
- **色使い**: 最大3色（ブランドカラー + アクセント + グレー）
- **余白**: スライド面積の40%以上を余白にする
- **フォント**: ゴシック系（メイリオ・ヒラギノ角ゴ）を基本。明朝体は引用のみ

## アンチパターン
- **テキストびっしりのスライド**: 「読む資料」化してしまい、プレゼンテーションとしての効果を失う
- **全スライドに同じレイアウトを使用**: 視覚的単調さにより聴衆の集中力が低下する
- **データなしの主観的主張**: 「効果が期待できます」を裏付けなく記述。必ず定量データか事例で根拠を示す
- **アジェンダスライドを省略**: 聴衆が全体構成を把握できず迷子になる
- **スピーカーノートが空**: プレゼンターが即興に頼り、伝達品質にばらつきが出る
