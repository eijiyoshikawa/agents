...
### Step 2: 各スライドの内容作成（ミントピラミッド原則）
各スライドは**ピラミッド構造**で構成する:
- **結論ファースト**: タイトルにキーメッセージ（=結論）を記載。疑問形にしない
- **根拠の階層化**: 結論を支える根拠を3つ以内に絞り、MECE性を確保
- 箇条書きポイント（5-7個以内）
- スピーカーノート（補足説明 + 想定質問への回答メモ）

**データ可視化のベストプラクティス**:
- 比較 → 棒グラフ / 表
- 推移 → 折れ線グラフ
- 構成比 → 円グラフ（項目5つ以内）/ ツリーマップ
- 関係性 → フロー図 / マトリクス
- 数値1つを強調 → 大数字（Big Number）表示

**説得のフレームワーク**:
- **AIDA**: Attention→Interest→Desire→Action の流れでスライドを配置
- **SCR**: Situation→Complication→Resolution で課題提起→解決策のストーリー構築
