...
| 原則 | チェック項目 |
|------|------------|
| **C**ontrast（コントラスト） | 見出し vs 本文のサイズ差が明確か、CTA が背景から視覚的に際立っているか |
| **R**epetition（反復） | 同カテゴリの要素が統一されたスタイルか、デザイントークンに沿っているか |
| **A**lignment（整列） | テキスト・画像の左端/中央が揃っているか、暗黙の整列線が存在するか |
| **P**roximity（近接） | 関連要素がグルーピングされているか、無関係な要素と十分な余白があるか |

### LP コンバージョン最適化
| 要素 | 基準 |
|------|------|
| ヒーローCTA | ファーストビュー（ATF）に必ず配置、ボタンテキストは動詞で始める |
| 視線誘導 | F/Zパターンに沿った情報配置、重要要素を視線の自然な到達点に |
| CTA配置 | ヒーロー / 中間 / ページ末の最低3箇所、スクロール50%以降で固定CTA検討 |
| 社会的証明 | ロゴバー・導入実績・レビューをCTA直前に配置 |
| フォーム最適化 | 入力フィールドは5個以内、段階的開示で心理的負担を軽減 |
...
|------|--------|------------|------|
| Webバナー | 72dpi | sRGB | — |
| SNS広告画像 | 72dpi, 各媒体推奨サイズ準拠 | sRGB | テキスト安全領域内に収める |
| 印刷物（名刺・チラシ） | 350dpi | CMYK | 塗り足し3mm |
| 大判印刷（ポスター） | 150-200dpi | CMYK | 塗り足し5mm |

**注意**: Web制作が主業務のため印刷入稿は最終確認を外部に依頼することを推奨。
