...
### AI Designer MCP の効果的なプロンプティング
- **具体的な要素指定**: 「ヒーローセクションに動画背景、左寄せの見出し、右下にCTAボタン」
- **禁止事項の明示**: 「グラデーション背景を使わない」「アイコン一覧セクションを作らない」
- **参考サイトの構造指定**: 「Stripe.com のプライシングページの構成を参考に」
- **反復改善**: 1回目で構成を確定 → 2回目で色・フォント調整 → 3回目でレスポンシブ確認
- **判断根拠の記録**: 各デザイン案に `design_rationale`（なぜこの色・レイアウト・フォントか）を必ず記録し、クライアントFB時に説明可能な状態を維持する
### レスポンシブデザインの5段階チェック
| 画面幅 | デバイス | 確認ポイント |
|--------|---------|------------|
| 375px | iPhone SE | 最小幅での崩れ |
| 390px | iPhone 14 | 標準スマホ |
| 768px | iPad | タブレット |
| 1024px | iPad Pro / 小型ノート | 中間サイズ |
| 1440px | デスクトップ | 標準デスクトップ |

## アンチパターン
- AI Designer MCP に「かっこいいデザインを作って」と曖昧な指示を出す
- 1回の生成で完成を期待する（最低3回の反復改善が必要）
- design-tokens.json を参照せずにデフォルトカラーで生成
- モバイル版を確認せずにデスクトップ版だけで納品
- クライアントの業界・ブランドを考慮しない汎用デザイン
