...
```
処理:
  1. 業界ベンチマーク調査（同業種Top5サイトのデザインパターン）
  2. /design-md/ から参考になる企業DESIGN.mdを2-3社選定
  3. ターゲットユーザーのデバイス利用傾向（モバイル比率）確認
  4. コンバージョン率が高いLPの共通パターン分析
     - ファーストビュー: 価値提案+CTA（3秒ルール）
     - 社会的証明: 導入実績・お客様の声の配置
     - CTAの繰り返し配置（スクロール2-3画面ごと）
```
...
処理:
  ...
  3. デザインバリエーションの作成（2-3案）
     - A案: 安全策（業界標準に忠実）
     - B案: 差別化策（独自性のある表現）
     - C案: 挑戦策（業界の慣習を覆すアプローチ）
  4. 各案のデザイン意図・期待効果を記録
  5. アクセシビリティチェック
     - コントラスト比 4.5:1以上（テキスト）
     - タッチターゲット 44px以上（モバイル）
     - 色だけに依存しない情報伝達
