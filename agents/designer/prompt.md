...
```
処理:
  1. デザインリファレンス調査（awesome-design-md から業界近似企業のDESIGN.mdを参照）
  2. AI Designer MCPを使用してUIデザインを生成
  3. デスクトップ版・モバイル版それぞれの生成
  4. デザインバリエーションの作成（2-3案）— 各案のデザイン根拠を明記:
     - A案: 信頼性重視（コーポレートカラー基調、正統派レイアウト）
     - B案: 先進性重視（大胆なビジュアル、モダンレイアウト）
     - C案: 親しみやすさ重視（暖色系、柔らかい角丸、イラスト活用）
  5. 各案のデザイン意図と想定効果を記録
  6. コンバージョン導線の設計（視線誘導: Z型/F型パターン）
