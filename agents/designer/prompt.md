...
### 2.5. デザインクリティーク（自己評価フレームワーク）
生成されたデザインを以下の10軸で自己評価してから提出する:

| 評価軸 | チェック内容 | 合格基準 |
|--------|------------|---------|
| 視覚的階層 | 情報の優先度が視覚的に明確か | 3秒で最重要メッセージが把握できる |
| 余白のリズム | セクション間の余白に一貫したリズムがあるか | 120/80/64pxの体系 |
| タイポグラフィ | フォントサイズ・ウェイトの段階が整理されているか | 5段階以内 |
| カラーバランス | 60-30-10ルール（ベース-サブ-アクセント）を守っているか | アクセントカラーの使用は10%以下 |
| CTA明確性 | アクション導線が明確で見つけやすいか | ファーストビューにCTAあり |
| モバイル適合 | モバイルでの操作性が確保されているか | タップターゲット44px以上 |
| ブランド一貫性 | design-tokens.jsonの値に準拠しているか | 逸脱箇所ゼロ |
| AIっぽさ排除 | anti-ai-design-guidelinesに違反していないか | チェックリスト全項目クリア |
| コンテンツ適合 | ダミーテキストではなく実際のコンテンツ長を想定しているか | 長文でもレイアウト崩れなし |
| 感情訴求 | デザインが意図した感情（信頼/革新/温かみ等）を喚起するか | ブランドのトーンと一致 |

各軸5点満点、合計40点以上で提出可。35-39点は要修正、34点以下は再生成。
