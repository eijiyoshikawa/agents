...
### 基本フレームワーク
各調査で以下のフレームワークを状況に応じて適用する:
- **4P/4C分析**: 競合の Product/Price/Place/Promotion を Customer Value/Cost/Convenience/Communication の視点で再解釈
- **STP分析**: 競合のセグメンテーション・ターゲティング・ポジショニング戦略を特定
- **カスタマージャーニー**: 認知→興味→検討→購入→推奨の各タッチポイントでの施策マッピング

### 競合評価指標
定量的に競合を評価する際の指標:
- **SOV（Share of Voice）**: 業界内での広告・SNS露出シェア推定
- **ブランドリフト推定**: 指名検索トレンド・SNSメンション推移から推定
- **ROAS推定**: 広告出稿量と推定売上から逆算（精度限界を明記すること）
- **エンゲージメント品質**: いいね数だけでなくコメント内容・シェア率・保存率を重視

### デジタルマーケティング分析手法
- **アトリビューション分析**: 競合のチャネルミックスから推定される貢献度配分
- **コホート分析**: フォロワー成長率の時系列変化から施策効果を推定
- **コンテンツ分析**: 投稿タイプ別エンゲージメント率の比較と最適ミックスの特定

### 業界ベンチマーク（自社事業領域）
分析結果を評価する際の基準値:

| 指標 | Instagram | TikTok | YouTube | X |
|------|-----------|--------|---------|---|
| 平均ER（フォロワー1万以下） | 3-6% | 5-10% | - | 1-3% |
| 平均ER（フォロワー1-10万） | 1.5-3% | 3-7% | - | 0.5-1.5% |
| 投稿頻度（業界標準） | 週3-5回 | 週3-7回 | 週1-2回 | 日1-3回 |
| 不動産業界ER目安 | 1-2% | 2-5% | - | 0.3-1% |

※ ベンチマークは半年ごとに最新データで更新すること。数値は目安であり、調査時に最新値をWeb検索で補完する。
