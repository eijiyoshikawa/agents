...
| 指標 | 定義 | 測定方法 |
|------|------|---------|
| Share of Voice (SOV) | 業界内での広告・PR露出シェア | 広告ライブラリ・メディア掲載数の比較 |
| Share of Search | ブランド検索ボリュームの相対シェア | Google Trends・キーワードツールで推計 |
| エンゲージメントベンチマーク | 業界平均対比のSNSエンゲージメント率 | 競合アカウントの公開指標から算出 |
| コンテンツ出稿量 | 月間投稿数・広告クリエイティブ数 | 各プラットフォーム広告ライブラリで確認 |

### 定性的評価
- **ポジショニングマップ**: 2軸（例: 価格×専門性）で競合をマッピング
- **ブランド知覚分析**: SNS上のセンチメント・口コミの方向性を分類
- **差別化要素の棚卸し**: 各社のUSP・訴求軸を一覧化し、空白ポジションを特定

## アトリビューションモデリング知識

施策評価時に適切なアトリビューションモデルを選択・推奨する:

| モデル | 概要 | 適用場面 |
|--------|------|---------|
| ファーストタッチ | 初回接点に全貢献を帰属 | 認知施策の効果測定 |
| ラストタッチ | 最終接点に全貢献を帰属 | CV直前施策の評価 |
| 線形（リニア） | 全接点に均等配分 | タッチポイントが少ない場合 |
| タイムディケイ | CV に近い接点ほど高配分 | 検討期間が長い商材（不動産等） |
| データドリブン | ML で貢献度を自動算出 | データ量が十分な場合のみ |

分析時に「どのモデルで評価しているか」を明示し、モデル選択の根拠を記載する。

## コンテンツギャップ分析手法

1. **競合コンテンツの棚卸し**: 競合が発信しているトピック・形式を一覧化
2. **自社コンテンツの棚卸し**: 自社の既存コンテンツ資産をマッピング
3. **ギャップの特定**: 競合がカバーしているが自社が未対応のテーマを抽出
4. **機会の優先順位付け**: 検索ボリューム × 競合の弱さ × 自社の専門性 で優先度決定
5. **コンテンツカレンダーへの反映**: ギャップを埋める施策を時系列で計画

## SEO競合分析

### キーワードギャップ分析
- 競合がランクインしているが自社が未対応のキーワードを特定
- 検索意図（情報収集型 / 比較検討型 / 購買型）ごとに分類
- 難易度 × ビジネスインパクトのマトリクスで優先順位付け

### 被リンクプロファイル比較
- 競合の被リンク元ドメインの質と量を調査
- 自社が獲得可能なリンク機会（業界メディア・ディレクトリ等）を特定
- コンテンツ型リンク獲得（調査レポート・インフォグラフィック等）の可能性を評価
