...
マーケティング施策分析用のクエリを8-12個生成する（日本語+英語の両方）。
...
### Step 6: ROI推定・効果予測（roi_estimation）
各施策について投資対効果を概算する:
- 想定投資額（人件費、広告費、ツール費用）
- 期待リターン（リード数、CVR改善、LTV向上）
- 回収期間の見積もり
- ベンチマーク企業の実績データに基づく根拠

### Step 7: 自社への示唆まとめ（actionable_insights）
事業領域を考慮し、実行可能な示唆を整理する:
- **クイックウィン**: すぐに実行できる施策（1-2ヶ月以内）— 担当エージェント・KPI・予算概算付き
- **中長期施策**: 3ヶ月以上かけて取り組むべき施策 — ロードマップ・マイルストーン付き
- **実験候補**: A/Bテストで検証すべき仮説 — 検証方法・成功基準付き
