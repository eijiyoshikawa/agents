...
### アトリビューションモデリング
```
分析対象: 競合のマーケティングチャネル貢献度推定
  - ラストクリック: 最終接点のチャネル
  - マルチタッチ: 複数接点の貢献度配分
  - 推定手法: 公開情報（広告出稿量・SEO順位・SNS活動量）から逆算
  → クライアントへの示唆: どのチャネルに投資すべきかの優先順位
```

### 競合のマーケティング成熟度評価
| レベル | 特徴 | 対策 |
|--------|------|------|
| Level 1 | 場当たり的な施策 | 体系的アプローチで差別化容易 |
| Level 2 | チャネル別最適化済み | クロスチャネル統合で差別化 |
| Level 3 | データドリブン運用 | AI活用・パーソナライズで差別化 |
| Level 4 | 統合マーケティング | ニッチ特化・コミュニティで差別化 |

### クリエイティブ分析の深度
```
コピー分析:
  - 訴求軸の分類（機能訴求/感情訴求/社会的証明/希少性/権威性）
  - AIDA/PAS フレームワークとの適合度
  - CTA の行動喚起力評価

ビジュアル分析:
  - 使用カラーのブランド戦略との整合性
  - 画像/動画の感情誘発効果（安心/興奮/信頼/共感）
  - レイアウトの視線誘導パターン（Z/F/ガットバーグ）
```

### プラットフォームアルゴリズム知識（2026年版）
| プラットフォーム | 重要シグナル | 推奨戦略 |
|---------------|------------|---------|
| Instagram | 保存率・シェア率・リール完視聴率 | 保存されるナレッジ系コンテンツ |
| TikTok | 完視聴率・ループ再生・コメント率 | 冒頭2秒のフック・短尺高密度 |
| YouTube | 視聴維持率・CTR・セッション時間 | サムネイル最適化・長尺高品質 |
| Google Ads | 品質スコア・関連性・LP体験 | LP最適化・広告文のA/Bテスト |

### 競合モート（堀）分析
```
競合の持続的競争優位を以下で評価:
  - ネットワーク効果: ユーザー数が価値を高めるか
  - スイッチングコスト: 顧客が離脱しにくい仕組み
  - コスト優位性: 規模の経済・学習効果
  - ブランド資産: 認知度・信頼・ロイヤルティ
  - データ優位性: 蓄積データによる精度向上
→ モートが弱い競合の隙を突く戦略を優先提案
```
