...
| モデル | 説明 | 適用場面 |
|--------|------|---------|
| ラストクリック | 最後の接点に100%帰属 | 短い検討期間の商材 |
| ファーストクリック | 最初の接点に100%帰属 | 認知施策の評価 |
| 線形 | 全接点に均等配分 | バランスの取れた評価 |
| 時間減衰 | CV近い接点ほど高評価 | 検討期間が長い商材 |
| データドリブン | 実データから貢献度算出 | 十分なデータがある場合 |

### コンテンツ分析の定量指標
| 指標 | 計算方法 | ベンチマーク |
|------|---------|------------|
| エンゲージメント率 | (いいね+コメント+保存+シェア)/インプレッション | IG:3-6%, TT:5-10% |
| コンテンツ効率 | CV数/投稿本数 | 月次で追跡 |
| バイラル係数 | シェア数/リーチ数 | 2%以上で良好 |
| 視聴完了率 | 最後まで視聴/総視聴 | TT:40%+, YT:50%+ |
| セーブ率 | 保存数/リーチ数 | 2%以上で高品質 |

### 競合デジタルフットプリント分析
1. SEO: 推定オーガニック流入・上位キーワード
2. 広告: 広告出稿量の推定・クリエイティブの方向性
3. SNS: フォロワー成長率・投稿頻度・エンゲージメントの質
4. コンテンツ: ブログ更新頻度・テーマ・キーワード戦略
5. テクノロジー: 使用ツール・マーテックスタックの推定

### アンチパターン
- バニティメトリクスに惑わされない: ビジネス成果直結指標を重視
- 自社バイアスを持ち込まない: 競合の施策を客観的に評価
- プラットフォーム間の指標を直接比較しない: 算出方法が異なる
- 相関と因果を混同しない
