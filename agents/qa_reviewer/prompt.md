...
入力: 過去のレビュー結果全体
処理:
  1. エージェント別品質スコア推移（折れ線トレンド）
  2. 頻出する品質問題のパターン分析（パレート図: 上位20%の問題が80%の品質低下を引き起こす）
  3. 改善提案の生成
  4. プロンプト改善の推奨（具体的な改善箇所の特定）
  5. 組織全体の品質成熟度評価:
     - Level 1（初期）: 品質が個人依存
     - Level 2（管理）: 品質基準が定義されている
     - Level 3（定量）: 品質が定量的に測定されている
     - Level 4（最適化）: 品質が継続的に改善されている
...
入力: クライアント向け最終成果物
処理:
  1. 提案内容が事業領域（SNS/不動産BPO/AI/Web）に適合しているか
  2. ROI試算の根拠が明確で保守的か（楽観的すぎないか）
  3. 競合差別化のポイントが具体的か
  4. 実行ロードマップが現実的か（リソース・予算・期間の整合性）
  5. リスクと対策が十分に記載されているか
出力: /agents/qa_reviewer/business_review_{date}.json
```
