...
## 高度品質管理スキル（Advanced Quality Management）

### 統計的品質管理
- **管理図（Control Chart）**: 品質スコアの推移をプロットし、管理限界線の逸脱を検知
- **UCL/LCL**: 平均±3σを管理限界として設定し、プロセスの安定性を判定
- **傾向検知**: 7点連続の上昇/下降、管理線の片側に連続9点等のルールで異常を早期発見

### パレート分析
品質問題の80%は20%の原因から発生する:
1. 品質指摘をカテゴリ別に分類・集計する
2. 頻度の高い順に並べ、累積比率を算出する
3. 上位20%のカテゴリに対して重点的に改善策を実施する
4. 月次でパレート図を更新し、改善効果を追跡する

### 品質コスト分析（COQ: Cost of Quality）
| コスト種別 | 説明 | 例 |
|-----------|------|-----|
| 予防コスト | 不良を防ぐための投資 | プロンプト改善、レビュー基準整備 |
| 評価コスト | 品質を確認するための費用 | QAレビューの工数 |
| 内部失敗コスト | 納品前に発見された不良の修正 | 差し戻し・再作成の工数 |
| 外部失敗コスト | 納品後に発見された不良の対応 | クレーム対応・信用失墜 |

### レビューのキャリブレーション
複数の品質基準が一貫して適用されることを保証する:
- **月次キャリブレーション**: 同一の出力を複数回評価し、スコアのばらつきを確認
- **評価基準の明文化**: 各スコア帯の具体的な出力例を蓄積
- **偏り検知**: エージェント別のスコア分布を分析し、過度に甘い/厳しい評価を是正

### 根本原因分析（品質問題向け）
品質スコアが低い場合の原因分析手法:
- **5 Whys**: 「なぜスコアが低いのか」を5回深掘り
- **Ishikawa**: プロンプト/入力データ/ツール/プロセス/相互干渉の5カテゴリで原因を分類
- **是正措置 vs 予防措置**: 今回の問題の修正（是正）と、同種の問題の再発防止（予防）を区別

### 品質文化の醸成
- **失敗から学ぶ文化**: 差し戻しを「罰」ではなく「学習機会」として位置づける
- **品質メトリクスの透明性**: 全エージェントが自身の品質スコアとトレンドを確認可能にする
- **ベストプラクティスの共有**: 高スコア出力のパターンを抽出し、全エージェントに共有
