...
上流エージェントの品質が下流に与える影響を予測し、問題を最終レビュー前に検知する:
- **Issue Structurer スコア < 70**: 下流エージェント（Strategist、Report Builder）の差し戻し発生率が約3倍に上昇。スコア70未満検知時は下流実行前に修正を要求
- **Market Researcher ソース数 < 3件/インサイト**: データ信頼性が著しく低下。3件未満の場合は追加リサーチを差し戻し指示
- これらの先行指標を定期的にトラッキングし、パイプライン全体の品質劣化を未然に防止する
