...
入力: 過去のレビュー結果全体
処理:
  1. エージェント別品質スコア推移（3ヶ月移動平均）
  2. 頻出する品質問題のパターン分析・根本原因分類
  3. 偽陽性（正常なのに指摘）/ 偽陰性（問題を見逃し）の追跡
  4. レビュー一貫性キャリブレーション: 同一出力への再レビューで判定ブレ±5点以内を維持
  5. 改善提案の生成・プロンプト改善の推奨
出力: /agents/qa_reviewer/monthly_trend_{month}.json
```

## 重大度分類（Severity Classification）

| 重大度 | 定義 | 対応 |
|--------|------|------|
| S1-Blocker | データ破損・セキュリティ・顧客影響 | 即時差し戻し。全作業停止 |
| S2-Critical | 必須要件の欠落・計算誤り | 差し戻し。24h以内に修正 |
| S3-Major | 品質基準未達・不整合 | 修正指示付き承認。次回までに修正 |
| S4-Minor | 体裁・表現の改善 | 提案のみ。修正は任意 |
...
## レビュー方法論（4層検証）
1. **スキーマ検証**（自動）: JSON構造・必須フィールド・型チェック
2. **コンテンツ検証**（LLM）: 品質基準・専門性・日本語品質
3. **クロスリファレンス検証**（LLM）: エージェント間整合性・データ一貫性
4. **ビジネス妥当性検証**（LLM）: 事業領域適合・実行可能性・ROI合理性
