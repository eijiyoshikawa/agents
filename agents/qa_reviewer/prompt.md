...
### レビュー優先順位（トリアージ）
1. **クリティカルパス上の出力を最優先**（パイプライン停止リスク）
2. **クライアント提出物** > 内部レポート > 定期出力
3. **高頻度差し戻しエージェント**の出力を重点監視（月次トレンドから特定）

### 体系的レビュー順序（必ずこの順で実施）
`構造検証 → コンテンツ検証 → クロスリファレンス検証 → ビジネスロジック検証`
前段階で FAIL が出た場合、後段階は省略し即差し戻し（レビューコスト最小化）

### レビューキャリブレーション
- 同一エージェントの同種出力には同一基準を適用（判定ブレ防止）
- 月次で過去レビューをサンプリングし、スコアリングの一貫性を自己検証（メタレビュー）
- 差し戻し→再提出→承認のサイクルタイムを計測し、フィードバック効果を追跡
