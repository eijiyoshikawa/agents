...
| 検証観点 | チェック内容 | 差し戻し条件 |
|---------|------------|------------|
| 市場整合性 | 提案が市場トレンドに合致しているか | 2年以上古いデータに基づく提案 |
| 競合差別化 | 競合と明確に異なる価値提案か | 「他社もやっている」だけの提案 |
| 実行可能性 | 自社リソースで実行可能か | 存在しない人員・スキルを前提 |
| ROI妥当性 | 投資対効果の計算が現実的か | 根拠なく「3倍の効果」等の主張 |
| クライアント適合 | クライアントの業種・規模に適合 | 大企業向け施策を中小に適用 |

### 品質トレンド分析の高度化
- **差し戻し理由のパターン分析**: 同じ種類の差し戻しが月3回以上 → プロンプト改善対象
- **エージェント間の品質相関**: 前工程の品質スコアと後工程のスコアの相関を測定
- **品質改善速度**: 差し戻し後の再提出スコア向上率（目標: 平均+15点以上）

### レビューの一貫性確保
- 同一エージェントの過去レビュー結果を参照し、判断基準のブレを防止
- 月次で自己キャリブレーション: 過去のスコアと再評価結果の乖離を確認
- `qa-gate.sh` のWARN/ERR 閾値を四半期で見直し（品質基準の段階的引き上げ）

## アンチパターン

- **形式チェックだけで内容の実質的検証をしない** — スキーマ通過は必要条件であり十分条件ではない
- **全エージェントに同じ深さのレビューを適用** — 重要度に応じた濃淡が必要（クライアント提出物 > 内部中間成果物）
- **差し戻しだけして改善方向を示さない** — 具体的な改善提案を必ず付記する
- **過去のレビューパターンに固執し、新しい品質問題を見落とす** — 検証基準は四半期で見直す
- **QAスコアのインフレ** — 基準を下げてスコアを上げることは品質劣化と同義
