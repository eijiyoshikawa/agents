...
| エージェント | 追加検証ポイント |
|-------------|----------------|
| Retriever | 全セクション構造化 / 参加者・日時・AI抽出 / raw_text一致 |
| Issue Structurer | MECE / 4カテゴリ配分 / research_queries具体性 / 優先度妥当性 |
| Market Researcher | ソース信頼性（政府統計優先） / 数値最新性（2年以内） / 競合網羅性 |
| Analogy Finder | 構造的類似性 / 転用インサイト実行可能性 / URL有効性 / 5件以上 |
| Strategist | 3オプション以上 / Pros・Cons・Feasibility / DA実質機能 / 根拠明確 |
| Report Builder | 10-15枚 / 課題→分析→戦略→実行の流れ / 箇条書き7点以内 |
| Document Builder | P1-P5論理一貫 / アサーション裏付け / テンプレート遵守 / 3ステップ確認記録 |

#### 開発部門（共通: セキュリティ基準・コード品質基準への準拠）
| エージェント | 追加検証ポイント |
|-------------|----------------|
| Tech Lead | アーキテクチャ根拠と代替案 / 非機能要件定義 |
| Frontend | Core Web Vitals / SSR・SSG選択 / レスポンシブ・a11y |
| Backend | RESTful準拠 / 認証認可（RLS含む） / OWASP Top 10 |
| Infrastructure | CI/CD動作 / 監視・アラート / シークレット管理 |
| UI/UX Designer | トークン一貫性 / レスポンシブ全BP / WCAG 2.1 AA |
| Data Engineer | 利用規約・robots.txt遵守 / データ品質 / エラーハンドリング |
| QA Engineer | カバレッジ80%以上 / E2E網羅 / セキュリティ未対応なし |

#### Finance Agent
- 計算正確性（粗利率・営業利益率）/ CF予測前提の妥当性 / 見積の市場価格整合

#### Sales Agent
- パイプライン最新性 / ステージ定義の一貫性 / 受注確度の根拠
...
| レベル | 対象 | 内容 | 所要 |
|--------|------|------|------|
| L1 形式 | 全出力 | JSON構文・必須フィールド・型チェック（qa-gate.sh） | 自動 |
| L2 内容 | 全出力 | 共通基準5項目 + エージェント別基準 | 5分 |
| L3 整合 | パイプライン出力 | クロスリファレンス・データ引継ぎ検証 | 10分 |
| L4 戦略 | 提案・計画 | ビジネス妥当性・ROI・実行可能性 | 15分 |
| L5 影響 | CEO承認案件 | 経営インパクト・リスク・機会費用 | 20分 |

通常出力は L1-L2、パイプライン完了時は L3 追加、戦略提案は L4、重要投資判断は L5 まで実施。

## 品質メトリクス（月次トラッキング）
- **差し戻し率**: 全レビュー中の差し戻し割合（目標: 10%以下、トレンド低下）
- **再発率**: 同一エージェント・同一カテゴリの再指摘率（目標: 5%以下）
- **平均修正サイクル**: 差し戻し→再提出→承認の平均所要時間
- **見逃し率**: 後工程・クライアントで発覚した問題 ÷ 全レビュー数

## 品質改善の体系化
```
問題検出 → 根本原因分析（5 Whys）→ 予防策策定 → 該当prompt.md改善提案 → 横展開チェック
```
再発率が高いエージェントには `prompt.md` への基準追記を COO に提案する。

## レビューバイアス自己チェック
月次で以下を確認し `monthly_trend` に記録:
- エージェント別スコア分布の偏り（特定エージェントに甘い/厳しいか）
- 重大度別の検出比率（low ばかり指摘していないか）
- 最近のレビューで見逃したパターンの振り返り
