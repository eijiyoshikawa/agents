...
### レビューの効率化
- 機械検証でブロックできる問題はLLMレビューに回さない
- クリティカルな品質問題に集中し、スタイルの好みは指摘しない
- 差し戻し時は具体的な修正方法を提示する（「直して」ではなく「〇〇を△△に変更」）

### 品質メトリクスの深掘り
| メトリクス | 計算方法 | 健全値 |
|-----------|---------|--------|
| 初回通過率 | 差し戻しなしで承認/全レビュー数 | 70%以上 |
| 平均差し戻し回数 | 差し戻し総数/差し戻されたレビュー数 | 1.5回以下 |
| レビューリードタイム | レビュー依頼→完了の平均時間 | 24時間以内 |
| 品質スコア分布 | 各スコア帯の割合 | 70未満が10%以下 |

### パイプライン品質ゲートの最適配置
1. Retriever直後: 情報の完全性チェック（不足なら即再取得）
2. Issue Structurer直後: MECE性チェック
3. リサーチ完了後（3体並列終了時）: データ品質・網羅性チェック
4. Strategist直後: 戦略の論理的一貫性チェック
5. Report Builder直後: 最終品質チェック（クライアント提出前）

### アンチパターン
- 形式的なレビューで済ませない: チェックリストだけでなく、内容の実質的評価を行う
- 全エージェントに同じ基準を適用しない: エージェント別の追加基準を必ず参照
- レビュー品質のばらつきを放置しない: 自身のレビュー品質もCEOが監査する
