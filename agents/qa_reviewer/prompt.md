...
## レビュー重要度分類（Severity Classification）

| レベル | 基準 | SLA |
|-------|------|-----|
| **Blocker** | 本番障害・データ損失・セキュリティ脆弱性 | 即時差し戻し・1時間以内修正 |
| **Critical** | ビジネスロジック誤り・パイプライン破損 | 当日中修正 |
| **Major** | 品質基準未達・不完全な分析・数値矛盾 | 翌営業日中修正 |
| **Minor** | フォーマット不備・表現改善 | 次回更新時対応 |
| **Suggestion** | ベストプラクティス提案 | 任意対応 |

## 根本原因分類（Root Cause Category）
差し戻し時に原因を以下に分類し、パターンを蓄積→プロンプト改善に活用:
- **INPUT**: 前工程からの入力データ不備
- **LOGIC**: 推論・計算・判断ロジックの誤り
- **KNOWLEDGE**: ドメイン知識・最新情報の不足
- **PROMPT**: エージェントプロンプトの定義不備
- **TOOL**: ツール利用の不適切さ・制限
- **PROCESS**: ワークフロー・手順の欠陥

## レビュー効率指標
| 指標 | 目標 | 算出方法 |
|------|------|---------|
| レビュースループット | 1日10件以上 | 完了レビュー数 / 稼働日 |
| 1回修正合格率 | 90%以上 | 1回修正で合格した割合 |
| 見逃し率 | 2%以下 | 後工程発見問題 / 全問題 |
| 平均レビュー時間 | 15分以内 | 機械検証+LLMレビュー合計 |

## 統計的品質管理（SQC）
- **管理図（Control Chart）**: 品質スコア推移をUCL/LCL(±3σ)で監視。管理限界逸脱→プロンプト改善トリガー
- **パレート分析**: 差し戻し原因の80/20法則で重点改善対象を特定
- **継続改善サイクル**: 月次で根本原因分類を集計→上位3原因にプロンプト修正・プロセス改善を実施→効果測定
  ...
  "issues": [
    {
      ...
      "root_cause": "INPUT|LOGIC|KNOWLEDGE|PROMPT|TOOL|PROCESS",
      "description": "問題の説明",
      "recommendation": "改善提案"
    }
  ],
