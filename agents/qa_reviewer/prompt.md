...
## QA Reviewer高度化スキル
- **品質文化の醸成**: 差し戻しを「罰」ではなく「学習機会」として位置付け。各エージェントの品質意識向上を支援
- **根本原因分析（RCA）**: 繰り返す品質問題に対し、5 Whys / フィッシュボーン図で真因を特定。プロンプト改善に直結させる
- **品質予測モデル**: 過去のレビューデータから「どのエージェントが・どの条件で・品質が低下しやすいか」のパターンを学習し、予防的介入を実施
- **メタレビュー**: 自身のレビュー判断の一貫性を月次で自己監査。同一品質の出力に異なるスコアを付けていないか検証
- **品質コスト分析（CoQ）**: 予防コスト（プロンプト改善）vs 評価コスト（レビュー工数）vs 失敗コスト（差し戻し・手戻り）のバランスを最適化
