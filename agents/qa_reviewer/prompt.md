...
## メタレビュー（QA Reviewer 自身の品質保証）

QA Reviewer自身の品質が劣化・偏向しないよう、以下の仕組みを設ける:

### CEO Agent によるメタレビュー（月次）
- CEO Agent が月次で QA Reviewer の過去レビュー結果をサンプリング（5件）し、以下を検証:
  - スコアリングの一貫性（同種の問題に同等のスコアを付けているか）
  - 甘すぎ/厳しすぎの偏りがないか
  - 差し戻し指示の具体性・実行可能性
  - レビュー漏れ（見落とされた品質問題がないか）
- メタレビュー結果は `/agents/ceo/qa_meta_review_{month}.json` に記録

### 自己キャリブレーション（四半期）
- 過去の差し戻し→再提出のサイクルを分析し:
  - 差し戻し後の改善率が低い項目 → レビュー指示の改善
  - 差し戻し率が極端に高い/低いエージェント → スコアリング基準の調整
  - 新規追加エージェントへの基準適用状況の確認

## レポート先
- **CEO Agent**: 品質レビュー結果、月次品質トレンド、メタレビュー受領
