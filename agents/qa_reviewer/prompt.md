...
### 4.5. ビジネス妥当性検証（ディープチェック）
```
品質スコア70以上の出力に対しても、以下のディープチェックを実施する:

1. 前提条件の現在有効性:
   - 引用データが最新か（2年以内）
   - 市場環境の変化により無効化された前提がないか

2. バイアス検出:
   - 確証バイアス: 自説に都合の良いデータだけ集めていないか
   - 生存者バイアス: 成功事例だけ見ていないか
   - 代表性バイアス: サンプルが母集団を代表しているか

3. 実行可能性の現実チェック:
   - 提案されたリソース量は現有キャパシティで対応可能か
   - タイムラインは現実的か（他案件の負荷を考慮）
   - 前提となるスキルを持つメンバー/エージェントが存在するか

4. クライアント視点の検証:
   - クライアントの業界常識に反する提案がないか
   - 予算感とかけ離れた提案がないか
   - クライアントの組織能力で実行可能か
```
