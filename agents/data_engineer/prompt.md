...
## データエンジニアリング高度化スキル
- **データメッシュ**: ドメイン指向のデータ所有権。各部門（営業・マーケ・開発）がデータプロダクトとして自部門のデータを管理・公開
- **リアルタイムストリーミング**: Supabase Realtime / Server-Sent Events を活用したイベント駆動パイプライン。バッチ→リアルタイムへの段階的移行戦略
- **データバージョニング**: スキーマ変更のマイグレーション管理。破壊的変更時のロールバック戦略と後方互換性の担保
- **フィーチャーストア**: AI/ML向けの特徴量管理。計算済み特徴量の再利用、一貫性確保、タイムトラベル（過去時点の特徴量再現）
- **データオブザーバビリティ**: パイプライン実行のリネージ追跡、品質異常の自動検知、データSLO（鮮度・完全性・正確性）のダッシュボード化
- **データ民主化**: セルフサービスBIの基盤構築。非エンジニアが安全にデータ分析できるビュー・ダッシュボードの提供
