...
| パターン | 適用場面 | ツール例 |
|---------|---------|---------|
| バッチ処理 | 日次集計・月次レポート | cron + Python |
| マイクロバッチ | 準リアルタイム（5-15分間隔） | Supabase Edge Functions |
| ストリーミング | リアルタイム通知 | Supabase Realtime |
| CDC（Change Data Capture） | DB変更の即時反映 | Supabase Webhooks |

### Webクローリングの高度化
- **robots.txt パーサー**: 必ず最初にチェック、disallow 遵守
- **クロール間隔**: 最低1秒間隔（同一ドメイン）、429エラー時は指数バックオフ
- **User-Agent**: 正直に自社ボット名を設定（偽装しない）
- **ヘッドレスブラウザ**: SPA サイトは Playwright でレンダリング後にスクレイピング
- **差分検知**: 前回取得データとのハッシュ比較で変更分のみ処理

### データ品質の自動検証パイプライン
```
取得 → スキーマ検証 → 型チェック → 範囲チェック → 重複排除 → 欠損補完 → 格納
```
各ステップで不合格の場合、アラートを送信し手動確認を要求する。
...
| フェーズ | ルール |
|---------|--------|
| 収集 | 利用規約・robots.txt 遵守、Legal Agent 承認 |
| 保管 | 暗号化、アクセス制御、保持期間設定 |
| 利用 | 目的外利用禁止、匿名化処理 |
| 廃棄 | 保持期間超過データの定期削除 |

## アンチパターン
- robots.txt / 利用規約を無視したスクレイピング
- エラーハンドリングなしのパイプライン（失敗を検知できない）
- データの鮮度を管理しない（いつ取得したか不明なデータ）
- 個人情報を含むデータをログに出力
- パイプラインのテストなしで本番投入
