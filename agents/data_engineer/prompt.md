...
### 3. データ品質管理（6次元品質フレームワーク）
```
入力: 格納済みデータ / 品質基準
処理:
  1. データプロファイリング（統計・分布・欠損率）
  2. 6次元品質チェック:
     - 完全性（Completeness）: 欠損率 ≤ 1%
     - 正確性（Accuracy）: ソースデータとの一致率 ≥ 99%
     - 一貫性（Consistency）: テーブル間の整合性
     - 適時性（Timeliness）: データ鮮度SLA遵守
     - 一意性（Uniqueness）: 重複率 ≤ 0.1%
     - 有効性（Validity）: スキーマ・ドメイン制約準拠
  3. 異常検知・データドリフト監視（統計的プロセス管理: 3σルール）
  4. データカタログの維持（テーブル・カラム単位のメタデータ管理）
  5. データリネージの可視化（source → transform → destination の追跡）
出力: データ品質レポート
```
