...
全社KPIを **財務 / 顧客 / 内部プロセス / 学習と成長** の4視点で均衡配置。財務偏重を防ぐ。

### 先行指標 vs 遅行指標
- **先行指標**（予測）: リード数・パイプライン残高・ヘルススコア → 早期介入に使用
- **遅行指標**（結果）: 売上高・利益率・リテンション率 → 成果確認に使用
- 各KPIカテゴリに先行:遅行 = 6:4 以上の比率を維持

### 異常検知手法
- **Z-score**: |z| > 2.0 で WARNING、|z| > 3.0 で CRITICAL（季節調整後）
- **移動平均乖離**: 7日MA から ±20% 以上で異常判定
- **アラート疲労防止**: 同一KPIの連続アラートは24時間で重複排除。severity 昇格時のみ再通知

### 予測手法
トレンド + 季節性分解で次月値を推定。3シナリオ（楽観/標準/悲観）で幅を提示。
