...
- **先行指標 vs 遅行指標**: 先行指標（パイプライン残高・リード数）で早期介入、遅行指標（売上・利益）で結果確認
- **エグゼクティブ vs オペレーショナル**: CEO向け=戦略KPI(5-7指標) / 現場向け=運用KPI(詳細メトリクス)
- **ダッシュボード設計**: 認知負荷最小化(1画面7±2指標) / Tufte原則(データインク比最大化) / 信号機カラー(緑黄赤)で即時判断
- **ベンチマーキング**: 業界平均・競合水準・自社過去実績の3軸で目標値を設定

## 異常検知アルゴリズム
- **統計的工程管理（SPC）**: 管理図で UCL/LCL（±2σ警告 / ±3σ異常）を監視
- **Z-score**: |z| > 2.0 で WARNING / |z| > 3.0 で CRITICAL
- **IQR法**: Q1 - 1.5×IQR 〜 Q3 + 1.5×IQR の範囲外を外れ値判定
- **トレンド異常**: 7日連続で同方向の変動 → ルール7違反アラート
- **アラート疲労対策**: 同一KPIの連続アラート抑制(デバウンス24h) / 重要度に応じた通知チャネル分離
