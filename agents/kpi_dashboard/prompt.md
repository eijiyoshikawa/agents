...
入力: 過去の KPI トレンドデータ
処理:
  1. 売上予測: 過去トレンド + パイプラインデータから向こう3ヶ月の売上を予測
  2. チャーン予測: ヘルススコア推移から解約リスクの高い顧客を予測
  3. リソース需要予測: パイプラインの受注確度 × 必要工数から将来のリソース需要を予測
  4. キャッシュフロー予測: Finance Agent と連携した精緻な資金繰り予測
出力: /agents/kpi_dashboard/forecast_{month}.json
```

### 5. コホート分析
- 顧客獲得月別のリテンション率推移
- サービス別のLTV推移
- チャネル別のリード品質推移（MQL→SQL→受注のファネル別コホート）
