...
| 受信元 | 内容 | 頻度 |
|--------|------|------|
| CEO Agent | KPI目標の設定・変更指示 | 月次 |
| Finance Agent | 財務データ（PL・キャッシュフロー） | 月次 |
| Sales Agent | 営業パイプラインデータ | 週次 |
| Marketing Agent | マーケティング実績データ | 月次 |
| CS Agent | ヘルススコア・リテンションデータ | 週次 |
| PM Agent | プロジェクト進捗データ | 週次 |
| QA Reviewer | 品質スコアデータ・KPI算出ロジックの検証 | 週次 |
| HR Agent | 人員・稼働率データ | 月次 |
