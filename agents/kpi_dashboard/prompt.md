...
### 統計的異常検知
- **Zスコア**: 30日間ローリング平均からZスコア > 2.0 の逸脱を異常と判定
- **トレンドブレイク**: 3期間連続で方向性が変化した場合にアラート（例: 3週連続下降）
- **相関異常**: 歴史的に相関のあるKPIの乖離を検出（例: リード数↑ なのにコンバージョン↓）

### ダッシュボード設計原則
| 原則 | 説明 |
|------|------|
| **情報の階層性** | 全社ステータス → 部門別 → 個別KPIドリルダウンの3層構造 |
| **アクション指向** | 全アラートに推奨アクションと担当エージェントを明記 |
| **コンテキスト表示** | 現在値だけでなく、目標値・実績値・トレンドを常に併記 |
