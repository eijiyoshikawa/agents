...
### 3.5. 先行指標・遅行指標の管理
```
KPIを先行指標と遅行指標に分類し、先行指標の変化から将来を予測する:

先行指標（Leading Indicators）— 将来の結果を予測する:
  | 先行指標 | 予測する結果 | 時間差 |
  |---------|------------|--------|
  | Web訪問数 | リード数 | 2-4週間 |
  | リード数 | 商談数 | 2-4週間 |
  | 商談数 | 受注数 | 4-8週間 |
  | ヘルススコア低下 | チャーン | 1-3ヶ月 |
  | 品質スコア低下 | 差し戻し増加 | 1-2週間 |
  | エンゲージメント率低下 | フォロワー減少 | 2-4週間 |

遅行指標（Lagging Indicators）— 結果を確認する:
  - 月間売上、営業利益率、リテンション率、NPS、受注率

先行指標のトレンド変化を検知した時点で、遅行指標の悪化を待たずにアラートを発出する。
```

### 3.6. コホート分析ダッシュボード
```
月次でコホート別の指標を追跡:
  1. 顧客コホート（契約月別のリテンション推移）
  2. リードコホート（流入月別のSQL転換推移）
  3. プロジェクトコホート（開始月別の納期遵守推移）

可視化: 三角形のコホートテーブルで月ごとの推移を一覧化
→ 特定コホートの異常（急激なチャーン、転換率低下）を早期検知
```
