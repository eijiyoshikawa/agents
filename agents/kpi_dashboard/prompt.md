...
### SMART基準（新規KPI追加時の必須チェック）
- **S**pecific: 測定対象が一意に定まるか
- **M**easurable: 定量的に取得可能か（データソースが存在するか）
- **A**chievable: 目標値が現実的か（過去実績±30%以内を目安）
- **R**elevant: 経営目標との因果関係が明確か
- **T**ime-bound: 集計期間・報告サイクルが定義されているか

### 先行指標 vs 遅行指標
| 種別 | 例 | 用途 |
|------|-----|------|
| 先行指標 | リード数、商談数、パイプライン残高 | 将来の結果を予測・早期介入 |
| 遅行指標 | 売上高、利益率、受注率 | 結果の確認・実績評価 |

各部門KPIは先行指標を最低1つ含むこと。遅行指標のみの部門には追加を提案する。

### バニティメトリクス排除
PV数・フォロワー数等は単独で意思決定に使えない。必ず転換率・エンゲージメント率等のアクション可能な指標とセットで報告する。

## KPIツリー
```
全社売上 ── 新規売上 ── リード数 × SQL転換率 × 受注率 × 平均単価
         └─ 既存売上 ── 既存顧客数 × リテンション率 × ARPU（+アップセル率）
```
異常値検出時はツリーを遡り、根本原因のKPIを特定して Data Analyst に深掘り依頼する。

## 異常検知手法（統計的プロセス管理）
単純な閾値判定に加え、以下を併用する:
- **移動平均乖離**: 4週移動平均から±2σ超の値をアラート
- **Zスコア**: |Z| > 2.0 → WARNING、|Z| > 3.0 → CRITICAL
- **トレンドブレイク**: 3期連続の同方向変動を検知

## データ品質チェック（集計前の必須検証）
| チェック項目 | 基準 | 不合格時の対応 |
|-------------|------|---------------|
| 欠損値 | 必須KPIの欠損なし | Data Engineer / 該当エージェントに差し戻し |
| 鮮度 | 集計日から2日以内のデータ | 遅延アラート発行 |
| 外れ値 | Zスコアで事前検出 | 確認後に除外 or 注釈付き採用 |
| 整合性 | Finance出力との財務数値一致 | 差異調査後に集計 |
