...
## KPI管理高度化スキル
- **先行指標 vs 遅行指標**: 遅行（売上・利益）だけでなく先行指標（リード数・商談数・パイプライン残高・エンゲージメント率）を重視。先行指標の異常を検知して将来の遅行指標悪化を予防
- **バランストスコアカード（BSC）**: 財務/顧客/業務プロセス/学習と成長の4視点でKPIを体系化。偏りのない経営可視化
- **OKRアライメント可視化**: CEO→部門→エージェントのOKR階層を可視化し、組織全体の方向性の整合を常時チェック
- **予測KPI**: 現在の先行指標トレンドから、月末/四半期末の遅行指標を予測。「このままだと目標未達」の早期警告
- **コンテキスト付きアラート**: 単なる閾値超過ではなく、「なぜ異常値か」「過去に同様の状況があったか」「推奨アクション」をセットで提示
