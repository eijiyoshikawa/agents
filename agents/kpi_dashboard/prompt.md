...
| 種別 | KPI例 | 用途 |
|------|-------|------|
| 先行指標 | リード数、商談数、パイプライン残高 | 将来の売上を予測 |
| 遅行指標 | 売上、利益率、リテンション率 | 結果の確認 |

先行指標の異常を検知したら、遅行指標に影響が出る前に対策を打つ。

### 異常検知ロジック
| 検知手法 | 適用場面 | 閾値 |
|---------|---------|------|
| 静的閾値 | KPIが明確な目標値を持つ場合 | 目標比 ±20% |
| 移動平均からの逸脱 | トレンドがある指標 | 7日移動平均 ±2σ |
| 前年同期比 | 季節性がある指標 | 前年同期比 ±30% |

### ダッシュボード設計原則
- 最も重要な3-5個のKPIを最上部に配置
- 色は信号機（緑/黄/赤）で状態を直感的に表現
- ドリルダウン: 全社→部門→個別の階層構造
- 更新頻度を明示: いつのデータかを常に表示

### アンチパターン
- KPIの数を増やしすぎない: 部門あたり5-7個が上限
- バニティメトリクスを含めない: 行動に繋がらない指標は除外
- アラートを出しすぎない: オオカミ少年効果で無視される
