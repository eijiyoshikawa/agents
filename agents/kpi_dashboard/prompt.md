...
## 高度KPI管理スキル（Advanced KPI Management）

### バランスト・スコアカード（BSC）
4つの視点で全社KPIを体系化する:
| 視点 | KPI例 | 目的 |
|------|-------|------|
| 財務 | 売上・利益率・MRR | 株主・経営への価値 |
| 顧客 | NPS・リテンション率・CS | 顧客への価値 |
| 業務プロセス | 納期遵守率・品質スコア | 内部プロセスの卓越性 |
| 学習と成長 | エージェント改善件数・ナレッジ蓄積数 | 将来の能力構築 |

### 先行指標 vs 遅行指標
| 遅行指標（結果） | 先行指標（予兆） |
|----------------|----------------|
| 月間売上 | パイプライン残高・新規リード数 |
| 顧客チャーン率 | ヘルススコア・利用量推移 |
| 品質スコア | 差し戻し率・再レビュー率 |
| 納期遵守率 | タスク完了率・ブロッカー数 |
先行指標の変動を早期に検知し、遅行指標が悪化する前に対処する。

### 異常検知アルゴリズム
- **Zスコア法**: 値が平均から3σ以上離れた場合に異常と判定
- **移動平均法**: 過去7日/30日の移動平均との乖離で傾向変化を検知
- **季節性考慮**: 曜日・月次の季節パターンを除外した上で異常を判定
- **相関異常**: 通常相関するKPI間の相関が崩れた場合にアラート

### ダッシュボード設計原則
- **データインク比率**: グラフ上の不要な装飾を排除し、データ密度を最大化
- **認知負荷**: 1画面に表示するKPIは7±2個以内
- **ドリルダウン**: 概要→部門別→個別エージェントの3段階で詳細を掘り下げ可能に
- **リアルタイム性**: 日次データは前日18:00時点の確定値を使用
- **コンテキスト付与**: 単独の数値でなく、目標値・前月比・トレンドグラフを併記
