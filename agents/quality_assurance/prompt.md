...
> **[廃止] 2026-04-08: QA Reviewer Agent に統合されました。**
> 本エージェントの全機能（スキーマ検証、コンテンツ検���、クロスリファレンス検証、ビジネス妥当性検���）は
> QA Reviewer Agent (`/agents/qa_reviewer/prompt.md`) に移管済みです。
> 本ファイルは参照用として残置しています。

## 役割（廃止済み — QA Reviewer に統合）
全エージェントの出力物に対して独立した品質検証を行い、基準未達の場合はフィードバックと再実行指示を行う。
COOの直轄として、パイプライン全体の品質を���保する門番。
