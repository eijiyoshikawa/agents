...
## 高度実装スキル（Advanced Implementation Skills）

### 工数見積技法
| 手法 | 説明 | 精度 |
|------|------|------|
| PERT | 楽観値×1 + 最頻値×4 + 悲観値×1 ÷ 6 | 高 |
| 類推法 | 過去の類似案件の実績から推定 | 中 |
| フィボナッチポイント | 1/2/3/5/8/13 の相対見積 | チーム利用向け |
| Tシャツサイジング | S/M/L/XL の概算 | 初期スクリーニング |
...
### 技術的負債管理
- **即座に返済**: セキュリティ脆弱性、データ損失リスク
- **計画的に返済**: パフォーマンス劣化、テスト不足
- **許容**: 将来変更予定の箇所、プロトタイプコード
- **記録**: TODO/FIXME コメント + Issue トラッカーに必ず記録

### デプロイ戦略
| 戦略 | 説明 | リスク |
|------|------|--------|
| Blue-Green | 2環境を切替え | 低（即時ロールバック可能） |
| Canary | 一部ユーザーに先行リリース | 低 |
| ローリング | 段階的に全インスタンスを更新 | 中 |
| Big Bang | 全ユーザーに一斉リリース | 高（ロールバック困難） |

### パフォーマンスプロファイリング
- **Lighthouse CI**: PR毎にパフォーマンススコアを自動計測
- **Bundle Analysis**: バンドルサイズの変化を追跡し、肥大化を早期検知
- **Server Timing**: APIレスポンスタイムをヘッダーで計測
- **Core Web Vitals**: LCP/INP/CLS を実ユーザーデータ（RUM）で継続監視

### セキュリティハードニング
- **入力バリデーション**: Zod で全入力を型安全にバリデーション
- **出力エスケープ**: React の自動エスケープ + dangerouslySetInnerHTML は禁止
- **CORS設定**: 必要なオリジンのみ許可（ワイルドカード禁止）
- **HTTPヘッダー**: CSP, X-Frame-Options, X-Content-Type-Options を設定
- **依存関係**: npm audit を CI に組み込み、Critical は即日対応
