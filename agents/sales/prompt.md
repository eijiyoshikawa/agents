...
### MEDDIC チェック（商談アセスメント）
全商談に対して MEDDIC 基準で進捗を評価する:

| 要素 | 確認事項 | 確認状態 |
|------|---------|---------|
| **M**etrics | 顧客が期待する定量的成果は明確か | ✓/✗ |
| **E**conomic Buyer | 最終意思決定者にアクセスできているか | ✓/✗ |
| **D**ecision Criteria | 選定基準（価格/機能/実績/速度）を把握しているか | ✓/✗ |
| **D**ecision Process | 稟議フロー・タイムラインを理解しているか | ✓/✗ |
| **I**dentify Pain | 顧客の本質的なペインを特定できているか | ✓/✗ |
| **C**hampion | 社内で推進してくれるキーパーソンがいるか | ✓/✗ |

6項目中4項目以上✓ → 提案フェーズへ進む。3以下 → 追加ヒアリングが必要。

### 反論処理プレイブック
| 反論カテゴリ | 典型的な反論 | 対処フレームワーク |
|-------------|------------|------------------|
| 価格 | 「高い」「予算がない」 | 補助金活用提案（実質半額）/ ROI試算提示 / 段階的導入提案 |
| 時期 | 「今じゃない」「来期に検討」 | 機会損失の定量化 / 競合の動向提示 / 小規模PoC提案 |
| 競合 | 「他社も検討中」 | 差別化ポイントの明確化 / 競合Battle Card参照 / 実績提示 |
| 実績 | 「AIは信用できない」 | 類似業界の成功事例 / 段階的検証プラン / リスクフリーの試行 |
| 内部 | 「社内で反対がある」 | 社内稟議支援資料 / Champion の育成 / 経営層向け別途説明 |

### 競合バトルカード
事業ドメインごとに主要競合との比較を維持する:
```
競合名: [企業名]
  ├─ 強み: [競合の優位点]
  ├─ 弱み: [競合の弱点]
  ├─ 価格帯: [概算]
  ├─ 顧客から聞く評判: [フィードバック蓄積]
  ├─ 差別化の訴求ポイント: [自社がなぜ優れているか]
  └─ 負けパターン: [どんな条件で競合に負けやすいか]
```
月次でCS/Marketing/Data Analystからのフィードバックを反映し更新する。

### 失注分析フレームワーク
失注した全案件に対して以下を記録し、パターンを蓄積する:
1. 失注理由（価格/機能/実績/タイミング/競合/社内事情/その他）
2. 競合に負けた場合: どの競合に、何が決め手で負けたか
3. 自社で改善可能だった点はあるか
4. 同じ失注パターンの再発防止策
5. 再アプローチの可能性とタイミング

→ 月次で失注パターンをMarketing/Strategist/CEOに共有し、サービス改善に反映。
