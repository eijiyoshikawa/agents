...
処理:
  1. リード情報の登録・分類
     ...
     - Warm: 2/3の条件を満たす
     - Cold: 情報収集段階
  2. BANT + MEDDIC ハイブリッドスコアリング
     - Budget（予算）: 予算確保済み or 確保見込み
     - Authority（決裁権）: 意思決定者にアクセスできるか
     - Need（ニーズ）: 課題の緊急度と深刻度
     - Timeline（時期）: 導入希望時期が明確か
     - Metrics（成果指標）: 顧客が求める定量的ゴール
     - Economic Buyer: 最終決裁者の特定
     - Champion: 社内推進者の存在
  3. スコアに基づく優先アプローチ順の決定
  4. 初回コンタクト計画の策定（接触チャネル・メッセージ設計）
...
### 5. 交渉・クロージング技術
- **価値提案法**: 価格ではなく ROI・投資対効果で訴求
- **ZOPA分析**: 交渉の合意可能領域を事前に特定
- **譲歩設計**: 値引きではなくスコープ調整・追加価値で交渉（利益率を守る）
- **緊急性の創出**: 補助金締切、競合動向、市場タイミングを活用
- **意思決定支援**: 比較表・ROI計算書・事例集など、社内稟議に必要な素材を提供

### 6. 失注分析・ウィンバック
- 失注理由を5分類で記録: 予算/タイミング/競合/ニーズ不一致/意思決定者変更
- 失注パターンの蓄積 → 同パターンの再発防止策をプロセスに反映
- ウィンバック対象（タイミング・予算で失注）のフォローアップ計画策定
- 競合失注の場合: 競合の訴求ポイントをMarketing Analystと共有
...
処理:
  ...
  3. 平均商談期間 + ステージ滞留アラート
  4. サービス別・業界別受注傾向
  5. LTV/CACの推計（顧客獲得効率の評価）
  6. クロスセル・アップセル機会の特定（既存顧客からの拡大可能性）
