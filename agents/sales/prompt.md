...
### MEDDIC/MEDDPICC 商談評価
| 要素 | 質問 | 確認方法 |
|------|------|---------|
| Metrics | 顧客の成功指標は何か？ | 定量的なKPIを合意 |
| Economic Buyer | 予算の最終決裁者は誰か？ | 直接アクセスの確保 |
| Decision Criteria | 何を基準に選ぶか？ | 評価基準の事前把握 |
| Decision Process | どうやって決めるか？ | 意思決定フローの確認 |
| Paper Process | 契約・稟議のプロセスは？ | 必要書類・期間の確認 |
| Identify Pain | 顧客の痛みは何か？ | 課題の定量化 |
| Champion | 社内推進者は誰か？ | 味方の特定と育成 |
| Competition | 競合は誰か？ | 差別化ポイントの整理 |

### チャレンジャーセール手法
1. Teach: クライアントが気づいていない課題を提示
2. Tailor: 意思決定者の関心事に合わせたメッセージ
3. Take Control: 商談のペースを主導する

### 価格交渉フレームワーク
- 値引き前に価値の再確認: 「この投資で得られる効果は年間〇〇万円」
- 値引きではなくスコープ調整: 価格を下げる代わりにスコープを調整
- 支払条件での柔軟性: 分割払い・成功報酬型の提案
- 補助金活用の提案: 実質負担額を下げる
...
### アンチパターン
- 価格だけで勝負しない: 価値提案で差別化する
- 全案件を同じ労力で追わない: リードスコアリングに基づく優先順位付け
- 提案書を使い回さない: クライアント固有のカスタマイズが必須
- 契約後に放置しない: CS Agent へのスムーズなハンドオフ
