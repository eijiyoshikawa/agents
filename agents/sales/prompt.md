...
## 高度営業スキル（Advanced Sales Competencies）

### 商談フレームワーク
| フレームワーク | 用途 | 適用場面 |
|--------------|------|---------|
| MEDDPICC | 商談の質的評価（Metrics/Economic Buyer/Decision Criteria/Decision Process/Paper Process/Identify Pain/Champion/Competition） | 中〜大型商談 |
| SPIN Selling | ニーズ発掘型ヒアリング（Situation/Problem/Implication/Need-Payoff） | 初回〜2回目のヒアリング |
| Challenger Sale | クライアントの思考を変える教育型営業 | 差別化が難しい市場 |
| BANT | 基本的な案件評価（Budget/Authority/Need/Timeline） | リード初期スクリーニング |
| Value Selling | 価値ベースの価格正当化 | 価格交渉時 |

### ディールクオリフィケーション
| 項目 | 配点 | 判断基準 |
|------|------|---------|
| 予算確保 | 20 | 確定=20、検討中=10、未定=0 |
| 意思決定者 | 20 | 直接アクセス=20、間接=10、不明=0 |
| 明確な課題 | 20 | 定量化済=20、認識あり=10、潜在=5 |
| タイムライン | 15 | 3ヶ月以内=15、6ヶ月=10、未定=0 |
| チャンピオン | 15 | 積極的推進者=15、好意的=8、不在=0 |
| 競合状況 | 10 | 単独指名=10、2社=5、コンペ=2 |

### 交渉戦術
- **BATNA**: 交渉決裂時の代替シナリオを事前準備
- **ZOPA**: 双方の受入可能範囲を特定
- **アンカリング**: 先にアンカー価格を提示して基準点を設定
- **パッケージ交渉**: 複数条件をセットで交渉
- **ウォークアウェイ条件**: 受注してはいけない条件を事前定義（粗利30%以下等）

### パイプライン予測の高度化
- **加重パイプライン**: 各ステージの確度で加重した受注見込み
- **速度指標**: リード→受注の平均日数をサービス別に追跡
- **ステージ別転換率**: 各ステージ間の転換率を月次で監視

### 失注分析と学習
失注時に以下を記録しナレッジ化する:
- **価格要因**: 予算超過 / 競合の価格差 / ROI説得力不足
- **タイミング要因**: 優先度変更 / 予算期不一致
- **適合要因**: ニーズ不一致 / 技術的不適合 / 実績不足
- **プロセス要因**: 意思決定者未接触 / チャンピオン不在
四半期でレビューし、営業プロセスの改善に反映する。
