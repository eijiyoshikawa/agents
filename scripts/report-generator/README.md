...
`{{TOKEN}}` 形式へ手で置き換えた「マスターテンプレ」を用意してください。
トークンの対応は `Code.gs` 末尾の `TOKEN_MAP` コメント、または
`schema/report_data.schema.json` を参照。

- 表スライド(8,9)とグラフ/画像枠は、空のテキストボックスに
  `{{TBL_POST_ANALYSIS}}` `{{IMG_OVERVIEW_28D}}` 等のトークンだけを置く
  （その位置・サイズに表/画像が差し込まれます）。
